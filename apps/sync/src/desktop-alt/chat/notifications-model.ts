...
import { dayKey, dayLabel } from '../../lib/notificationGroups';
 ...
 */
export function mapServerType(type: string | null | undefined): NotificationDisplayKind {
  ...
  if (
    ...
    t === 'story_finished' ||
    (t.includes('agent') && (t.includes('finish') || t.includes('complete') || t.includes('done')))
  ) {
  ...
  if (
    ...
    (t.includes('agent') && (t.includes('review') || t.includes('action_needed'))) ||
    t.includes('action_needed')
  ) {
    return 'agent_review_request';
  }

  if (t === 'file_share' || t === 'file_shared' || t.includes('file_share') || t === 'share') {
    return 'file_shared';
  }

  if (t === 'dm' || t === 'dm_received' || t === 'message') return 'dm_received';
...
export function typeIconForKind(kind: NotificationDisplayKind): NotificationTypeIcon {
  switch (kind) {
    case 'mention':
      return 'mention';
...
export function actionButtonsFor(
  ...
  if (k.endsWith('_decline') || k.includes('decline')) {
    return [{ id: k, label: 'Decline', actionKind: kind, variant: 'secondary' }];
  }
  if (k.includes('block')) {
...
export function parseNotificationsResponse(
  ...
  } else if (typeof raw.unreadCount === 'string' && raw.unreadCount.trim() !== '') {
    const n = Number(raw.unreadCount);
    unreadCount = Number.isFinite(n) ? Math.max(0, Math.floor(n)) : items.filter((i) => i.status === 'unread').length;
  } else {
 ...
 */
export function reduceReadAll(state: NotificationsFeedState): NotificationsFeedState {
  return {
    ...state,
    unreadCount: 0,
...
export const NOTIFICATIONS_UNSUPPORTED_MESSAGE = 'Notifications are not available on this server yet';
