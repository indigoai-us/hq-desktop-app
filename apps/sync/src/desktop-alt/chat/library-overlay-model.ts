...
import { packIdentity } from '../../lib/packages';
 ...
 */
export function resolveOverlayTab(tab: LibraryTab | undefined | null): LibraryOverlayTab {
  if (tab === 'workers') return 'workers';
  if (tab === 'installed' || tab === 'submit' || tab === 'profile') return 'marketplace';
  return 'skills';
}
...
export function libraryNavCounts(items: LibraryItems | null | undefined): LibraryNavCounts {
  return {
...
export function buildLibraryNavRows(items: LibraryItems | null | undefined): LibraryNavRow[] {
  const counts = libraryNavCounts(items);
  return [
    { id: 'skills', label: 'Skills', count: counts.skills },
    { id: 'workers', label: 'Workers', count: counts.workers },
...
export function filterSkillCards(cards: SkillCardModel[], query: string): SkillCardModel[] {
...
export function filterWorkerCards(cards: WorkerCardModel[], query: string): WorkerCardModel[] {
  const q = query.trim().toLowerCase();
  if (!q) return cards;
  return cards.filter((c) =>
    [c.name, c.type, c.description, c.status, c.team ?? ''].join(' ').toLowerCase().includes(q),
  );
}
