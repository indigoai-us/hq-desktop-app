...
import type { LibraryItems, LibrarySkill, LibraryWorker } from '../lib/library';
...
function listing(
  overrides: Partial<MarketplaceListing> & Pick<MarketplaceListing, 'id' | 'slug'>,
): MarketplaceListing {
  return {
    type: 'skill',
...
describe('library-overlay-model (US-017)', () => {
  ...
  describe('marketplace badge derivation', () => {
    it('returns get when not installed', () => {
      ...
      expect(
        marketplaceBadgeForListing(listing({ id: '1', slug: 'engineering' }), index),
      ).toBe('get');
    });
    ...
    it('returns update when installed with updateAvailable', () => {
      const installed: InstalledPackRef[] = [
        { name: 'hq-pack-gstack', source: 'marketplace:gstack', updateAvailable: true },
      ];
      const index = indexInstalledPacks(installed);
      expect(
    ...
    it('builds marketplace cards with badges + search filter', () => {
      const listings = [
        listing({ id: '1', slug: 'engineering', name: 'hq-pack-engineering', summary: 'Eng' }),
        listing({ id: '2', slug: 'gstack', name: 'gstack', summary: 'Stack tools' }),
      ];
      const installed: InstalledPackRef[] = [
        { name: 'engineering', updateAvailable: false },
      ...
      expect(cards.find((c) => c.slug === 'engineering')?.badge).toBe('installed');
