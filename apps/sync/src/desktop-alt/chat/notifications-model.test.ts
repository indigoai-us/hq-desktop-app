...
function item(partial: Partial<NotificationItem> & { id: string }): NotificationItem {
  return {
    serverType: 'dm',
...
describe('notifications-model (US-012)', () => {
  describe('mapServerType', () => {
    it('maps catalog types into the UI taxonomy', () => {
      ...
      expect(mapServerType('agent_finished_story')).toBe('agent_finished_story');
      expect(mapServerType('agent_channel_action_needed')).toBe('agent_review_request');
      expect(mapServerType('agent_review_request')).toBe('agent_review_request');
  ...
  describe('actionButtonsFor', () => {
    it('renders Accept/Decline for connection_accept', () => {
      const buttons = actionButtonsFor('connection_accept', 'connection_request');
    ...
    it('renders Approve for agent_owner_approve', () => {
      expect(actionButtonsFor('agent_owner_approve').map((b) => b.label)).toEqual([
        'Approve',
      ]);
    });
      ...
      const parsed = parseNotificationsResponse(
        {
          notifications: [
            ...
            wire({ id: 'b', type: 'file_share', status: 'read', actorName: 'Bob' }),
