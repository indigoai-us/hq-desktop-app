...
import { extractStoryId } from './channel-status-model';
 ...
 * assigned.
 */
export function hasReviewSignal(signal: BoardStorySignal | null | undefined): boolean {
  if (!signal) return false;
  if (signal.prOpen === true) return true;
  if (signal.ciGreen === true || signal.ciGreen === false) return true;
  if (typeof signal.reviewer === 'string' && signal.reviewer.trim().length > 0) {
    return true;
  }
  return false;
}
...
export function buildStatusLine(input: {
  ...
  if (live && isPortfolioLiveStatus(live.status)) {
    const progress =
      typeof live.progressPercent === 'number' && Number.isFinite(live.progressPercent)
...
function sessionAssigneeLabel(session: BoardSessionInput | null): string | null {
...
function isSystemEventMessage(msg: BoardActivityMessageInput): boolean {
  if (msg.systemEvent != null && typeof msg.systemEvent === 'object') return true;
  const kind = (msg.messageKind ?? '').toLowerCase();
  return kind === 'system' || kind === 'system_event' || kind === 'system-event';
}
