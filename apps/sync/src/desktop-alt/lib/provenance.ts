...
const ORIGIN_LABEL_KEYS = ['label', 'name', 'type', 'provider', 'source'] as const;
...
export function hasProvenance(value: WorkProvenance | null | undefined): boolean {
...
export function responsiblePerson(
  ...
  return normalized.owner ?? normalized.assignee ?? normalized.creator ?? 'Unassigned';
}
