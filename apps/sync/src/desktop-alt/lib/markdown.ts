...
export function renderInline(text: string): string {
  ...
  work = work.replace(/(`+)([\s\S]*?)\1/g, (_match, _ticks: string, code: string) =>
  ...
  work = work.replace(/<(https?:\/\/[^ >]+|mailto:[^ >]+)>/gi, (_match, url: string) => {
    const href = safeHref(url);
...
function listMarker(line: string): ListMarker | null {
  ...
  const indent = [...match[1]].reduce((columns, character) => columns + (character === '\t' ? 4 : 1), 0);
  return {
    indent,
...
function renderListItem(contentLines: string[], nested: string[]): { html: string; task: boolean } {
...
function renderListBlock(lines: string[], startIndex: number): { html: string; nextIndex: number } {
  ...
  while (index < lines.length) {
    ...
    while (index < lines.length) {
      ...
      if (columns > baseIndent || !/^\s*(?:#{1,6}\s|>|```|~~~)/.test(lines[index])) {
  ...
  return { html: `<${tag}${taskClass}>${items.join('')}</${tag}>`, nextIndex: index };
}
...
export function renderMarkdown(source: string): string {
  ...
  while (i < lines.length) {
    ...
    if (fence) {
      ...
      const language = fence[2] ? ` class="language-${escapeHtml(fence[2].toLowerCase())}"` : '';
    ...
    const alignments = i + 1 < lines.length ? tableAlignments(lines[i + 1]) : null;
    if (line.includes('|') && alignments) {
      ...
      const headerHtml = Array.from({ length: width }, (_, column) =>
        renderTableCell('th', headers[column] ?? '', normalizedAlignments[column]),
      ).join('');
      i += 2;
      const rows: string[] = [];
      while (i < lines.length && lines[i].trim() !== '' && lines[i].includes('|')) {
        ...
        rows.push(
          `<tr>${Array.from({ length: width }, (_, column) =>
            renderTableCell('td', cells[column] ?? '', normalizedAlignments[column]),
          ).join('')}</tr>`,
        );
        i += 1;
      }
