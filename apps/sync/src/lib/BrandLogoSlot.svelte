<script lang="ts">
  import { onMount } from 'svelte';
  import PoweredByHq from './PoweredByHq.svelte';
  import {
    currentColorScheme,
    selectLogoUrl,
    type CachedBrand,
    type ColorScheme,
    type CompanyBrandSettings,
  } from './brand';

  /**
   * In-app header logo slot (US-005).
   *
   * Branded: tenant logo via `<img>` only (light/dark by appearance, cross-
   * variant fallback, HQ mark on load failure) + permanent "powered by" lockup.
   * Unbranded: default HQ lettermark, no lockup.
   *
   * OS app icon / productName stay "HQ Sync" — this slot is in-app only.
   */
  interface Props {
    /** Live or cached brand; null/undefined → HQ defaults. */
    brand?: CachedBrand | CompanyBrandSettings | null;
    /** Entitlement stamp; false/absent → unbranded even if brand is set. */
    brandingEnabled?: boolean | null;
    /** Cached data-URL logos for offline render. */
    logoDataLight?: string | null;
    logoDataDark?: string | null;
    /** Optional company name for img alt. */
    companyName?: string | null;
    /** Force a scheme (tests); defaults to prefers-color-scheme. */
    scheme?: ColorScheme | null;
    /** Compact popover sizing vs desktop sidebar. */
    size?: 'popover' | 'desktop';
  }

  let {
    brand = null,
    brandingEnabled = null,
    logoDataLight = null,
    logoDataDark = null,
    companyName = null,
    scheme = null,
    size = 'desktop',
  }: Props = $props();

  let activeScheme = $state<ColorScheme>('dark');
  let logoFailed = $state(false);

  // Normalise CachedBrand vs bare CompanyBrandSettings.
  const settings = $derived.by((): CompanyBrandSettings | null => {
    if (!brand) return null;
    if ('brand' in brand && brand.brand) return brand.brand;
    if ('logoUrlLight' in brand || 'logoUrlDark' in brand || 'accentColor' in brand) {
      return brand as CompanyBrandSettings;
    }
    return null;
  });

  const entitled = $derived(
    brandingEnabled === true ||
      (brand != null &&
        typeof brand === 'object' &&
        'brandingEnabled' in brand &&
        (brand as CachedBrand).brandingEnabled === true),
  );

  const cachedData = $derived.by(() => {
    if (brand && typeof brand === 'object' && 'logoDataLight' in brand) {
      return brand as Pick<CachedBrand, 'logoDataLight' | 'logoDataDark'>;
    }
    return { logoDataLight, logoDataDark };
  });

  const logoSrc = $derived(
    entitled ? selectLogoUrl(settings, activeScheme, cachedData) : null,
  );

  const showTenantLogo = $derived(entitled && Boolean(logoSrc) && !logoFailed);
  const showLockup = $derived(entitled && showTenantLogo);

  // Reset failure when the resolved URL changes (scheme flip / new brand).
  $effect(() => {
    void logoSrc;
    logoFailed = false;
  });

  $effect(() => {
    if (scheme) activeScheme = scheme;
  });

  onMount(() => {
    if (scheme) {
      activeScheme = scheme;
      return;
    }
    activeScheme = currentColorScheme();
    const mq = window.matchMedia('(prefers-color-scheme: light)');
    const onChange = () => {
      activeScheme = mq.matches ? 'light' : 'dark';
    };
    mq.addEventListener('change', onChange);
    return () => mq.removeEventListener('change', onChange);
  });

  function onLogoError() {
    console.log(
      '[brand] tenant logo failed to load; falling back to default HQ mark',
      logoSrc,
    );
    logoFailed = true;
  }
</script>

<div
  class="brand-logo-slot"
  class:size-popover={size === 'popover'}
  class:size-desktop={size === 'desktop'}
  class:is-branded={showTenantLogo}
  data-testid="brand-logo-slot"
  data-branded={showTenantLogo ? 'true' : 'false'}
>
  {#if showTenantLogo && logoSrc}
    <img
      class="brand-tenant-logo"
      src={logoSrc}
      alt={companyName?.trim() || 'Company logo'}
      data-testid="brand-tenant-logo"
      data-scheme={activeScheme}
      onerror={onLogoError}
    />
  {:else}
    <svg
      class="brand-hq-mark"
      viewBox="0 0 280 161"
      fill="currentColor"
      role="img"
      aria-label="HQ"
      data-testid="brand-hq-mark"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M85.7251 3.66162H118.034V154.434H85.7251V89.8176H32.3085V154.434H0V3.66162H32.3085V57.5091H85.7251V3.66162Z"
      />
      <path
        d="M257.169 160.035L241.014 144.096C235.343 147.973 229.096 150.988 222.276 153.142C215.527 155.296 208.419 156.373 200.952 156.373C190.757 156.373 181.172 154.363 172.197 150.342C163.223 146.25 155.325 140.65 148.505 133.542C141.684 126.362 136.335 118.07 132.458 108.664C128.581 99.187 126.642 89.0278 126.642 78.1865C126.642 67.417 128.581 57.3296 132.458 47.9242C136.335 38.4471 141.684 30.1187 148.505 22.939C155.325 15.7593 163.223 10.1592 172.197 6.1386C181.172 2.0462 190.757 0 200.952 0C211.219 0 220.84 2.0462 229.814 6.1386C238.789 10.1592 246.686 15.7593 253.507 22.939C260.328 30.1187 265.641 38.4471 269.446 47.9242C273.323 57.3296 275.261 67.417 275.261 78.1865C275.261 86.0123 274.184 93.5151 272.031 100.695C269.948 107.803 267.077 114.444 263.415 120.618L280 137.203L257.169 160.035ZM200.952 124.065C203.896 124.065 206.732 123.741 209.46 123.095C212.26 122.449 214.952 121.552 217.537 120.403L208.491 111.357L231.322 88.5252L239.291 96.4946C240.512 93.6946 241.409 90.7509 241.984 87.6637C242.63 84.5764 242.953 81.4173 242.953 78.1865C242.953 71.8684 241.84 65.9452 239.614 60.4168C237.461 54.8885 234.445 50.0422 230.568 45.878C226.691 41.642 222.204 38.3394 217.106 35.9701C212.08 33.529 206.696 32.3085 200.952 32.3085C195.208 32.3085 189.788 33.529 184.69 35.9701C179.664 38.3394 175.213 41.642 171.336 45.878C167.459 50.0422 164.407 54.8885 162.182 60.4168C160.028 65.9452 158.951 71.8684 158.951 78.1865C158.951 84.5046 160.028 90.4637 162.182 96.0639C164.407 101.592 167.459 106.474 171.336 110.71C175.213 114.875 179.664 118.141 184.69 120.511C189.788 122.88 195.208 124.065 200.952 124.065Z"
      />
    </svg>
  {/if}

  {#if showLockup}
    <PoweredByHq />
  {/if}
</div>

<style>
  .brand-logo-slot {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    gap: 4px;
    min-width: 0;
    color: var(--v4-text-1, var(--popover-text-heading, #f2f2f3));
  }

  .brand-tenant-logo {
    display: block;
    width: auto;
    max-width: 130px;
    object-fit: contain;
    object-position: left center;
  }

  .size-desktop .brand-tenant-logo {
    height: 18px;
  }

  .size-popover .brand-tenant-logo {
    height: 15px;
  }

  .brand-hq-mark {
    display: block;
    width: auto;
    fill: currentColor;
  }

  .size-desktop .brand-hq-mark {
    height: 14px;
  }

  .size-popover .brand-hq-mark {
    height: 15px;
  }

  .size-popover {
    /* Popover header is a single horizontal bar — keep mark compact. */
    justify-content: center;
  }
</style>
