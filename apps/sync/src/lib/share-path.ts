 ...
 * `projects/client-stats-redesign/*` (or `/**` for a recursive share), or as a
 * private-folder ACL path ending in `/`. Naively taking the last path segment
 * (`split('/').pop()`) surfaces the literal `*` as the card title — meaningless
 * to the recipient. `shareTitle` strips the wildcard suffix and shows the
 * directory name with a trailing slash (`client-stats-redesign/`) so a folder
 * share reads as a folder, while plain file shares keep their filename unchanged.
 */
 ...
 * - `projects/foo/`   → `foo/`   (private folder share)
...
export function shareTitle(path: string): string {
  const isWildcardDir = WILDCARD_SUFFIX.test(path) || path.endsWith('/');
