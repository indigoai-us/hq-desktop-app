<script lang="ts">
  ...
  // V2 Cloud Off (US-001): settings-backed pause flag. Hydrated on mount and
  // on every popover open, and re-checked right before a manual sync — the
  // Rust `start_sync` gate is the enforcement; this state only renders the
  // paused notice and short-circuits the click with an honest message.
  ...
  async function handleSyncNow() {
    ...
    // Cloud Off (V2 US-001): sync is paused on this device. Check the
