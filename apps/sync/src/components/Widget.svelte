<script lang="ts">
  /**
   * Floating desktop widget — HQ wordmark only (US-002).
   *
   * Locked design: no circle, no badge chip, no rounded container. Idle
   * translucency + full opacity on hover. Color tracks system appearance via
   * prefers-color-scheme (no screen-recording permission). `queued` is fed by
   * US-003; default 0 so the superscript stays hidden for now.
   */
  let { queued = 0 }: { queued?: number } = $props();
</script>
