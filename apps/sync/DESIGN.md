...
- **Sidebar multi-select checkbox** (`ChatSidebar.svelte`). Multi-selection in the conversation rail is marked by a 15px checkbox at the left of the row: `--radius-sm`-scale 4px corners, 1px `--line` border when empty, `--accent` fill with a white check glyph when ticked, and a 2px `--accent` focus ring. The checkbox lives in a gutter that is 0px wide at rest and animates to 22px only while a selection exists or the user holds Shift over the rail, so resting rows keep their geometry and nothing jumps on hover.

  **Banned: the curved left-edge selection stroke.** Selected rows must never be marked by an inset accent `box-shadow` or `border-left` on the row — against the row's 8px radius it renders as a curved purple bracket hugging the left edge. It reads as decoration rather than state, and it collides with the row's own active/hover highlight. Selection state belongs to the checkbox; the row's background highlight stays reserved for active and hover.
