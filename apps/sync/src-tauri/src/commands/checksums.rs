...
//!
//! Ported from the legacy `hq-installer` command. The only cross-platform
//! change is the atomic `core.yaml` replacement: Unix/macOS uses a temp
//! write + fsync + `std::fs::rename`; Windows keeps the original
//! `MoveFileExW(REPLACE_EXISTING | WRITE_THROUGH)` path behind
//! `cfg(windows)`.
//!
...
/// Tauri command: recompute `core/core.yaml` checksums for the HQ install at
/// `install_path`. The hashing/IO is CPU + disk bound over potentially
/// thousands of files, so it runs on the blocking pool rather than wedging the
/// async executor. Authorized by `core:default` — no new capability token.
#[tauri::command]
pub async fn compute_checksums(install_path: String) -> Result<ChecksumResult, String> {
    tokio::task::spawn_blocking(move || compute_checksums_at(Path::new(&install_path)))
        .await
        .map_err(|e| format!("compute_checksums task failed: {e}"))?
}

/// Pure body for `compute_checksums` — takes an explicit install root so the
/// hashing + YAML splice are unit-testable and callable in-process (the
/// onboarding content stage invokes this directly after extracting the
/// template, so `core.yaml` carries fresh checksums before git-init/sync).
pub fn compute_checksums_at(install_root: &Path) -> Result<ChecksumResult, String> {
...
///
/// Unix/macOS: `std::fs::rename` is atomic when both paths live on the same
/// filesystem (the temp file is created in the target's own directory, so it
/// always does). Windows: the temp file is created up-front, so a plain
/// `rename` would fail with ERROR_ALREADY_EXISTS — use
/// `MoveFileExW(REPLACE_EXISTING | WRITE_THROUGH)` for an atomic,
...
/// Best-effort directory-entry durability flush after the atomic rename. A
/// failure here must never fail the checksum write (the rename itself already
/// landed the new bytes) — log and continue.
#[cfg(not(windows))]
...
fn sync_parent_dir(parent: &Path) {
    ...
    // directory returns ERROR_ACCESS_DENIED even when the handle is opened with
    // FILE_FLAG_BACKUP_SEMANTICS — and the MoveFileExW rename was already
    // WRITE_THROUGH, so a failure here is non-fatal. Log and continue.
...
fn sha256_dir(dir: &Path) -> std::io::Result<String> {
    ...
    // regular file. Mirror `find -type f` exactly: include files (not
    // symlinks), exclude dirs/sockets/etc. Following symlinks would change
    // the hash and could double-count template parent-links.
...
"#;
    ...
    #[test]
    fn compute_checksums_at_errors_when_core_yaml_missing() {
        let tmp = TempDir::new().unwrap();
        let err = compute_checksums_at(tmp.path()).unwrap_err();
        assert!(err.contains("core.yaml not found"), "got: {err}");
    }
}
