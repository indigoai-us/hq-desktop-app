...
fn microphone_status() -> PermStatus {
    match permissions::authorization_status_for_audio() {
...
pub fn meetings_permissions_state() -> Result<MeetingPermissionsState, String> {
    #[cfg(target_os = "macos")]
    {
        let ax_trusted = permissions::accessibility_is_trusted();
        let sc_authorized = permissions::screen_capture_preflight();
