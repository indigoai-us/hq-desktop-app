...
async fn program(tool: SessionTool) -> Result<String, String> {
    tokio::time::timeout(
        ...
        tauri::async_runtime::spawn_blocking(move || match tool {
            SessionTool::Claude => programs::claude_program(),
            SessionTool::Codex => programs::codex_program(),
            SessionTool::Grok => programs::grok_program(),
        }),
    )
    .await
...
async fn probe(tool: SessionTool, program: &str) -> Result<bool, ()> {
    ...
    let result = tokio::time::timeout(PROBE_TIMEOUT, async {
        ...
        match tool {
            ...
            SessionTool::Grok => status::grok_login_status_succeeded(success, &out, &err),
        }
    })
    .await;
    match result {
