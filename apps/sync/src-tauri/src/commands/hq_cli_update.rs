...
async fn install_hq_cli_update_once(app: AppHandle) -> Result<HqCliUpdateInfo, String> {
    ...
    if !install_run.output.status.success() {
        ...
        // describes the user's own toolchain, never HQ's managed retry. It also
        // feeds the environment-aware classification just below.
        let install_env =
        ...
        // Classify WITH the probed environment. An unsupported user Node makes npm
        // itself unrunnable, so npm emits no `npm error` block and the env-blind
        // classifier falls through to `Unexpected` (the empty `none:unknown:none`
        // group). The environment-aware layer refines exactly that case to
        // `UnsupportedNode`, which arms the SAME one-shot managed-Node self-heal
        // below and, when the heal cannot run, pages once per CLI target version
        // instead of on every scheduled check.
        ...
        // HQ-DESKTOP-4V / HQ-DESKTOP-4W (and HQ-DESKTOP-56) self-heal. Two shapes
        // HQ can repair itself by installing its managed Node and retrying: a
        // THIRD-PARTY native-build lifecycle failure (better-sqlite3 /
        // node-llama-cpp) under the user's OWN Node — the reported Node 20 arm64
        // Macs have no prebuilt binary for their ABI and no Xcode CLT to build from
        // source — and an UnsupportedNode failure where the user's PATH Node is too
        // old to run npm at all. In both cases HQ already ships a checksum-verified
        // managed Node 22 whose ABI has prebuilds and whose major satisfies the
        // CLI's engines. Only user-path, an ABI that differs from HQ's managed one,
        // and (for the lifecycle shape) a cause a different runtime can actually fix
        // (never disk-space/network) arm the bounded, one-shot managed-toolchain
        // retry; every other kind/cause keeps today's behaviour. Gating on UserPath
        // + a differing ABI also means a run already on the managed toolchain never
        // retries into itself. HQ blames the user's toolchain (the copy below) only
        // AFTER its own repair was attempted and could not converge.
...
/// Node and retrying. Two shapes qualify, both under the user's OWN Node and both
/// repairable by a runtime whose ABI has prebuilds and whose major satisfies the
/// CLI's `engines.node`. Pure so the gate is unit-testable without an `AppHandle`
/// or a real install.
///
/// The eligible failure shapes:
///   * a third-party native-build lifecycle failure (`UnexpectedLifecycle`) whose
...
///   * an unsupported user Node (`UnsupportedNode`): the PATH Node is older than
///     the CLI's `engines.node` (`>=20`), so `npm i -g` can never succeed and npm
///     never ran far enough to have a diagnosed cause. Provisioning HQ's managed
///     Node 22 makes the very same pinned install succeed, so the cause gate does
///     not apply to it.
///
...
///     reported clusters (Node 20 ABI 115, Node 6 ABI 48) still arm.
fn install_failure_earns_managed_retry(
    ...
    (repairable_lifecycle || unsupported_node)
...
mod tests {
    ...
    #[test]
    fn managed_toolchain_retry_also_arms_for_an_unsupported_user_node() {
        // HQ-DESKTOP-56: the user's PATH Node is too old to run npm at all, so the
        // classifier reports `UnsupportedNode`. Under the user's OWN toolchain with
        // an ABI that differs from HQ's managed Node, provisioning managed Node 22
        // is the exact repair, so this shape ALSO arms the one-shot retry. The
        // reported machine ran Node 6 (ABI 48); an unknown ABI must arm too.
        for abi in [Some(48), None] {
            assert!(
                install_failure_earns_managed_retry(
                ...
                "unsupported user Node (abi {abi:?}) must arm the managed retry"
            );
        }

        // The unsupported-node arm is independent of the diagnosed cause (unlike
        // the lifecycle arm) — npm never ran far enough to have one — so even a
        // would-be-excluded cause still arms.
        for cause in ["disk-space", "network", "toolchain-missing"] {
            assert!(
                ...
                "cause {cause:?} must not gate out the unsupported-node retry"
            );
        }

        // A run already ON the managed toolchain, or already on the managed ABI,
        // gains nothing from provisioning Node 22 again — it must NOT retry.
        for source in [NpmToolchainSource::Managed, NpmToolchainSource::Unknown] {
            assert!(!install_failure_earns_managed_retry(
                InstallFailureKind::UnsupportedNode,
                source,
