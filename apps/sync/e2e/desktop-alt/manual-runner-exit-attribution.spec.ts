...
describe('manual runner-exit attribution — shared classifier source', () => {
  ...
  it('re-pins the built-in JS + errno cause families, the errno classes, and the compile-time drift guard (r2)', () => {
    ...
    // personal-vault exclusion, hq-cloud#501) and ~6.16.26 (the area-collision
    // heal, hq-cloud#502) were each re-derived against both tags and add no
    // identity, so the vocabulary is unchanged and only the source-version
    // marker moves with the pin.
    expect(shapeSource).toContain('CAUSE_VOCABULARY_SOURCE_VERSION: &str = "~6.16.26"');
