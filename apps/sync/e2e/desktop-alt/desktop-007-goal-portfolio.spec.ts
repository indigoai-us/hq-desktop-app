...
describe('DESKTOP-007: goal portfolio and drill-down', () => {
  ...
  it('shows linked projects and opens ProjectDetailView in place', () => {
    ...
    expect(page).toContain('ids.add(projectIdentity(project))');
  ...
  it('uses naked hairline list/detail with square open selection rows', () => {
    ...
    expect(page).toMatch(
      /\.goal-list-row\s*\{[\s\S]*?border:\s*0;[\s\S]*?border-radius:\s*0;[\s\S]*?background:\s*transparent;/,
    );
    // True controls and progress indicators may retain a compact radius.
