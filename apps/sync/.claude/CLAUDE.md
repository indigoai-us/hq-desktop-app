...
| `commands/desktop_alt.rs` | GA desktop window gate, open/focus command, and read-only company panel commands. Uses `feature_gate::desktop_features_enabled()` for UI eligibility and backend enforcement. Board/Activity call the vault API and Deployments calls hq-deploy with `x-org-slug` (backend data still feeds Overview digests, but the standalone Deployments / Secrets / Activity screens moved to the HQ web console in V2 and `get_company_secrets` was deleted — the app never handles secret values, not even metadata). The window is built `transparent(true)` and gets its native glass backing applied via `glass::apply_liquid_glass_window` right after build (0.8.1-beta.1). `get_company_project_creators` (0.8.2-beta.1) fetches the cloud board for a company and returns only projects that carry a non-empty creator (derived from the prd's S3 `created-by` metadata) for the Projects-list Lead column — projects with no stamped creator stay "Unassigned" (pure parse in `parse_project_creators`, unit-tested) |
...
Frontend files live under `src/desktop-alt/`. `DesktopApp.svelte` owns the route, V4 chrome, command-K palette, titlebar updater, sync event listeners, workspace loading, and meetings cache hydration. `route.ts` maps the sidebar rows and the V2 workspace-switcher hotkeys — `⌘0` opens the Personal workspace and `⌘1`–`⌘9` switch to companies in connected-first order (primary destinations like Inbox / Meetings / Marketplace / Library no longer own number chords). The company work-system views read local goals/projects where possible (goal–project linking is native); Deployments, Secrets, the Activity feed, and fleet Mission Control are HQ web console surfaces — legacy deep links remap to the nearest V2 screen or open the console via `resolvePendingDesktopRoute`.

Important constraints:

- Any new desktop-alt invoke path must be allowed by `src-tauri/capabilities/desktop-alt.json`.
- Secrets are not a desktop surface (V2 / US-021): `get_company_secrets` is deleted and the console owns secrets. Do not reintroduce secret-carrying commands or fields; `e2e/desktop-alt/secrets-never-leak.spec.ts` exists to lock this down.
