...
The `HQ_CLOUD_VERSION` pin in `crates/hq-desktop-core/src/hq_cloud.rs` is `~6.16.51`. `MANIFEST_UPLOAD_MIN_HQ_CLOUD` (6.16.23), `UNROUTED_OVERFLOW_MIN_HQ_CLOUD` (6.16.24), `ROOT_BIN_EXCLUSION_MIN_HQ_CLOUD` (6.16.25), `AREA_COLLISION_HEAL_MIN_HQ_CLOUD` (6.16.26), and `RESCUE_CONTRACT_FLOOR` (6.16.0) are desktop-visible behavior floors and remain unchanged. The 6.16.51 runner fixes recovery of a failed HQ Core update: it can restore copied Windows file links and junctions, restore the entire recorded snapshot, reject unsafe snapshot metadata, preserve copied files when metadata updates are unavailable, and stream macOS cloud placeholders. Those are runner bug fixes, not a new desktop-visible capability, so this pin bump deliberately adds no behavior floor. Semver admission is not delivery: changing the requested spec is what moves npm's npx cache key, while the exact-pin test keeps that move deliberate.
