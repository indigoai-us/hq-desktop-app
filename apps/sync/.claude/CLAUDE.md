...
| `commands/hq_cli_update.rs` | "Update available" check + direct installer for the separate `@indigoai-us/hq-cli` npm package (thin Tauri wrapper over `crates/hq-desktop-core/src/hq_cli_update.rs`, which holds the full flow docs). Background check 15s after launch (offset from the app updater's 10s), then every 6h; `cliAutoUpdate` (default on) installs directly. **Package-manager-aware (v0.10.68/v0.10.69):** anchors version reads, install, and execution to the *resolved* `hq` binary's prefix (not `npm root -g`), and detects pnpm-managed installs — both pnpm ≤10's flat global dir (`~/Library/pnpm/hq`) and pnpm ≥11's nested `<pnpm-home>/bin/hq` layout, disambiguated from an npm prefix by the grandparent being the pnpm home — updating those via `pnpm add -g` instead of clobbering with npm. |
