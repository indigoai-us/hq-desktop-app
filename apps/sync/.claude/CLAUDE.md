...
**Frontend:** Svelte 5 with runes (`$state`, `$effect`). No component library — vanilla CSS in `src/styles/popover.css` owns the canonical Liquid Glass tokens for the classic popover, and `src/desktop-alt/styles/desktop-alt.css` imports those tokens for both the V4 desktop window and standalone Messages window. The V4 surface is light-mode adaptive (0.8.2-beta.1): `src/desktop-alt/v4/tokens.css` carries a light token set under `prefers-color-scheme: light` and `desktop-alt.css` carries the matching light glass/surface overrides, so the desktop window follows the OS appearance instead of being dark-only. External links the desktop window opens into the HQ web console are centralized in `src/desktop-alt/lib/hq-console.ts` (single source of truth — Company Settings, invite, integrations, creator profiles), opened in the system browser via `@tauri-apps/plugin-shell`. Views: `SignInPrompt` (OAuth), `Popover` (main sync UI with per-workspace rows, real per-file progress bar, **Stop** button mid-sync, Connect diagnostics drawer, and a footer HQ-version row whose action pills — drift count + Restore vX.Y.Z + update-done chip — wrap to a second line below the version label so they never overlap it, fixed v0.2.2), `Settings` (preferences + folder re-tether), and the desktop-alt shell under `src/desktop-alt/` (Home, Mission Control, Inbox, Meetings, Marketplace, Library, Files, company workspaces, Settings, safety flows, V4 sidebars, titlebar updater, command palette). Conflict resolution via `ConflictModal` + `ConflictRow` components. There are no first-launch onboarding pages — on a brand-new install `App.svelte` just pops the popover open and starts the first sync (the old `FirstRunWelcome` carousel + `AutoSyncNotice` card were removed). New-file notification via `NewFilesBadge` (in-popover count) + `NewFilesDetail` (secondary window with file list and attribution). Recent Changes via `ActivityLog` (secondary window listing the session's per-file syncs, each attributed as "{author} added/updated" — author email + verb — falling back to the company slug when no author). DM notifications open `DmDetail` (secondary window showing the full two-way **conversation thread** — received vs. sent bubbles, scrollable, with the live message at the bottom — plus a reply composer: textarea + **Send** / ⌘↵; sent replies append optimistically); share notifications open `ShareDetail`. **Notifications-first popover (0.9.8):** the popover carries a `Notifications | Workspaces` segmented control — the redesigned compact `NotificationFeed` (unread badge, Mark all read, data assembled by `src/lib/notificationFeedData.ts`) is the first tab, workspaces the second. The desktop-alt **Inbox** combines the full-size notification feed with messaging entry points; the standalone Messages window remains independent. **Share reactions (0.9.8):** shares support emoji reactions via the reaction scope `share:{eventId}` reusing the DM `ReactionBar` (`src/lib/shareReactionController.svelte.ts`); `ShareDetail` and the feeds gain reactions plus a **Message the sharer** action that deep-links into the Messages window (`src/lib/pendingConversation.ts` + `messages:open-conversation`). **Share history in Messages (0.9.8):** `src/lib/shareTimeline.ts` merges the peer's share events into DM threads as inline share-card bubbles and drives the rail's "Shared a file" previews (agent conversations get no reactions surface).
...
| `commands/work_daemon/` | **Work Mesh local sessions** — the desktop half of Slack "Send to HQ Agent → Local session". **FEATURE-GATED OFF by default** (`workMeshEnabled` in menubar.json, absent = off; `WORK_MESH_ENABLED=1` overrides for dev; re-read per cycle so a flip needs no restart). With the gate off nothing runs — no creds fetch, no socket, no feed request. Armed, it subscribes to `hq/{personUid}/work` over MQTT-over-WSS (same creds/presign/backoff structure as `dm_mqtt.rs`, reusing `sessions::outpost`'s `build_signed_wss_url` + `client_id`), and on connect + on each wake fetches the **person-scoped** feed `GET /v1/work-mesh/work` — NOT `?companyUid=`; a `prs_*` person uid is never a valid companyUid and that form 403s. Each open thread carrying an additive `localSession` block opens a local runtime, deduped by `threadId` for the process lifetime (claim-before-launch, release-on-failure so a later wake retries). `providers.rs` splits the routing into a pure `plan_launch` (fully unit-tested) and an IO `execute`: **claude** → the existing `claude://code/new?q=…&folder=…` path through `validate_claude_deep_link` + `claude_launch::preflight_claude_code_url`; **codex** → the NEW `codex://threads/new?path=…&prompt=…` link into `/Applications/ChatGPT.app` (Codex ships inside the ChatGPT app and registers the `codex` scheme; both param names are empirically established — evidence recorded on `work_mesh::build_codex_thread_url`); **grok** → the existing `launch_cli_in_terminal(path, "grok")` and never a deep link (Grok Build Desktop declares no `CFBundleURLTypes`). An unavailable runtime POSTs a `blocked` event naming the provider + reason and never crashes the listener. Windows has no agent URL schemes, so every provider degrades to the terminal launcher. The pure contract (wire types, topic derivation, both URL builders, the fenced prefill) lives in `hq-desktop-core` `work_mesh.rs`. Log codes `WORK_MESH_*` |
...
- Otherwise it rides the data `list_syncable_workspaces` already returns. The proactive "notify even while the popover is closed" piece (native notification + idle poll) is a separate, not-yet-shipped story.

## First-Run Onboarding

There are no onboarding pages. The welcome carousel (`FirstRunWelcome`) and the one-time auto-sync notice (`AutoSyncNotice`) were removed — on a brand-new install the app simply pops the popover open and starts syncing.

The launch kind is still classified once at `.setup()` (`commands/first_run.rs`) and drives `App.svelte`'s `runOnboarding`:

- **FirstRun** (brand-new install) — the popover force-opens (`show_main_window`), the first cloud sync auto-starts (`handleSyncNow`), and `mark_first_run_complete` is persisted immediately (writes `firstRunCompleted` + `autoSyncNoticeShown`, and makes "sync is on" explicit via `realtimeSync` + `personalSyncEnabled` true). Never repeats — every later launch classifies as `Normal`, so the popover only force-opens once.
...
Left-click toggles popover window. Right-click shows context menu (Sync Now / Open desktop view / Sign Out / Quit HQ — aligned to the 0.9.8 notifications-first redesign). Tray state auto-updates from sync event listeners.
