...
The `HQ_CLOUD_VERSION` pin in `crates/hq-desktop-core/src/hq_cloud.rs` is `~6.16.52`. `MANIFEST_UPLOAD_MIN_HQ_CLOUD` (6.16.23), `UNROUTED_OVERFLOW_MIN_HQ_CLOUD` (6.16.24), `ROOT_BIN_EXCLUSION_MIN_HQ_CLOUD` (6.16.25), `AREA_COLLISION_HEAL_MIN_HQ_CLOUD` (6.16.26), and `RESCUE_CONTRACT_FLOOR` (6.16.0) are desktop-visible behavior floors and remain unchanged. The 6.16.52 runner can skip a file that cannot be copied into a safety snapshot, protect it from the update, emit `HQ_RESCUE_SKIPPED_KIND`, and keep incomplete snapshots within the configured retention limit. Semver admission is not delivery: changing the requested spec is what moves npm's npx cache key, while the exact-pin test keeps that move deliberate.
