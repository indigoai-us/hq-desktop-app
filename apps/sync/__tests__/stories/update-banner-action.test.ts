...
describe('update banner "Update now" chip opens the popover and starts a guarded install', () => {
  ...
  it('routes kind=update/action=update to show_main_window THEN handleInstallUpdate (not a bare install_update)', () => {
    ...
    expect(a).toContain("await invoke('show_main_window'); await handleInstallUpdate(true);");
  });

  it('keeps the body-click (action=open) revealing the popover without forcing an install', () => {
    const updateBranch = a.slice(
      a.indexOf("else if (kind === 'update')"),
      a.indexOf("else if (kind === 'meeting')"),
    );
    expect(updateBranch).toContain(
      "if (action === 'open') { await invoke('show_main_window'); return; }",
    );
    expect(updateBranch).not.toContain("if (action === 'open') { await handleInstallUpdate");
  });
  ...
  it('handleInstallUpdate is the guarded path that flips the banner to Installing…', () => {
    ...
    expect(a).toContain('async function handleInstallUpdate(throwOnError = false) { if (updateInstalling) return; updateInstalling = true;');
    expect(a).toContain('if (throwOnError) throw err;');
  });
});
