...
describe('US-006: Alt Meetings page wires to existing detection + memberships', () => {
  ...
  it('renders every upcoming meeting grouped by day with Up next, cached signal totals, and recent signals', () => {
    ...
    expect(page).toContain(
      '<MeetingsAgenda groups={dayGroups} {upNext} totalCount={upcomingEvents.length} companyNames={companyNamesByUid} {liveEventId} {botsByEventId} {scheduledBots} {pendingEventIds} {focusedMeetingId} {onInvite} {onUninvite} {onJoinNow} onOpenExternal={openExternal} />',
    );
