...
/**
 * PORT NOTE (packages/ui purity): every former Tauri command call in this
 * module is now a method on `MarketplaceApi` / `ShellApi` from `@hq/platform`,
 * injected by the caller. Helpers return `AdapterResult<T>` — `unavailable`
 * renders the standard degraded state, `error` carries code/message for the
...
function asListing(value: unknown): MarketplaceListing | null {
  ...
  return value as unknown as MarketplaceListing;
}

/**
 * hq-pro browse is `{ listings: PublicListing[] }` (v1 Rust BrowseEnvelope).
 * Also accept a bare array so older mocks / adapters keep working.
 */
export function listingsFromBrowsePayload(raw: unknown): MarketplaceListing[] {
  if (raw == null) return [];
...
/** hq-pro detail is `{ listing: PublicListing & { downloadUrl } }`. */
export function listingFromDetailPayload(
  raw: unknown,
): MarketplaceListingDetail | null {
  if (!isRecord(raw)) return null;
  const inner = isRecord(raw.listing) ? raw.listing : raw;
  const listing = asListing(inner);
  if (!listing) return null;
  const downloadUrl =
    typeof inner.downloadUrl === "string"
      ? inner.downloadUrl
      : typeof inner.download_url === "string"
        ? inner.download_url
        : "";
...
export async function loadMarketplaceListings(
  api: MarketplaceApi,
  query?: string,
): Promise<AdapterResult<MarketplaceListing[]>> {
  const trimmed = query?.trim() ?? "";
  const res = await api.listListings(trimmed ? { q: trimmed } : undefined);
  if (!res.ok) return res;
  const listings = listingsFromBrowsePayload(res.value);
  return {
    ok: true,
    value: trimmed ? filterListings(listings, trimmed) : listings,
  };
}
...
export async function loadMarketplaceListing(
  api: MarketplaceApi,
  id: string,
): Promise<AdapterResult<MarketplaceListingDetail>> {
  const res = await api.getListing(id);
  if (!res.ok) return res;
  const listing = listingFromDetailPayload(res.value);
  if (!listing) {
    return {
      ok: false,
      reason: "error",
      message: "Listing response was empty.",
    };
  }
...
export type InstallScope =
  { kind: "personal" } | { kind: "company"; slug: string };
 ...
 */
export function companyInstallTargets(
  ...
  const companies = workspaces
    ...
    .map((w): InstallTarget => {
      ...
      if (admin && active) {
        return {
          scope: { kind: "company", slug: w.slug },
          label,
          enabled: true,
        };
      }
...
export async function yankMarketplaceListing(
  ...
  return {
    ...
    value: {
      id,
      status: "yanked",
      note: "Already-installed users are not auto-removed.",
    },
  };
}
...
export async function loadModerationQueue(
  ...
  const res = await api.listModerationQueue();
  if (!res.ok) return res;
  const queue: unknown = res.value;
  return {
    ok: true,
    value: Array.isArray(queue) ? (queue as ModerationQueueItem[]) : [],
  };
}
...
export async function decideModerationListing(
  ...
  note?: string | null,
  versionLock?: string | null,
): Promise<AdapterResult<ModerationDecisionResult>> {
  // ADAPTER GAP: the adapter surface carries only (id, decision); `note` and
  // `versionLock` are trimmed here for parity but cannot be forwarded yet.
  void note;
  void versionLock;
  const res = await api.decideModerationListing(id, decision);
  if (!res.ok) return res;
  return { ok: true, value: res.value as unknown as ModerationDecisionResult };
}
 ...
 *
 * BEST-EFFORT / fire-and-forget: callers run this AFTER a successful install
...
export async function recordMarketplaceInstall(
  ...
  return api.recordInstall(listingId, {
    scope: scope.kind,
    ...(scope.kind === "company" ? { companySlug: scope.slug } : {}),
  });
}
...
/**
 * Type-guard: was an `publishPack` failure a structured
 ...
 */
export async function publishMarketplacePack(
  api: MarketplaceApi,
  path: string,
): Promise<AdapterResult<PublishResult>> {
  const res = await api.publishPack(path);
  if (!res.ok) return res;
  return { ok: true, value: res.value as unknown as PublishResult };
}
 ...
 */
export async function pickPackDirectory(
  shell: ShellApi,
): Promise<AdapterResult<string | null>> {
  return shell.pickFolder();
}
 ...
 * admin gate (`isAdminGate`) is UX only — this is not the authorization boundary.
 */
export async function loadCreatorApplications(
  api: MarketplaceApi,
): Promise<AdapterResult<CreatorApplication[]>> {
  const res = await api.listCreatorApplications();
  if (!res.ok) return res;
  const applications: unknown = res.value;
  return {
    ok: true,
    value: Array.isArray(applications)
      ? (applications as CreatorApplication[])
      : [],
  };
}
...
export async function decideCreatorApplication(
  ...
  note?: string | null,
): Promise<AdapterResult<ApplicationDecisionResult>> {
  // ADAPTER GAP: audit `note` is not forwardable on this surface yet.
  void note;
  const res = await api.decideCreatorApplication(id, decision);
  if (!res.ok) return res;
  return { ok: true, value: res.value as unknown as ApplicationDecisionResult };
}
...
/**
 * Type-guard: was an `claimHandle` failure a structured
...
export function checkHandleFormat(
  ...
  if (/^[-_]|[-_]$/.test(handle)) {
    return {
      ok: false,
      reason: "Cannot start or end with a hyphen or underscore.",
    };
  }
  if (/[-_]{2,}/.test(handle)) {
 ...
 */
export async function claimCreatorHandle(
  api: MarketplaceApi,
  handle: string,
): Promise<AdapterResult<ClaimResult>> {
  const res = await api.claimHandle(handle.trim());
  if (!res.ok) return res;
  return { ok: true, value: res.value as unknown as ClaimResult };
}
...
): Promise<AdapterResult<CreatorProfile>> {
  const res = await api.updateCreatorProfile({
    bio: patch.bio ?? null,
...
export async function uploadCreatorAvatar(
  ...
  const res = await api.uploadCreatorAvatar(filePath);
  if (!res.ok) return res;
  return { ok: true, value: String(res.value ?? "") };
}
...
export async function getCreatorProfile(
  api: MarketplaceApi,
  handle: string,
): Promise<AdapterResult<PublicCreatorPreview>> {
  const res = await api.getCreatorProfile(handle.trim());
  if (!res.ok) return res;
  return { ok: true, value: res.value as unknown as PublicCreatorPreview };
}
 ...
 */
export async function loadMyCreator(
  api: MarketplaceApi,
): Promise<AdapterResult<MyCreator | null>> {
  const res = await api.getMyCreator();
  if (!res.ok) return res;
  const me = res.value as unknown as MyCreator | null;
  ...
  if (me === null || me === undefined) return { ok: true, value: null };
  if ((me as { code?: unknown }).code === "NO_CREATOR") {
