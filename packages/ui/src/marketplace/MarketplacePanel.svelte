<script lang="ts">
  ...
  /** Host seam for the streamed install progress events the desktop emits
   *  (`marketplace:install-progress` / `-complete` / `-error`). Optional —
   *  hosts without streaming simply rely on the promise's terminal result. */
  export interface MarketplaceInstallEvents {
    subscribe(handlers: {
  ...
  interface Props {
    /** Platform seam: `marketplace.*` (cloud), `sync.listSyncableWorkspaces`
     *  (desktop) for company install targets, capability gates for install. */
    adapter: PlatformAdapter;
    /** Optional desktop install-progress stream. */
    installEvents?: MarketplaceInstallEvents | null;
  }

  let { adapter, installEvents = null }: Props = $props();

  /** Desktop only — install writes into the local HQ folder. */
  let canInstall = $state(adapter.isAvailable("canInstallLocally"));
  ...
  let installTargets = $state<InstallTarget[]>([
    { scope: { kind: "personal" }, label: "Personal", enabled: true },
  ]);
  ...
  const chosenTarget = $derived(
    installTargets[scopeIndex] ?? installTargets[0],
  );
  ...
  onMount(() => {
    ...
    void (async () => {
      const res = adapter.isAvailable("canSync")
        ? await adapter.sync.listSyncableWorkspaces()
        : null;
      if (res && res.ok) {
        const result = res.value as unknown as WorkspacesResult;
        ...
        if (res && res.reason !== "unavailable") {
          console.error(
            "listSyncableWorkspaces (marketplace install targets) failed:",
            res.message,
          );
        }
      ...
      // Install writes into the local HQ folder — never unlock it on web
      // just because a journal observer once reported a core version.
    })();

    // Stream install progress + terminal result when the host provides the
    // event seam. Hook-consent prompts the CLI prints flow through as lines.
    if (installEvents) {
      void installEvents
        .subscribe({
          onProgress: (line) => {
            if (line) installLog = [...installLog, line];
          },
          onComplete: () => {
        ...
        .then((fn) => {
          unlistenProgress = safeUnlisten(fn);
        });
    }
  ...
  async function runInstall(): Promise<void> {
    ...
    installResult = null;
    installLog = [];
    const installRes = await installMarketplacePack(
      adapter.marketplace,
      selected.slug,
      selected.version,
      target.scope,
  ...
  $effect(() => {
    ...
    void (async () => {
      const result = await loadMarketplaceListings(adapter.marketplace, q);
      if (cancelled) return;
      if (result.ok) {
        listings = result.value;
      } else {
        if (result.reason !== "unavailable") {
          console.error("loadMarketplaceListings failed:", result.message);
        }
...
<div class="marketplace" data-testid="marketplace-panel">
  ...
  <section class="your-listings" data-testid="marketplace-your-listings">
    <div>
      ...
      <p>
        Approved packs from marketplace creators, with version and author
        attribution.
      </p>
    </div>
  ...
  {#if error}
    <div class="state-error" role="alert" data-testid="marketplace-error">
      {error}
    </div>
  {:else if loading}
  ...
  {:else}
    <div class="grid" aria-label="Marketplace listings">
      {#each visible as listing (listing.id)}
        ...
        >
          <div
            class={`cover tone-${coverTone(listing)}`}
            data-testid="marketplace-cover"
          >
            {#if cover}
              <img
                class="cover-img"
            ...
            <span
              class="pill version cover-version"
              data-testid="marketplace-version">v{listing.version}</span
            >
            <h3 class="card-name" title={listingDisplayName(listing)}>
...
{#if selected}
  ...
  >
    ...
    <div class="detail-body">
      ...
      <section
        class="detail-section readme-preview"
        data-testid="marketplace-readme-preview"
      >
        <h3 class="section-title">README preview</h3>
        <p class="section-body">
          {selected.summary ??
            selected.contributes ??
            "No README preview is available for this listing yet."}
        </p>
      </section>

      <!-- Install writes into the local HQ folder — hide it on web. -->
      {#if canInstall}
        <section
        ...
        >
          ...
          {#if chosenTarget?.scope.kind === "company"}
            <p class="consent-note" data-testid="marketplace-consent-note">
              This pack will sync to everyone on this team. Any hooks or scripts
              it contains stay scoped to this company and ask each teammate for
              consent on their machine before running — nothing is wired
              silently.
            </p>
          {/if}
          ...
          >
            {#if installing}
              <span
                ...
                data-testid="marketplace-install-spinner"
                aria-hidden="true"
              ></span>
              Installing…
          ...
          {#if installLog.length > 0}
            <pre
              class="install-log"
              data-testid="marketplace-install-log">{installLog.join(
                "\n",
              )}</pre>
          {/if}
        </section>
...
<style>
  ...
  .cover-monogram {
    ...
    text-shadow: 0 2px 10px
      color-mix(in srgb, var(--graphic-fg) 45%, transparent);
  }
  ...
  .card-name {
    ...
    text-shadow: 0 1px 8px
      color-mix(in srgb, var(--graphic-fg) 65%, transparent);
    display: -webkit-box;
    -webkit-line-clamp: 2;
    line-clamp: 2;
