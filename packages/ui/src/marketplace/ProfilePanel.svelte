<script lang="ts">
  ...
  interface Props {
    /** Platform seam: `marketplace.*` creator routes + `shell.pickFile`
     *  (desktop-only avatar picker — hidden when unavailable). */
    adapter: PlatformAdapter;
  }

  let { adapter }: Props = $props();

  /** The native avatar picker is desktop-only; hide the control elsewhere. */
  const canPickAvatar = $derived(adapter.isAvailable("canLaunchApps"));
  ...
  const hasInvalidSocialUrl = $derived(
    socialLinks.some(
      (l) => l.url.trim().length > 0 && socialUrlHint(l.url) !== null,
    ),
  );
  ...
  const avatarPreviewName = $derived(
    pendingAvatarPath
      ? (pendingAvatarPath.split("/").filter(Boolean).pop() ??
          pendingAvatarPath)
      : null,
  );
  ...
  async function claim(): Promise<void> {
    ...
    claimError = null;
    handleTaken = false;
    const res = await claimCreatorHandle(adapter.marketplace, handleInput);
    if (res.ok) {
  ...
  async function chooseAvatar(): Promise<void> {
    ...
    choosingAvatar = true;
    saveError = null;
    const picked = await pickAvatarFile(adapter.shell);
    if (picked.ok) {
      if (picked.value) {
        pendingAvatarPath = picked.value;
        saved = false;
      }
  ...
  async function save(): Promise<void> {
    ...
    try {
      ...
      if (pendingAvatarPath) {
        const uploaded = await uploadCreatorAvatar(
          adapter.marketplace,
          pendingAvatarPath,
        );
        if (!uploaded.ok) {
          throw new Error(uploaded.message ?? "Avatar upload failed.");
        }
        avatarUrl = uploaded.value;
        pendingAvatarPath = null;
      }
      ...
      const mergedRes = await updateCreatorProfile(adapter.marketplace, {
        bio: bio.trim(),
        tipUrl: tipUrl.trim(),
        socialLinks: cleanedSocials,
      });
      if (!mergedRes.ok) {
        throw new Error(mergedRes.message ?? "Could not save the profile.");
      }
      const merged: CreatorProfile = mergedRes.value;
  ...
  async function loadPreview(): Promise<void> {
    ...
    previewLoading = true;
    previewError = null;
    const res = await getCreatorProfile(adapter.marketplace, handle);
    if (res.ok) {
      preview = res.value;
    } else {
      preview = null;
  ...
  $effect(() => {
    ...
    resolvedOnce = true;
    void (async () => {
      // Any failure (signed out, transport, route not implemented yet) →
      // fall back to the claim step. Never surface as a blocking error.
      const res = await loadMyCreator(adapter.marketplace);
      if (res.ok && res.value) {
        const me = res.value;
        handle = me.handle;
        bio = me.bio ?? "";
      ...
      // res.value === null → leave handle null → claim step (graceful).
      resolving = false;
    })();
  });
</script>
...
<div class="profile" data-testid="profile-panel">
  ...
  {:else if handle === null}
    ...
    <section class="claim" data-testid="profile-claim">
      ...
      {#if formatHint}
        <p class="field-hint warn" data-testid="profile-handle-format-hint">
          {formatHint}
        </p>
      {:else if handleInput.trim().length > 0}
        <p class="field-hint ok" data-testid="profile-handle-ok-hint">
          Looks valid — claim to confirm it’s available.
        </p>
      {:else}
        <p class="field-hint">
      ...
      {#if handleTaken}
        <p
          class="claim-status taken"
          role="alert"
          data-testid="profile-handle-taken"
        >
          <strong>@{handleInput.trim()}</strong> is unavailable — try another.
        </p>
      {:else if claimError}
        <p
          class="claim-status error"
          role="alert"
          data-testid="profile-claim-error"
        >
          {claimError}
        </p>
      {/if}
  ...
  {:else}
    ...
    <header class="profile-head">
      ...
      <p class="profile-sub">
        Edit your public bio, avatar, links, and tip URL.
      </p>
    </header>
    ...
    <section class="edit" data-testid="profile-edit">
        ...
        <div class="avatar-row">
          ...
          <div class="avatar-actions">
            ...
            {#if avatarPreviewName}
              <span class="avatar-chosen" title={pendingAvatarPath}
        ...
        {#each socialLinks as link, i (i)}
          <div class="social-row" data-testid="profile-social-row">
            ...
            <input
              ...
              class:invalid={link.url.trim().length > 0 &&
                socialUrlHint(link.url) !== null}
              type="url"
              placeholder="https://…"
          ...
          {#if link.url.trim().length > 0 && socialUrlHint(link.url)}
            <p class="field-hint warn" data-testid="profile-social-url-hint">
              {socialUrlHint(link.url)}
            </p>
          {/if}
        {/each}
        <button
      ...
      <div class="field">
        ...
        />
        {#if tipUrlHint}
          <p class="field-hint warn" data-testid="profile-tip-hint">
            {tipUrlHint}
          </p>
        {:else}
          <p class="field-hint">
      ...
      <div class="save-row">
        ...
        {#if saved}
          <span class="save-ok" role="status" data-testid="profile-save-ok"
            >✓ Saved.</span
          >
        {/if}
      </div>
    ...
    <section class="preview" data-testid="profile-preview">
        ...
        <article class="preview-card">
          <div class="preview-id">
            ...
            {:else}
              ...
              >
                {(preview.creator.displayName || preview.creator.handle)
                  .slice(0, 1)
                  .toUpperCase()}
              </div>
            {/if}
          ...
          {#if preview.creator.bio}
            <p class="preview-bio" data-testid="profile-preview-bio">
              {preview.creator.bio}
            </p>
          {/if}
          ...
          <div class="preview-listings">
            <span class="preview-listings-title"
              ...
              {preview.listings.length === 1
                ? "published pack"
                : "published packs"}</span
            >
            {#each preview.listings as listing (listing.id)}
              <span
                class="preview-listing"
                data-testid="profile-preview-listing">{listing.name}</span
              >
            {/each}
          </div>
        </article>
      {:else}
        <p class="preview-empty">
          Save your profile to preview it, or load the current public version.
        </p>
      {/if}
    </section>
  {/if}
