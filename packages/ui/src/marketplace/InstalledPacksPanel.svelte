<script lang="ts">
  ...
  interface Props {
     ...
     *  canManagePackages). Without it the panel renders the standard
     *  degraded state instead of dead controls. */
    adapter: PlatformAdapter;
    /** Optional desktop package-operation event stream. */
    packagesEvents?: PackagesEvents | null;
  }

  let { adapter, packagesEvents = null }: Props = $props();

  const canManage = $derived(adapter.isAvailable("canManagePackages"));
  ...
  let repairCommandState = $state<"idle" | "copying" | "copied" | "failed">(
    "idle",
  );
  ...
  const updatesCount = $derived(
    installed.filter((p) => p.updateAvailable).length,
  );
  ...
  async function refresh(): Promise<void> {
    ...
    refreshing = true;
    const res = await adapter.packages.listPackages();
    if (!res.ok) {
      errorMsg =
        res.reason === "unavailable"
          ? "Installed packs need the HQ desktop app."
          : (res.message ?? "Installed packs could not be loaded");
      errorContext = "read";
      refreshing = false;
      return;
    }
    {
      const next = res.value as unknown as PackagesView;
      if (next.packs) {
        view = next;
        errorMsg = null;
      } else {
  ...
  async function install(source: string, registry = false): Promise<void> {
    ...
    if (!res.ok) {
      ...
      busy = null;
    } else if (!packagesEvents) {
      // Without the event stream there is no `packages:complete` — settle here.
      busy = null;
      await refresh();
    }
  }
  ...
  async function update(name: string): Promise<void> {
    ...
    logLines = [];
    errorMsg = null;
    const res = await adapter.packages.update(name);
    if (!res.ok) {
  ...
  async function uninstall(name: string): Promise<void> {
    ...
    logLines = [];
    errorMsg = null;
    const res = await adapter.packages.uninstall(name);
    if (res.ok) {
      logLines = [`Uninstalled ${name}.`];
      await refresh();
      confirmUninstall = null;
    } else {
      errorMsg = res.message ?? "Uninstall failed.";
      errorContext = "mutation";
    }
    busy = null;
  }
  ...
  function checkUpdates(): void {
    void adapter.packages.checkUpdates();
  }
    ...
    void (async () => {
      ...
      if (packagesEvents) {
        const unlisten = await packagesEvents.subscribe({
            ...
            onError: (done) => {
              errorMsg = done.message ?? "Operation failed";
              errorContext = "mutation";
              busy = null;
            },
            onUpdates: (payload) => {
              ...
              view = {
                packs: payload.packs,
                registry: payload.registry ?? view?.registry ?? null,
                error: null,
              };
  ...
  function clearClipboardFailure(
    p: InstalledPack,
    action: ClipboardAction,
  ): void {
    clipboardFailures = {
  ...
  async function copySetupPrompt(p: InstalledPack): Promise<void> {
    ...
    try {
      ...
      console.error(
        "installed-packs: setup-prompt clipboard write failed",
        err,
      );
...
<div class="installed-packs" data-testid="installed-packs-panel">
  {#if !canManage}
    <UnavailableNote
      ...
      message="Managing installed packs needs the HQ desktop app."
      testid="installed-packs-unavailable"
    />
  {:else}
    <div class="toolbar">
            ...
            <div class="row-main">
              ...
              {#if getStartedCommand(p)}
                ...
                {#if clipboardFailure(p, "get-started")}
                  {@const failure = clipboardFailure(
                    p,
                    "get-started",
                  ) as ClipboardFailure}
                  <p
            ...
            -->
              {#if isPromptRenderable(p)}
                <div class="setup-prompt" data-testid="installed-setup-prompt">
                  ...
                  <pre class="setup-prompt-text">{p.initialization
                      ?.prompt}</pre>
                  <p class="setup-prompt-note">
                    The pack author's setup prompt.
                  </p>
                </div>
            ...
            <div class="row-actions">
              {#if p.updateAvailable}
                <button
                  ...
                  aria-label={isPackBusy("update", p.name)
                    ? `Updating ${p.name}`
                    : `Update ${p.name}`}
                >
              ...
              <button
                ...
                aria-label={isPackBusy("uninstall", p.name)
                  ? `Removing ${p.name}`
                  : `Uninstall ${p.name}`}
              >
            ...
            <div class="row">
              <div class="row-main">
                <div class="row-title">
                  ...
                  {#if isGatedOff(a)}<span class="pill warn">gated off</span
                    >{/if}
                </div>
                {#if a.description}<div class="row-sub">
                    {a.description}
                  </div>{/if}
              </div>
              <div class="row-actions">
                <button
                  ...
                  aria-label={isPackBusy("install", a.source)
                    ? `Installing ${shortSource(a.source)}`
            ...
            <div class="row">
              ...
              <div class="row-actions">
                <button
                  ...
                  aria-label={isPackBusy("install", r.slug)
                    ? `Installing ${shortSource(r.slug)}`
...
<style>
  ...
  .row {
    ...
    gap: var(--v4-space-3);
    min-width: 0;
    padding: var(--v4-space-3) var(--v4-space-3) var(--v4-space-3)
      calc(var(--v4-space-3) + 4px);
    border: 0;
    border-radius: 0;
    background: transparent;
    box-shadow: none;
