...
describe("createWizardRouter", () => {
  ...
  it("starts at screen 0 by default and accepts a valid start option", () => {
    ...
    expect(createWizardRouter({ start: 4 }).currentStep).toBe(4);
  });
  ...
  it("clamps next() at the final step", () => {
    ...
    expect(
      router.canGoNext(
        makeState({ installPath: "/tmp/hq", consentAnswered: true }),
      ),
    ).toBe(false);
  });
...
describe("getStepValidity", () => {
  it("requires a non-empty installPath on the directory screen", () => {
    ...
    expect(getStepValidity(1, makeState({ installPath: "/tmp/hq" }))).toBe(
      true,
    );
  });
  ...
  it("keeps setup invalid for manual next because setup auto-advances", () => {
    ...
    expect(getStepValidity(2, makeState({ installPath: "/tmp/hq" }))).toBe(
      false,
    );
  });

  it("blocks the consent step until the telemetry question is answered", () => {
    expect(getStepValidity(3, makeState({ consentAnswered: false }))).toBe(
      false,
    );
