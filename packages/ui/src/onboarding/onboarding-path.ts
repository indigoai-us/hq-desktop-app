...
/** Primary directory-step action when we already know whether the folder is an HQ. */
export function directoryPrimaryAction(existingHq: boolean): "use" | "install" {
  return existingHq ? "use" : "install";
}

export function directoryPrimaryLabel(existingHq: boolean): string {
  return existingHq ? "Use this HQ" : "Install";
}
