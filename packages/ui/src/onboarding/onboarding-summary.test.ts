...
describe("onboarding summary launch state", () => {
  it("selects the newest available tool and uses deterministic priority for ties", () => {
    ...
    ).toEqual({ kind: "grok", label: "Open in Grok" });
    expect(
      selectPrimaryLaunch(
        tools({ claude_cli: true, codex_cli: true, grok_cli: true }),
      ),
    ).toEqual({ kind: "claude", label: "Open in Claude Code" });
  });
...
describe("onboarding summary ready command", () => {
  ...
  it("opens the folder directly when no terminal CLI is detected", () => {
    expect(
      readyCommandFor("/Users/me/hq", tools({ claude_desktop: true })),
    ).toBe('open "/Users/me/hq"');
  });
...
describe("onboarding summary tool-state updates", () => {
  ...
  it("clears any when the removed tool was the only detected option", () => {
    expect(
      markToolUnavailable(tools({ claude_desktop: true }), "claude_desktop"),
    ).toMatchObject({
      claude_desktop: false,
      any: false,
    });
