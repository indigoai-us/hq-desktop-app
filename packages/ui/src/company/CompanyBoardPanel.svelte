<script lang="ts">
  ...
  import {
    ...
    withProjectStatus,
  } from "../projects/local-projects";
  import type { WorkProvenance } from "../common/provenance";
  import {
    OVERVIEW_PROJECT_LIMIT,
    progressiveWindow,
  } from "../common/progressive-collection";
  import {
    classifyStories,
    projectDisplayName,
    projectListStatus,
  ...
  import OverviewActivityDigest from "../common/OverviewActivityDigest.svelte";
  import { companyStore } from "./company-store.svelte";
  import ProvenanceLine from "../common/ProvenanceLine.svelte";
  import type { HomeCardModel } from "../home/home-model";
  import type { PlatformAdapter } from "@hq/platform";
  import "../chat/tokens.css";

  interface Props {
    /** Platform backend seam (drill-in ProjectDetailView needs the full adapter). */
  ...
  const resourcesEnabled = $derived(
    cloudBacked && !connectionIssue && syncEnabled,
  );
  const summaryState = useCompanySummary({
    slug: () => slug,
    enabled: () => resourcesEnabled,
  });
  const boardState = useCompanyBoard({
    slug: () => slug,
    enabled: () => resourcesEnabled,
  });
  ...
  const storiesInProgress = $derived(
    boardState.board.doing.length + boardState.board.review.length ||
      incompleteStoryCount(inFlightProjects),
  );
  ...
  const cloudPulse = $derived.by(
    (): { label: string; tone: "ok" | "warn" | "error" | "idle" } => {
      if (connectionIssue)
        return { label: "cached data · reconnect needed", tone: "error" };
   ...
   */
  const needsYouCards = $derived.by(
    (): Array<HomeCardModel & { id: string }> => {
      ...
      if (connectionIssue) {
        cards.push({
          ...
          tone: "error",
          actions: [
            { id: "inspect-local", label: "Inspect", kind: "secondary" },
          ],
        });
      } else if (!cloudBacked) {
        cards.push({
        ...
        cards.push({
          ...
          tone: "warn",
          actions: [
            { id: "inspect-paused", label: "Inspect", kind: "secondary" },
          ],
        });
      }
  ...
  $effect(() => {
    const targets = visibleInFlightProjects.filter(
      (project) => project.prdPath,
    );
  ...
  function objectiveIds(objective: Objective): Set<string> {
    ...
    const linearId = (
      objective as Objective & { linearInitiativeId?: string | null }
    ).linearInitiativeId;
  ...
  function incompleteStoryCount(items: Project[]): number {
    return items.reduce(
      (sum, project) =>
        sum + Math.max(0, project.storiesTotal - project.storiesComplete),
      0,
    );
  }
  ...
  function projectsAcceptancePercent(items: Project[]): number {
    ...
    const complete = items.reduce(
      (sum, project) => sum + project.storiesComplete,
      0,
    );
  ...
  function objectiveProgress(objective: Objective): number {
    ...
    if (krPercents.length > 0) {
      return clampPercent(
        krPercents.reduce((sum, percent) => sum + percent, 0) /
          krPercents.length,
      );
    }
  ...
  function rowStatus(
    project: Project,
    detail: InFlightDetail | undefined,
  ): RowStatus {
...
<section
  class="company-board"
  aria-label="Board"
  data-testid="company-board-panel"
>
  {#if selected}
    <ProjectDetailView
      {adapter}
  ...
  {:else}
    <div class="overview-content" data-testid="company-overview">
      {#if error}
        <div class="board-error" role="alert" data-testid="board-error">
          {error}
        </div>
      {/if}
      ...
      >
        ...
        <div class="pulse-item pulse-cloud">
          <span class={`status-dot ${cloudPulse.tone}`} aria-hidden="true"
          ></span>
      ...
      <div class="overview-columns">
              ...
              <div class="needs-queue">
                {#each needsYouCards as card (card.id)}
                  <NeedsYouCard
                    {card}
                    onaction={(actionId) =>
                      handleNeedsYouAction(card.id, actionId)}
                  />
                {/each}
              </div>
            {/if}
                  ...
                  <div class="work-row" data-testid="inflight-row">
                    ...
                    >
                      ...
                      <span class="work-meta"
                        >{projectMeta(project, detail)}</span
                      >
                      <div
                        class="work-provenance"
                        data-testid="inflight-provenance"
                      >
                    ...
                    <span class="goal-chip" data-testid="inflight-goal-chip"
                      >{goalChip(project)}</span
                    >
                    <span
                      class="status-pill"
                    ...
                    <span
                      class="ac-cell"
                      aria-label={`${progress.complete} of ${progress.total} AC`}
                    >
        ...
        <div class="overview-col overview-col-side">
          ...
          <section
            class="overview-section"
            data-testid="overview-activity-section"
          >
...
<style>
  ...
  .board-error {
    ...
    border-bottom: 1px solid
      color-mix(in srgb, var(--v4-error) 28%, var(--v4-rowline));
    border-radius: 0;
    background: transparent;
    color: var(--v4-error);
  ...
  .work-row {
    display: grid;
    grid-template-columns: minmax(0, 1fr) minmax(72px, 100px) auto minmax(
        72px,
        96px
      );
    align-items: center;
