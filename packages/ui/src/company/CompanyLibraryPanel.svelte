<script lang="ts">
  ...
  import { loadLibraryCompany, type LibraryItems } from "../library/library";
  import {
    subscribeLibraryRefresh,
    type LibraryRefreshHost,
    type UnlistenFn,
  } from "../library/library-refresh";
  ...
  interface Props {
    ...
    /** Company library payload (adapter library slice). */
    library: LibraryApi;
    /**
     * Optional focus/sync refresh host (desktop). When absent the panel still
     * loads once per slug — no dead affordance, just no auto-refresh.
     */
    refreshHost?: LibraryRefreshHost | null;
  }

  let { slug, forcedFilter, library, refreshHost = null }: Props = $props();
  ...
  $effect(() => {
    ...
    void (async () => {
      try {
        const result = await loadLibraryCompany(library, activeSlug);
        if (cancelled) return;
        if (!result.ok) {
          throw new Error(result.message ?? result.code ?? result.reason);
        }
        items = result.value;
      } catch (err) {
...
<section
  ...
  aria-label={forcedFilter === "workers"
    ? "Workers"
    : forcedFilter === "skills"
      ? "Skills"
      : "Library"}
...
>
  <header class="company-library-header">
    ...
    <span
      >{loading
        ? "Loading"
        : `${visibleCount} ${visibleCount === 1 ? "item" : "items"}`}</span
    >
  </header>
  ...
  {#if !loading && !error && items.workers.length === 0 && items.skills.length === 0}
    <div class="empty-state">
      {#if forcedFilter === "skills"}
        No company-specific skills yet. Shared skills live in the top-level
        Library.
      {:else if forcedFilter === "workers"}
        No company-specific workers yet. Shared workers live in the top-level
        Library.
      {:else}
        No company-specific workers or skills yet. Shared ones live in the
        top-level Library.
      {/if}
    </div>
  {:else}
    <LibraryBrowser {items} {loading} {error} {forcedFilter} {library} />
  {/if}
</section>
