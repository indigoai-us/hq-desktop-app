<script lang="ts">
  ...
  interface Props {
    ...
    /** Company telemetry + member roster (cloud). */
    company: CompanyApi;
    /** Contact fallback when the company roster is not resolvable. */
  ...
  $effect(() => {
    ...
    void (async () => {
      try {
        // NOTE: the adapter telemetry call is slug-keyed and does not carry the
        // from/to range yet; `range` is computed for parity with the desktop
        // command and future adapter widening.
        void range;
        void activeCompanyUid;
        const [rawRes, contactResponse] = await Promise.all([
          company.getTeamTelemetry(activeSlug),
          company
            .listMembers(activeSlug)
            .then((res): ContactsResponse => {
              if (res.ok)
                return { contacts: res.value as unknown as ContactRow[] };
              throw new Error(res.message ?? res.code ?? res.reason);
            })
            .catch(async (): Promise<ContactsResponse> => {
              if (!messaging) return { contacts: [] };
              const contacts = await messaging.listContacts();
              return contacts.ok
                ? { contacts: contacts.value as unknown as ContactRow[] }
                : { contacts: [] };
            }),
        ]);
        if (!rawRes.ok) {
          const parts = [rawRes.code, rawRes.message].filter(Boolean);
          throw new Error(parts.join(": ") || rawRes.reason);
        }
        const raw: unknown = rawRes.value;
        if (cancelled) return;
...
<section class="team-panel" aria-label="Team" data-testid="company-team-panel">
  <header class="team-header">
    ...
    <div
      class="team-actions detail-primary-actions primary-actions"
      data-testid="team-primary-actions"
    >
      <button
        type="button"
  ...
  {#if loading}
    <p class="team-status" data-testid="team-loading" aria-busy="true">
      Loading team…
    </p>
  {:else if view.error}
  ...
  {:else if view.empty}
    <div class="team-empty" data-testid="team-empty">
      ...
      <p class="team-empty-meta">
        Usage appears after members work with HQ. Invite teammates from the
        console when ready.
      </p>
    </div>
  {:else}
      ...
      <div class="detail-pane team-detail-pane" data-testid="team-detail-pane">
            ...
            <header class="team-detail-header">
              ...
              <div class="team-detail-heading title-stack">
                ...
                {#if selectedMember.email && selectedMember.email !== selectedMember.displayName}
                  <span
                    class="team-detail-email"
                    data-testid="team-detail-email"
                  >
            ...
            <div class="team-section-grid">
              {#if selectedMember.topSkills.length > 0}
                <section
                  ...
                  data-testid="team-member-skills"
                >
                ...
                >
                  ...
                  <div class="chip-row">
                    {#each selectedMember.activeProjects as project (project)}
                      <span class="project-chip" data-testid="team-project-chip"
                        >{project}</span
                      >
                    {/each}
                  </div>
              ...
              {#if selectedMember.topSkills.length === 0 && selectedMember.activeProjects.length === 0}
                <p
                  class="team-status subtle"
                  data-testid="team-detail-no-activity"
                >
        ...
        {:else}
          <div class="team-detail-empty" data-testid="team-detail-empty">
            ...
            <p class="team-empty-meta">
              Humans and agents share one list. Open a row for skills and
              projects from real telemetry.
            </p>
          </div>
        {/if}
      </div>
