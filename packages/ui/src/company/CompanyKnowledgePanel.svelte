<script lang="ts">
  ...
  interface Props {
    slug: string;
    /** Platform seam — `files.*` for the tree, full adapter for the preview pane. */
    adapter: PlatformAdapter;
  }

  let { slug, adapter }: Props = $props();
  const files = $derived(adapter.files);
  ...
  function loadChildren(relPath: string): Promise<DirEntry[]> {
    if (!inKnowledgeScope(relPath)) {
      return Promise.reject(
        new Error(`path outside company knowledge scope: ${relPath}`),
      );
    }
    return files.listDir(relPath).then((res) => {
      if (res.ok) return res.value as unknown as DirEntry[];
...
>
  <aside
    class="list-pane knowledge-tree-pane"
  ...
  <div
    class="detail-pane knowledge-preview-pane"
    data-testid="knowledge-preview-pane"
  >
    {#if selectedPath}
      <button
        type="button"
    ...
    {:else}
      ...
      >
        ...
        <p id="knowledge-empty-description" class="knowledge-empty-meta">
          Select a file from the knowledge tree, or search by name. Markdown,
          images, PDFs, and text open here.
        </p>
      </div>
    {/if}
  </div>
