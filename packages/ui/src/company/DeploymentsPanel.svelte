<script lang="ts">
  ...
  interface Props {
    slug: string;
    /** Adapter slices for the Claude Code deploy-workflow handoff. */
    workflow: AgentWorkflowApi;
    /** Open a URL in the host's external browser (rows' Open action). */
  ...
  $effect(() => {
    ...
    void companyStore
      .loadDeployments(slug, force)
      .then((result) => {
        if (!cancelled) {
          deployments = Array.isArray(result)
            ? result.map(normalizeDeployment)
            : [];
        }
      })
      .catch((err) => {
        ...
        if (!cancelled) {
          error = isCompanyResourceUnavailable(err) ? null : String(err);
  ...
  function matchesDeploymentQuery(
  ...
  async function openDeployWorkflow(): Promise<void> {
    ...
    try {
      const result = await openAgentWorkflow(
        workflow,
        prompt,
        "deploy workflow",
      );
...
<section
  class="deployments-panel"
  aria-labelledby="deployments-panel-title"
  data-testid="company-deployments-panel"
>
    ...
    <div class="deployments-title title-stack">
      ...
      <span
        >{loading
          ...
          : error
            ? "Couldn't load"
            : `${deployments.length} subdomains`}</span
      >
    </div>

    <div
      class="deployments-controls detail-primary-actions primary-actions"
  ...
  {#if !cloudBacked}
    <div class="deployments-error deployments-note" role="status">
      <div>
        ...
        <span
          >Local-only companies can be opened and planned, but deploy targets
          need a cloud-backed company.</span
        >
      </div>
    </div>
  {:else if !syncEnabled}
    <div class="deployments-error deployments-note" role="status">
      <div>
        ...
        <span
          >Cloud membership is still connected — turn sync On to refresh deploy
          metadata.</span
        >
      </div>
    </div>
  ...
  {#if unavailable}
    <UnavailableNote
      label="Deployments"
      message="Deployment metadata isn't available on this platform yet."
      testid="deployments-unavailable"
    />
  {/if}
  ...
  <section
    class="deployments-card"
    aria-labelledby="deployments-list-title"
    aria-busy={loading}
  >
