<script lang="ts">
  ...
  import CompanyBoardPanel from "./CompanyBoardPanel.svelte";
  import CompanyGoalsPage from "../projects/CompanyGoalsPage.svelte";
  import CompanyProjectsPage from "../projects/CompanyProjectsPage.svelte";
  import CompanyOperationsPanel from "./CompanyOperationsPanel.svelte";
  ...
  import type { LibraryRefreshHost } from "../library/library-refresh";
  import {
  ...
  interface Props {
    /** Platform backend seam. company/settings/shell/sync/files/library/messaging slices are used. */
  ...
  // Wire the module-level company store to this host's adapter, then start
  // its shared 30s poll. Both are idempotent, so a remount is safe; the effect
  // re-wires if the host ever swaps the adapter prop.
  $effect.pre(() => {
    configureCompanyApi(adapter.company);
    startCompanyStore();
  });
  ...
  const connectable = $derived(
    company.state === "local-only" || company.state === "broken",
  );
  ...
  async function handleConnect() {
    ...
    connectBusy = true;
    try {
      const res = await adapter.company.connectToCloud(company.slug);
      if (!res.ok) throw new Error(failureMessage(res));
      onworkspaceschanged?.();
    } catch (err) {
      console.error(`connectToCloud(${company.slug}) failed:`, err);
  ...
  async function handleAcceptPendingInvite() {
    ...
    try {
      ...
      if (result.claimedSlugs?.length) {
        // Desktop-only capability: on hosts without local sync this resolves
        // `unavailable`, which is a silent no-op here (the claim already landed).
        void adapter.sync.startSync(company.slug).then((res) => {
          if (!res.ok && res.reason === "error") {
            console.warn("post-claim sync failed (non-fatal):", res);
          }
        });
      }
    } catch (err) {
      console.error(`claimPendingInvite(${company.slug}) failed:`, err);
      actionError =
  ...
  async function startNewProject() {
    ...
    try {
      const settingsRes = await adapter.settings.getSettings();
      const settings: SettingsWire = settingsRes.ok
        ? (settingsRes.value as SettingsWire)
        : { hqPath: null };
      ...
      if (!opened.ok) throw new Error(failureMessage(opened));
    } catch (err) {
      console.error("openClaudeCodeLink for new project failed:", err);
      try {
        await navigator.clipboard.writeText(prompt);
        actionError =
          "Project prompt copied. Paste it into Claude Code to continue.";
      } catch {
        actionError =
          "Could not open Claude Code. The Projects screen is open.";
      }
    } finally {
...
<section class="company-page" aria-labelledby="company-page-title">
  ...
  {#if pendingInvite}
    <section
      ...
      data-testid="company-invite-gate"
    >
      ...
      <div class="company-panel">
        ...
        {:else if tab === "skills"}
          <CompanyLibraryPanel
            ...
            refreshHost={libraryRefreshHost}
          />
        {:else if tab === "workers"}
          <CompanyLibraryPanel
            slug={company.slug}
        ...
        {:else if tab === "knowledge"}
          <CompanyKnowledgePanel slug={company.slug} {adapter} />
        {:else if tab === "team"}
          <TeamPanel
            slug={company.slug}
...
<style>
  ...
  .company-actions button {
    ...
    cursor: pointer;
    transition:
      opacity 120ms cubic-bezier(0.33, 1, 0.68, 1),
