      ...
      <div class="agenda-list">
        {#each group.events as event (event.id)}
          ...
          >
            ...
            <div class="mmeta">
              <div class="mname">
                {#if state === "live"}<span class="dot-live" aria-hidden="true"
                    >&#9679;</span
            ...
            <div class="mactions detail-primary-actions">
              {#if url}
                <button
                  ...
                  title={openingEventIds.has(event.id)
                    ? "Opening meeting…"
              ...
              {:else if kind === "invite"}
                <button
                  ...
                  title={invitePending
                    ? "Inviting…"
                    : pending
                      ? "Wait for the current bot action"
                      : recurring
                        ? "Invite bot to this series"
                        : "Invite bot to this meeting"}
                  aria-busy={invitePending}
                  aria-pressed={false}
                  aria-label={invitePending
                    ? "Inviting bot"
                    : recurring
                      ? "Invite bot to series"
                      : "Invite bot"}
              ...
              {:else if kind === "invited"}
                <button
                  ...
                  data-notetaker="on"
                  disabled={pending}
                  title={uninvitePending
                    ? "Cancelling…"
                    : pending
                      ? "Wait for the current bot action"
                      : recurring
                        ? "Bot scheduled for series — click to uninvite series"
                        : "Bot scheduled — click to uninvite"}
                  aria-busy={uninvitePending}
                  aria-pressed={true}
                  aria-label={uninvitePending
                    ? "Cancelling bot invitation"
                    : recurring
                      ? "Uninvite bot from series"
                      : "Uninvite bot"}
                ...
                <button
                  ...
                  title={uninvitePending
                    ...
                    : pending
                      ? "Wait for the current bot action"
                      : recurring
                        ? "Bot is in this series — click to remove from series"
                        : "Bot is in the meeting — click to remove"}
                  aria-busy={uninvitePending}
                  aria-pressed={true}
                  aria-label={uninvitePending
                    ? "Removing bot"
                    : recurring
                      ? "Remove bot from series"
                ...
                <button
                  ...
                  title={uninvitePending
                    ...
                    : pending
                      ? "Wait for the current bot action"
                      : recurring
                        ? "Bot is joining this series — click to cancel series"
                        : "Bot is joining — click to cancel"}
                  aria-busy={uninvitePending}
                  aria-pressed={true}
                  aria-label={uninvitePending
                    ? "Cancelling bot join"
                    : recurring
                      ? "Cancel bot series join"
                      : "Cancel bot join"}
                  onclick={() => onUninvite(event)}
                >
                  <span
                    class="row-icon-spinner row-icon-spinner-amber"
                    aria-hidden="true"
                  ></span>
                </button>
              ...
              {#if url}
                <button
                  ...
                  disabled={pending}
                  title={joinNowPending
                    ? "Telling bot to join…"
                    : pending
                      ? "Wait for the current bot action"
                      : "Tell bot to join now"}
                  aria-busy={joinNowPending}
                  aria-label={joinNowPending
                    ? "Telling bot to join now"
                    : "Tell bot to join now"}
