...
describe("resolveInviteCompanyId", () => {
  const known = new Set(["co_mine", "co_other"]);

  it("uses the settings default when the event has no company", () => {
    expect(resolveInviteCompanyId(null, "co_mine", known)).toBe("co_mine");
    expect(resolveInviteCompanyId(undefined, "co_mine", known)).toBe("co_mine");
  });

  it("lets the event company win over the settings default", () => {
    expect(resolveInviteCompanyId("co_event", "co_mine", known)).toBe(
      "co_event",
    );
  });

  it("treats a foreign/stale settings default as unset (no cross-company leak)", () => {
    expect(resolveInviteCompanyId(null, "co_foreign", known)).toBeNull();
    expect(resolveInviteCompanyId(undefined, "co_foreign", known)).toBeNull();
  });

  it("returns null when neither event nor a valid default is present", () => {
    expect(resolveInviteCompanyId(null, null, known)).toBeNull();
    expect(resolveInviteCompanyId(null, "  ", known)).toBeNull();
  });
});
...
describe("meetings-model", () => {
  ...
  it("uses a company name or a graceful fallback, never a raw company UID", () => {
    ...
    expect(companyLabel(event, new Map([["cmp_company", "Indigo"]]))).toBe(
      "Indigo",
    );
  ...
  describe("recurring meeting detection", () => {
    ...
    it("derives the series id from Google instance ids for legacy payloads", () => {
      const event = eventAt(
        "team_sync_20260527T190000Z",
        new Date(2026, 4, 27, 12, 0, 0),
      );
  ...
  describe("botForEvent", () => {
    ...
    it("prefers the exact event bot when both exact and series bots match", () => {
      ...
      expect(
        botForEvent(event, new Map([[event.id, exact]]), [series, exact])
          ?.botId,
      ).toBe("bot-exact");
    });
    ...
    it("matches a recurring event row to a bot scheduled for the series", () => {
      ...
      expect(rowButtonKind(botForEvent(event, new Map(), [seriesBot]))).toBe(
        "invited",
      );
    });
    ...
    it("falls back to normalized meeting URL when calendar IDs are missing or stale", () => {
      ...
      expect(botForEvent(event, new Map(), [urlOnlyBot])?.botId).toBe(
        "bot-url-only",
      );
      expect(
        botAttachmentState(botForEvent(event, new Map(), [urlOnlyBot])),
      ).toBe("invited");
    });
    ...
    it("prefers series identity over URL when both would match", () => {
      ...
      expect(botForEvent(event, new Map(), [urlBot, seriesBot])?.botId).toBe(
        "bot-series",
      );
    });
  ...
  describe("normalizeMeetingUrl", () => {
    it("collapses Zoom region hosts + drops pwd/query/fragment noise", () => {
      expect(
        normalizeMeetingUrl(
          "https://us02web.zoom.us/j/8412345678?pwd=secret&status=success#room",
        ),
    ...
    it("returns null for unsupported or malformed URLs", () => {
      ...
      expect(
        normalizeMeetingUrl("http://meet.google.com/abc-defg-hij"),
      ).toBeNull();
      expect(normalizeMeetingUrl("https://example.com/meet")).toBeNull();
      expect(
        normalizeMeetingUrl("https://acme.webex.com/meet/room"),
      ).toBeNull();
    });
  });
  ...
  describe("botAttachmentState — visible list lifecycle", () => {
    ...
    it("distinguishes available-to-invite / invited / joining / recording / completed", () => {
      ...
      expect(botAttachmentState(bot("completed", { sourceLanded: true }))).toBe(
        "completed",
      );
      expect(
        botAttachmentState(bot("completed", { sourceLanded: false })),
      ).toBe("processing");
    });
  ...
  describe("bot list lookup helpers", () => {
    ...
    it("keeps authoritative per-event bot rows before legacy full-list rows", () => {
      ...
      expect(
        mergeScheduledBots([eventBot], [fullListBot, recordedBot]),
      ).toEqual([eventBot, recordedBot]);
    });
    ...
    it("requires per-event bot rows when visible event ids are known", () => {
      ...
      expect(
        mergeScheduledBotLookups(["event-1"], null, [fullListBot]),
      ).toBeNull();
  ...
  describe("rowButtonKind — gates Done on the real source-landed signal", () => {
    ...
    it("REGRESSION: completed bot whose ingest FAILED (sourceLanded:false) does NOT render done", () => {
      ...
      expect(
        rowButtonKind(bot({ status: "completed", sourceLanded: false })),
      ).toBe("processing");
    });
    ...
    it("completed bot whose source LANDED (sourceLanded:true) renders done", () => {
      expect(
        rowButtonKind(bot({ status: "completed", sourceLanded: true })),
      ).toBe("done");
    });
    ...
    it("source-landed only flips done at the completed status — earlier statuses are unaffected", () => {
      ...
      expect(
        rowButtonKind(bot({ status: "scheduled", sourceLanded: true })),
      ).toBe("invited");
      expect(
        rowButtonKind(bot({ status: "joining", sourceLanded: true })),
      ).toBe("joining");
      expect(
        rowButtonKind(bot({ status: "recording", sourceLanded: true })),
      ).toBe("in-call");
      expect(
        rowButtonKind(bot({ status: "processing", sourceLanded: true })),
      ).toBe("processing");
    });
  ...
  describe("isPlausibleMeetingUrl", () => {
    it("accepts real Zoom / Google Meet / Teams / Webex links", () => {
      expect(
        isPlausibleMeetingUrl("https://us02web.zoom.us/j/8412345678"),
      ).toBe(true);
      // Bare zoom.us (no region subdomain) — common paste from calendar/email.
      expect(isPlausibleMeetingUrl("https://zoom.us/j/8412345678")).toBe(true);
      expect(
        isPlausibleMeetingUrl("https://meet.google.com/abc-defg-hij"),
      ).toBe(true);
      expect(
        isPlausibleMeetingUrl("https://teams.microsoft.com/l/meetup-join/xyz"),
      ).toBe(true);
      expect(isPlausibleMeetingUrl("https://acme.webex.com/meet/room")).toBe(
        true,
      );
    });
    ...
    it("rejects empty, non-meeting, and non-https URLs", () => {
      ...
      expect(isPlausibleMeetingUrl("   ")).toBe(false);
      expect(isPlausibleMeetingUrl("https://example.com/not-a-meeting")).toBe(
        false,
      );
      expect(isPlausibleMeetingUrl("http://meet.google.com/abc-defg-hij")).toBe(
        false,
      );
