...
/** Keep the next three weeks of upcoming rows plus a short past tail. */
export const AGENDA_UPCOMING_LIMIT = 60;
export const AGENDA_PAST_LIMIT = 20;
export const AGENDA_HORIZON_MS = 21 * 86_400_000;

/** Drop bulky calendar fields so the store does not retain full event payloads. */
export function slimMeetingEvent(event: MeetingEvent): MeetingEvent {
  ...
  if (event.summary != null) next.summary = event.summary;
  if (event.recurringEventId != null)
    next.recurringEventId = event.recurringEventId;
  if (event.hangoutLink != null) next.hangoutLink = event.hangoutLink;
  if (event.meetingUrl != null) next.meetingUrl = event.meetingUrl;
  if (event.sourceCalendarId != null)
    next.sourceCalendarId = event.sourceCalendarId;
  if (event.sourceCompanyUid != null)
...
/** Cap the in-memory agenda. Full calendar dumps make the page beachball. */
export function takeAgendaWindow(
  ...
  return [
    ...near.slice(0, AGENDA_UPCOMING_LIMIT),
    ...past.slice(0, AGENDA_PAST_LIMIT),
  ].map(slimMeetingEvent);
}
...
export const MEETINGS_UPCOMING_EMPTY =
  "No meetings in your synced calendars yet.";

/** US-010: loading copy while the first snapshot is still on its way. */
export const MEETINGS_LOADING_LABEL = "Loading your meetings…";

/** US-010: connect-first empty state when no calendar account is linked. */
export const MEETINGS_CONNECT_EMPTY_TITLE = "Connect a calendar";
export const MEETINGS_CONNECT_EMPTY_BODY =
  "Link a Google Calendar so HQ can show your meetings, send the notetaker, and capture recaps.";
