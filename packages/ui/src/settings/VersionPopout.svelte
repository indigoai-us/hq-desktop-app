<script lang="ts">
  ...
  interface Props {
    ...
    /** Updater seam (@hq/platform). Every call resolves to an AdapterResult. */
    updates: UpdatesApi;
    /** Read seam for the autoUpdate preference. */
    ...
    /** capabilities.canSelfUpdate — false renders the standard degraded state. */
  ...
  let phase = $state<"idle" | "checking" | "downloading" | "ready" | "error">(
    "idle",
  );
  ...
  const statusLabel = $derived.by(() => {
    if (!canSelfUpdate) return "Managed by this host";
  ...
  const showRestart = $derived(
    canSelfUpdate && Boolean(latestVersion) && phase !== "checking",
  );
  const displayedAutoUpdate = $derived(
    pendingAutoUpdate ?? autoUpdate ?? false,
  );
  const coreVersionDisplay = $derived.by(() => {
    if (!canSelfUpdate) return "Unavailable";
  ...
  const coreStatusLabel = $derived.by(() => {
    if (!canSelfUpdate) return "HQ Core is managed on the desktop app";
  ...
  $effect(() => {
    ...
    // is never missed.
    const unsubscribe = updaterBus?.subscribe({
      onAvailable: (info) => {
        if (cancelled) return;
        const next = info?.version;
        if (!next) return;
        updateLoadGeneration += 1;
        // installUpdate announces the same version again immediately before
        ...
        const sameInstall = phase === "downloading" && latestVersion === next;
    ...
    if (canSelfUpdate) {
      ...
      void (async () => {
        ...
        const res = await updates.getPendingUpdate();
  ...
  async function loadAutoUpdate() {
    ...
    if (res.ok) {
      const prefs = res.value as { autoUpdate?: boolean | null };
      autoUpdate = prefs?.autoUpdate ?? true;
    } else if (res.reason === "unavailable") {
      // No local preference store on this host — leave the toggle unresolved.
      autoUpdate = null;
    } else {
      console.error(
  ...
  async function refreshCoreInfo() {
    ...
    const versionRead = updates
      .getVersions()
      .then((res) => {
        if (generation !== coreLoadGeneration) return;
        if (res.ok) {
          coreVersion = res.value.core ?? null;
        } else {
          if (res.reason === "error") {
            console.error(
              "version-popout: HQ Core version refresh failed",
              res.message,
            );
          }
          coreVersion = null;
    ...
    const stateRead = updates
      ...
      .then((res) => {
        ...
        if (res.ok) {
          ...
          if (res.reason === "error") {
            console.error(
              "version-popout: HQ Core state refresh failed",
              res.message,
            );
          }
          coreState = null;
  ...
  async function handleCheckForUpdates() {
    if (!canSelfUpdate || checkingAnyUpdate || phase === "downloading") return;
    const generation = ++updateLoadGeneration;
    phase = "checking";
    errorMessage = null;
  ...
  async function handleRestartToUpdate() {
    ...
    phase = "downloading";
    errorMessage = null;
    // Backend re-runs the updater check inside installUpdate; on macOS the
    // process is usually replaced before this promise resolves.
    const res = await updates.installUpdate();
    if (updaterDisposed) return;
    if (generation === installActionGeneration) {
      if (res.ok) {
...
>
  <div class="vp-product">
    ...
    <div class="vp-rows">
      ...
      <div class="vp-row">
        ...
        <span class="vp-value mono" data-testid="version-popout-latest"
          >v{latestDisplay}</span
        >
      </div>
      <div class="vp-row">
        <span class="vp-label">Status</span>
        <span class="vp-status" data-testid="version-popout-status"
          >{statusLabel}</span
        >
      </div>
    </div>
  ...
  {#if !canSelfUpdate}
    <div class="vp-actions">
      <UnavailableNote
        label="Updates"
        message="App and HQ Core updates are managed from the HQ desktop app."
        testid="version-popout-unavailable"
      />
    </div>
  {:else}
    <div class="vp-actions">
  ...
  {#if canSelfUpdate}
    <div
      class="vp-toggle-block"
      aria-busy={autoUpdateLoading || autoUpdateSaving}
    >
...
<style>
  .version-popout {
    ...
    padding: 16px;
    border: 1px solid
      var(--border-strong, var(--pop-border, rgba(120, 120, 120, 0.3)));
  ...
  .vp-settings-link:focus-visible {
    outline: 2px solid
      var(--v4-focus-ring, var(--v4-control-border, currentColor));
