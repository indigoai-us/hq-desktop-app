...
describe("friendlyPackagesError", () => {
  it("turns a missing CLI spawn failure into actionable product language", () => {
    expect(
      friendlyPackagesError(
        "spawn `hq packs list --json`: No such file or directory (os error 2)",
      ),
    ).toBe("HQ tools need repair before installed packs can be loaded.");
  });

  it("keeps network and unknown failures concise", () => {
    expect(friendlyPackagesError("network request timed out")).toContain(
      "connection",
    );
    expect(friendlyPackagesError("unexpected parser failure")).toContain(
      "technical details",
    );
...
function init(overrides: Partial<PackInitialization> = {}): PackInitialization {
  return {
    entrypoint: "example",
    prompt: "Run the example to get going.",
    ...overrides,
  };
}
...
describe("isMarketplaceOrigin", () => {
  ...
  it("rejects local-path and git-URL origins (un-moderated)", () => {
    expect(isMarketplaceOrigin("github:owner/repo#packages/hq-pack-x")).toBe(
      false,
    );
...
describe("isPromptRenderable", () => {
  ...
  it("SUPPRESSES prose when the approval flag is missing (conservative default)", () => {
    ...
    expect(
      isPromptRenderable(
        pack({ source: "marketplace:impeccable", initialization: init() }),
      ),
    ).toBe(false);
    ...
    expect(
      isPromptRenderable(
        pack({
          source: "marketplace:impeccable",
          initialization: init({ promptModerated: false }),
        }),
      ),
    ).toBe(false);
  });
  ...
  it("SUPPRESSES when there is no prose or no initialization at all", () => {
    expect(isPromptRenderable(pack({ source: "marketplace:impeccable" }))).toBe(
      false,
    );
    expect(
      isPromptRenderable(
...
describe("shortSource (unchanged regression guard)", () => {
  it("drops the long git prefix to the trailing pack name", () => {
    expect(shortSource("github:owner/repo#packages/hq-pack-impeccable")).toBe(
      "hq-pack-impeccable",
    );
...
describe("packIdentity", () => {
  it("matches installed names to packs.yaml, scoped npm, and registry sources", () => {
    ...
    expect(packIdentity("@indigoai-us/hq-pack-engineering")).toBe(
      "hq-pack-engineering",
    );
    expect(
      packIdentity("github:indigoai-us/hq#packages/hq-pack-engineering@1.3.0"),
    ).toBe("hq-pack-engineering");
    expect(packIdentity("registry:hq-pack-engineering@1.3.0")).toBe(
      "hq-pack-engineering",
    );
  });
