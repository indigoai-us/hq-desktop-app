...
describe("filterLibraryItems", () => {
  const flat = toLibraryItems({
    workers: [
      ...
      worker({
        ...
        description: "APIs",
        type: "CodeWorker",
        path: "core/workers/public/dev-team/backend-dev/",
      }),
    ],
    skills: [skill()],
  });
  ...
  it("matches name, description, type and scope case-insensitively", () => {
    expect(
      filterLibraryItems(flat, "architect").map((i) => libraryItemKey(i)),
    ).toEqual(["worker:core/workers/public/dev-team/architect/"]);
...
describe("facets", () => {
  it("maps each item to its scope facet", () => {
    ...
    expect(
      libraryItemFacet({
        kind: "worker",
        worker: worker({ scope: "company", company: "indigo" }),
      }),
    ).toBe("indigo");
    ...
    expect(
      libraryItemFacet({
        kind: "skill",
        skill: skill({ scope: "personal", company: undefined }),
      }),
    ).toBe("personal");
    expect(
      libraryItemFacet({
        kind: "skill",
        skill: skill({ scope: "root", company: undefined }),
      }),
    ).toBe("core");
  });
