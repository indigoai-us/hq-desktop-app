 ...
 * to these TS types. This module owns the adapter calls and a small
...
import type { AdapterResult, LibraryApi } from "@hq/platform";
...
export function libraryFacets(items: LibraryItem[]): string[] {
  ...
  const companies = [...present]
    .filter((f) => f !== "core" && f !== "personal")
    .sort();
...
// ---- loaders ---------------------------------------------------------------
//
// Ported from desktop-alt: every former library command call is now a
// method on `LibraryApi` (the platform adapter seam). Loaders pass adapter
// failures through untouched (`unavailable` -> degraded UI, `error` -> error
// UI) and normalise the wire payload on success.

/** Load the root/shared library (public workers + root + personal skills). */
export async function loadLibraryRoot(
  api: LibraryApi,
): Promise<AdapterResult<LibraryItems>> {
  const res = await api.getRoot();
  if (!res.ok) return res;
  const wire = res.value as unknown as Partial<LibraryItems> | null;
  return {
    ok: true,
    value: { workers: wire?.workers ?? [], skills: wire?.skills ?? [] },
  };
}

/** Load a single company's library (its private workers + company skills). */
export async function loadLibraryCompany(
  api: LibraryApi,
  slug: string,
): Promise<AdapterResult<LibraryItems>> {
...
export async function loadWorkerDetail(
  api: LibraryApi,
  workerPath: string,
): Promise<AdapterResult<WorkerDetail>> {
  const res = await api.getWorkerDetail(workerPath);
  if (!res.ok) return res;
  const wire = res.value as unknown as Partial<WorkerDetail> | null;
  return {
    ok: true,
    value: {
      id: wire?.id ?? "",
...
export async function loadSkillDetail(
  api: LibraryApi,
  skillPath: string,
): Promise<AdapterResult<SkillDetail>> {
  const res = await api.getSkillDetail(skillPath);
  if (!res.ok) return res;
  const wire = res.value as unknown as Partial<SkillDetail> | null;
  return {
    ok: true,
    value: {
      name: wire?.name ?? "",
