...
describe("Files membership boundary", () => {
  ...
  it("extracts company slugs only from company-scoped HQ paths", () => {
    expect(companySlugForHqPath("companies/indigo/knowledge/overview.md")).toBe(
      "indigo",
    );
    expect(companySlugForHqPath("./companies/sender-agency")).toBe(
      "sender-agency",
    );
  ...
  it("rejects traversal, absolute, backslash, and ambiguous HQ paths before resolving a company", () => {
    expect(
      companySlugForHqPath("companies/indigo/../sender-agency/secret.md"),
    ).toBeNull();
    expect(
      companySlugForHqPath("companies\\sender-agency\\secret.md"),
    ).toBeNull();
    expect(
      companySlugForHqPath("/companies/sender-agency/secret.md"),
    ).toBeNull();
    expect(
      companySlugForHqPath("C:/companies/sender-agency/secret.md"),
    ).toBeNull();
    expect(
      companySlugForHqPath("companies//sender-agency/secret.md"),
    ).toBeNull();
  ...
  it("rejects pending, unknown, mismatched, and escaped company file routes", () => {
    ...
    expect(isFilesRouteAllowed({ slug: "sender-agency" }, workspaces)).toBe(
      false,
    );
    expect(
      isFilesRouteAllowed(
    ...
    expect(
      isFilesRouteAllowed(
        { slug: "indigo", path: "personal/overview.md" },
        workspaces,
      ),
    ).toBe(false);
    expect(
  ...
  it("filters pending and unknown company folders from HQ-root lazy results", () => {
    ...
    expect(
      filterFileEntriesForMembership(entries, [active, pending]).map(
        (entry) => entry.path,
      ),
...
describe("file-tree sortNodes (US-006)", () => {
  ...
  it("does not mutate the input array or input nodes (pure)", () => {
    ...
    const input: FileNode[] = [file("z.txt", "companies/test/z.txt"), root];
...
describe("file-tree lazy helpers (US-010)", () => {
  ...
  it("flattenLazy emits only loaded+expanded subtrees (lazy: unloaded dirs show no children)", () => {
    const tree: LazyNode[] = [
      // Loaded + has a child.
      lazyDir("companies", "companies", [
        lazyFile("manifest.yaml", "companies/manifest.yaml"),
      ]),
  ...
  it("filterLazyNodes is case-insensitive and empty query is identity", () => {
    ...
    expect(filterLazyNodes(tree, "  brand  ").map((n) => n.name)).toEqual([
      "Brand.md",
    ]);
  });
  ...
  it("filterLazyNodes does not mutate input", () => {
    const tree: LazyNode[] = [
      lazyDir("a", "a", [
        lazyFile("hit.md", "a/hit.md"),
        lazyFile("miss.md", "a/miss.md"),
      ]),
    ];
  ...
  it("parentPathOf returns the parent HQ-relative segment", () => {
    expect(parentPathOf("companies/x/knowledge/a.md")).toBe(
...
describe("file-tree row metadata", () => {
  ...
  it("keeps type metadata for top-level rows in the unscoped HQ tree", () => {
    expect(fileTreeRowMeta({ path: "companies", isDir: true }, "")).toBe(
      "Folder",
    );
    expect(fileTreeRowMeta({ path: "README.md", isDir: false }, "")).toBe(
      "File",
    );
  });
});
