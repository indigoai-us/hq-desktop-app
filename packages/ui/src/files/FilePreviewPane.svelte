<script lang="ts">
  ...
  interface Props {
    /** Platform seam: `files.*` for previews, `shell.*` + capability gates for
     *  the desktop-only Claude Code / Finder affordances. */
  ...
  /** Browse/preview unavailable on this platform (web: not-yet-implemented API). */
  ...
  $effect(() => {
    ...
    if (previewKind === "image" || previewKind === "pdf") {
      void adapter.files
        .getAuthorizedPreview(current)
        .then((res) => {
          if (cancelled) return;
          if (res.ok) {
            const { mimeType, dataBase64 } =
              res.value as unknown as AuthorizedFilePreview;
            mediaUrl = `data:${mimeType};base64,${dataBase64}`;
            unsupported = false;
          } else if (res.reason === "unavailable") {
            previewUnavailable = true;
          } else {
            console.error("files.getAuthorizedPreview failed:", res.message);
            mediaUrl = null;
            unsupported = true;
          }
        })
    ...
    void adapter.files
      ...
      .then((res) => {
        ...
        if (res.ok) {
          ...
          console.error("files.getFileContent failed:", res.message);
          content = null;
          unsupported = true;
        }
      })
  ...
  async function revealInFinder(): Promise<void> {
    ...
    revealError = null;
    try {
      const res = await adapter.files.revealInFinder(actedPath);
...
>
  <header class="preview-header">
    ...
    <div class="preview-actions detail-primary-actions primary-actions">
      ...
      <button
        ...
        title={copyError ??
          (pathCopied ? "Path copied" : `Copy path: ${copyPathValue}`)}
      ...
      {#if canReveal}<button
        ...
        >
          {#if revealing}
            <span class="action-spinner" aria-hidden="true"></span>
          {:else}<FolderOpen size={12} aria-hidden="true" />{/if}
          <span class="reveal-label">
            {revealing
              ? "Opening…"
              : revealError
  ...
  <div class="preview-body" class:media-body={isImage || isPdf}>
    {#if previewUnavailable}
      <UnavailableNote
        label="File preview"
        message="Not available here yet. Use the HQ desktop app to preview this file."
    ...
    {:else if unsupported || mediaError}
      <div class="preview-unsupported" data-testid="file-preview-unsupported">
        <FileX size={28} aria-hidden="true" />
        <strong>Can&rsquo;t preview this file</strong>
        <span
          >It&rsquo;s unsupported or too large to show here. Use the actions
          above to open it.</span
        >
      </div>
