<script lang="ts">
  ...
  import { sortV4CompaniesConnectedFirst } from "../home/model.js";
  import "../chat/tokens.css";

  interface Props {
    /** Platform files seam (`listDir`). On web this returns `unavailable`
     *  ("not-yet-implemented-api") and the tree renders its empty/error state. */
    files: FilesApi;
  ...
  const activeLabel = $derived(
    activeSlug
      ? (companyRows.find((row) => row.slug === activeSlug)?.label ??
          activeSlug)
      : null,
  );
  ...
  async function loadChildren(relPath: string): Promise<DirEntry[]> {
    ...
    const res = await files.listDir(relPath);
    if (!res.ok) {
      if (res.reason === "unavailable") {
        // Standard degraded behaviour: an empty tree, never a crash. The
        // parent view is expected to gate Files mode on availability too.
        return [];
      }
      throw new Error(res.message ?? "Could not list this folder");
    }
    return filterFileEntriesForMembership(
      res.value as unknown as DirEntry[],
      accessibleCompanies,
    );
  }
...
<aside class="files-sidebar" aria-label="Files explorer">
  ...
  <div class="fs-companies-area">
    <div class="fs-section-label" id="fs-companies-label">
      Filter by company
    </div>
