<script lang="ts">
   ...
   *    command). This component never calls the platform directly — it stays
  ...
  function withLoadedChildren(
    ...
    return nodes.map((node) => {
      ...
      if (node.isDir && node.children && path.startsWith(`${node.path}/`)) {
        return {
          ...node,
          children: withLoadedChildren(node.children, path, children),
        };
      }
      return node;
    });
  ...
  async function ensureLoaded(node: LazyNode): Promise<void> {
    ...
    try {
      ...
      roots = withLoadedChildren(
        roots,
        node.path,
        entries.map(dirEntryToLazyNode),
      );
    } catch (err) {
      console.error("list_hq_dir failed:", err);
      if (generation !== treeGeneration) return;
