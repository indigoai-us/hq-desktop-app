 ...
 */
export function fileAccessibleCompanies(
...
export function companySlugForHqPath(
  path: string | null | undefined,
): string | null {
...
function validCompanySlug(slug: string): boolean {
  ...
  return (
    normalized === `companies/${slug}` &&
    companySlugForHqPath(normalized) === slug
  );
}
...
export function isFilesRouteAllowed(
  ...
  const hasPath =
    typeof route.path === "string" && route.path.trim().length > 0;
