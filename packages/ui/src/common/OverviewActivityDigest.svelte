<script lang="ts">
   ...
   * Reuses the company activity feed (same as the Activity tab), warmed
   * through the injected company store (the desktop-alt store singleton,
   * re-expressed as a prop so this component stays platform-pure). Renders as
   * a naked section: hairline rows, no outer rounded dashboard boxes. All
   * values are real; empty/zero states stay honest.
   */

  /** Narrow slice of the company store this digest needs (cache + loader). */
  export interface CompanyActivityStore {
    /** Rune-backed revision counter bumped on cache writes/invalidations. */
    readonly revision: number;
    /** Warm cached activity payload for `slug`, if any. */
    activity(slug: string): unknown;
    /** Fetch (or refresh) the activity payload for `slug`. */
    loadActivity<T>(slug: string): Promise<T>;
  }

  interface Props {
    slug: string;
    /** Company activity cache/loader (see `CompanyActivityStore`). */
    store: CompanyActivityStore;
    cloudBacked?: boolean;
  ...
  function normalize(
    result: Partial<CompanyActivity> | null | undefined,
  ): CompanyActivity {
    ...
    return {
      ...
      sparkline: Array.isArray(result?.sparkline)
        ? result.sparkline.map(numberOrZero)
        : [],
  ...
  $effect(() => {
    ...
    const warm = store.activity(slug);
    if (warm != null) {
    ...
    void store
      .loadActivity<Partial<CompanyActivity>>(slug)
      .then((result) => {
        if (!cancelled) {
      ...
      .catch((err) => {
        console.warn(`company activity load (${slug}) failed:`, err);
      })
      .finally(() => {
  ...
  const hasActivity = $derived(
    activity.top.length > 0 ||
      activity.sparkline.some((v) => v > 0) ||
      activity.stats.edits7 > 0,
  );
    ...
    {#if activity.top.length > 0}
      <ul class="digest-list">
        {#each activity.top.slice(0, 5) as c, index (`${c.who}:${index}`)}
          <li class="digest-row">
            <span class="digest-mark" aria-hidden="true"
              >{c.who.slice(0, 1).toUpperCase()}</span
            >
            <div class="digest-copy">
              <span class="digest-title">{c.who}</span>
              <span class="digest-meta">
                {c.edits}
                {c.edits === 1 ? "edit" : "edits"} · recent window
              </span>
            </div>
