<script lang="ts">
  ...
  interface Props {
    ...
    /**
     * Open the meeting join URL in the system browser. Platform seam replacing
     * the desktop shell `open()` plugin; defaults to `window.open` on web.
     */
  ...
  const stateLabel = $derived(
    meeting ? labelForState(meeting.state) : "Standing by",
  );
  ...
  const detectedLabel = $derived(
    meeting ? relativeFromNow(meeting.detectedAt) : "",
  );
  ...
  const showCompanyPicker = $derived(
    !!meeting && !meeting.windowId.startsWith("scheduled-bot:"),
  );
    ...
    <div class="card-body">
      <div class="live-main">
        ...
        <div class="live-copy">
          ...
          <div class="live-sub">
            {platformLabel(meeting.platform) || "Meeting app"} · {stateLabel}
          </div>
        </div>
      </div>
