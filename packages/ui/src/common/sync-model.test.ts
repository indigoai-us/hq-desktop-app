...
import { CORE_SETUP_LABEL } from "./progressLabel";
...
describe("friendlySyncError", () => {
  ...
  it("maps network failures to a connection message with no detail", () => {
    ...
    expect(summary).toBe(
      "Couldn't reach the sync server — check your connection.",
    );
  ...
  it("maps auth failures to a sign-in message", () => {
    const { summary, detail } = friendlySyncError(
      "HttpError code=AUTH 401 token expired",
    );
  ...
  it("surfaces reauth, paused, cloud, manifest, and top-level error attention", () => {
    ...
    expect(attention).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          key: "reauth:broken",
          title: "Broken Co needs reconnect",
        }),
        expect.objectContaining({
          key: "paused:paused",
          title: "Paused Co is paused",
        }),
      ]),
    );
  });
