...
export type SyncState =
  "idle" | "syncing" | "error" | "conflict" | "setup-needed" | "auth-error";
...
export type SourceAction =
  "Up to date" | "Syncing" | "Reauth" | "Paused" | "Needs attention";
...
export function isPausedWorkspace(
  workspace: Workspace,
...
export function needsReauthWorkspace(
  workspace: Workspace,
  syncState: SyncState,
): boolean {
...
export function buildSourceRows(args: {
  ...
  return ordered.map((workspace) => {
    ...
    const syncing =
      syncState === "syncing" && progress?.company === workspace.slug;
    const reauth = needsReauthWorkspace(workspace, syncState);
    const sourceAttention =
      Boolean(stats.errorMessage) || stats.aborted || stats.conflicts > 0;
    const paused =
      !reauth &&
      !sourceAttention &&
    ...
    const progressPct =
      syncing && plannedFiles > 0
        ? Math.min(
            100,
            Math.max(4, (stats.progressedFiles / plannedFiles) * 100),
          )
        : syncing
          ? 18
          : null;
    ...
    const completedOrTransferred = Math.max(
...
export function buildAttentionItems(args: {
  ...
  if (syncState === "conflict" && !hasSourceConflict) {
    items.push({
      ...
      detail:
        syncErrorMessage ||
        "A sync stopped because a conflict needs attention.",
      tone: "warn",
  ...
  if (!cloudReachable) {
    items.push({
      ...
      detail:
        cloudError ||
        "Showing local workspace state until the vault is reachable again.",
      tone: "paused",
    });
  }
  ...
  for (const workspace of workspaces) {
    ...
    if (stats.errorMessage || stats.aborted || stats.conflicts > 0) {
      ...
      items.push({
        ...
        detail:
          workspace.brokenReason ||
          "Manifest and cloud membership are out of sync.",
        tone: "warn",
        actionLabel: "Open settings",
      });
      ...
      items.push({
        ...
        tone: "paused",
        actionLabel:
          workspace.state === "local-only" ? "Open settings" : undefined,
      });
    }
  }
...
export function friendlySyncError(raw: string): FriendlySyncError {
  ...
  const detail =
    truncated || cleaned.length > summaryCore.length ? cleaned : null;
  return { summary, detail };
}

export function latestFullSync(
  workspaces: Workspace[],
  ...
  const times = workspaces
    .map((workspace) =>
      workspace.lastSyncedAt ? new Date(workspace.lastSyncedAt).getTime() : 0,
    )
