...
describe("statusTone", () => {
  it("running + ready -> ok", () =>
    expect(statusTone("running", true)).toBe("ok"));
  it("running + not-ready (booting) -> idle", () =>
    expect(statusTone("running", false)).toBe("idle"));
  it("stopped -> idle", () =>
    expect(statusTone("stopped", false)).toBe("idle"));
  it("crash-loop -> warn", () =>
    expect(statusTone("crash-loop", false)).toBe("warn"));
  it("unknown -> idle", () =>
    expect(statusTone("unknown", false)).toBe("idle"));
});
...
describe("relativeTime", () => {
  ...
  it("seconds -> just now", () =>
    expect(relativeTime("2026-06-18T11:59:40Z", now)).toBe("just now"));
  it("minutes", () =>
    expect(relativeTime("2026-06-18T11:56:00Z", now)).toBe("4m ago"));
  it("hours", () =>
...
describe("shortDuration", () => {
  ...
  it("seconds", () =>
    expect(shortDuration("2026-06-18T11:59:30Z", now)).toBe("30s"));
  it("minutes", () =>
    expect(shortDuration("2026-06-18T11:48:00Z", now)).toBe("12m"));
  it("hours", () =>
    expect(shortDuration("2026-06-18T09:00:00Z", now)).toBe("3h"));
  it("days", () =>
    expect(shortDuration("2026-06-16T12:00:00Z", now)).toBe("2d"));
});
      ...
      workers: [
        {
          ...
          status: "running",
          ready: true,
          startedAt: "2026-06-18T11:48:00Z",
          updatedAt: "2026-06-18T11:59:30Z",
        },
      ],
    };
