...
import { parseWorkSessionEvent } from "./workSessionEvent.js";
...
describe("parseWorkSessionEvent", () => {
  it("parses a valid done event", () => {
    expect(parseWorkSessionEvent(DONE_SAMPLE)).toEqual(DONE_ACTIVITY);
  });

  it("parses a json-fenced done event the same as the unfenced sample", () => {
    expect(parseWorkSessionEvent("```json\n" + DONE_SAMPLE + "\n```")).toEqual(
      DONE_ACTIVITY,
    );
  });

  it("parses a bare fence with no language tag", () => {
    expect(parseWorkSessionEvent("```\n" + DONE_SAMPLE + "\n```")).toEqual(
      DONE_ACTIVITY,
    );
  });

  it("returns null for fenced non-JSON garbage", () => {
    expect(parseWorkSessionEvent("```json\nnot json at all\n```")).toBeNull();
    expect(parseWorkSessionEvent("```\nhello world\n```")).toBeNull();
  });
  ...
  it("maps start and claim to started", () => {
    expect(
      parseWorkSessionEvent(sessionBody({ kind: "start", by: "Ada" })),
    ).toMatchObject({
      kind: "start",
      actor: "Ada",
      verb: "started",
    ...
    expect(
      parseWorkSessionEvent(sessionBody({ kind: "claim", by: "Ada" })),
    ).toMatchObject({
      kind: "claim",
      actor: "Ada",
      verb: "started",
  ...
  it("maps blocked to is blocked on", () => {
    expect(
      parseWorkSessionEvent(sessionBody({ kind: "blocked", by: "Ada" })),
    ).toMatchObject({
      kind: "blocked",
      actor: "Ada",
      verb: "is blocked on",
  ...
  it("falls back to storyTitle when claim summary is empty (escaped payload)", () => {
    const body = sessionBody(
      { kind: "claim", by: "Ada", summary: "" },
      {
        payload: JSON.stringify({
          storyId: "US-001",
          storyTitle: "Fix the version menu",
  ...
  it("ignores a non-JSON payload string and still extracts storyId from threadId", () => {
    expect(
      parseWorkSessionEvent(
        sessionBody({ kind: "start", by: "Ada" }, { payload: "not-json" }),
      )?.storyId,
    ).toBe("T-002");
  });
  ...
  it("returns null for malformed or truncated JSON without throwing", () => {
    expect(
      parseWorkSessionEvent('{"v":1,"kind":"work-session-event"'),
    ).toBeNull();
    expect(parseWorkSessionEvent("{")).toBeNull();
  ...
  it("returns null when event is missing", () => {
    expect(
      parseWorkSessionEvent(
        JSON.stringify({ kind: "work-session-event", threadId: "x:T-002" }),
      ),
    ).toBeNull();
  });
  ...
  it("returns null for unknown event kinds", () => {
    expect(
      parseWorkSessionEvent(sessionBody({ kind: "pause", by: "Ada" })),
    ).toBeNull();
  });
