...
export interface Channel {
  ...
  /** US-009 channel fabric: true when an unread badge-driving message mentions
   * the caller. Carried from the server-shaped directory row; absent on the
   * legacy list payload. */
  mentionFlag?: boolean;
...
export function companyNameFor(
  c: Channel,
  ...
  if (uid) {
    const fromList = companies
      .find((co) => co.companyUid === uid)
      ?.companyName?.trim();
...
export function groupChannels(
  ...
  const byName = (a: Channel, b: Channel): number =>
    channelDisplayName(a)
      .toLowerCase()
      .localeCompare(channelDisplayName(b).toLowerCase());

  const groupDms = channels
    .filter((c) => c.scope === "group")
    .slice()
    .sort(byName);
  ...
  const remaining = [...companyBuckets.keys()]
    ...
    .sort((a, b) =>
      labelFor(a, companyBuckets.get(a)?.[0]).localeCompare(
        labelFor(b, companyBuckets.get(b)?.[0]),
      ),
    );
...
export function shouldBumpChannelUnread(args: {
  ...
  if (selfUid && fromPersonUid && selfUid === fromPersonUid) return false;
  const selected = (args.selectedId ?? "").trim();
  if (!selected) return true;
  return selected !== `ch:${channelId}` && selected !== channelId;
}

/**
 * Directory unread is authoritative when the snapshot is at least as new as
 * the local row. A missing unreadCount, or a stale snapshot with unread 0,
 * must not wipe a newer local wake bump.
 */
export function mergeDirectoryUnread(args: {
  incomingUnread?: number | null;
  incomingActivityAt?: string | null;
  prevUnread?: number | null;
  prevActivityAt?: string | null;
}): number {
  const incoming =
    typeof args.incomingUnread === "number" &&
    Number.isFinite(args.incomingUnread)
      ? Math.max(0, Math.floor(args.incomingUnread))
      : null;
  const prev = Math.max(0, args.prevUnread ?? 0);
  if (incoming == null) return prev;
  const incomingAt = (args.incomingActivityAt ?? "").trim();
  const prevAt = (args.prevActivityAt ?? "").trim();
  if (prev > incoming && prevAt && incomingAt && prevAt > incomingAt) {
    return prev;
  }
  return incoming;
}
...
/** Targeted sidebar patch for one channel wake — one row, no directory refetch. */
export function applyChannelMessageWake(
  list: Channel[],
  args: {
    channelId: string;
    createdAt?: string;
  ...
  const next = list.map((c) => {
    if (c.channelId !== args.channelId) return c;
    const unread =
      typeof args.unread === "number" && Number.isFinite(args.unread)
        ? Math.max(0, Math.floor(args.unread))
        : Math.max(0, (c.unread ?? 0) + (args.unreadDelta ?? 0));
    const stamp =
      args.createdAt && (!c.lastMessageAt || args.createdAt > c.lastMessageAt)
        ? args.createdAt
        : c.lastMessageAt;
    if (
      unread === (c.unread ?? 0) &&
      stamp === c.lastMessageAt &&
      stamp === c.lastActivityAt
    ) {
      return c;
    }
    changed = true;
    return {
      ...c,
      unread,
      lastMessageAt: stamp ?? c.lastMessageAt,
      lastActivityAt: stamp ?? c.lastActivityAt,
    };
  });
...
export function clearChannelUnread(
  list: Channel[],
  channelId: string,
): Channel[] {
