...
function companyUid(company: CompanyLabelSource): string | null {
  return (
    company.uid?.trim() ||
    company.companyUid?.trim() ||
    company.cloudUid?.trim() ||
    null
  );
}
...
export function sanitizeVisibleIdentifiers(
  ...
  const companies = [...(options.companies ?? [])]
    .map((company) => ({
      uid: companyUid(company),
      label: humanCompanyLabel(company),
    }))
    .filter((company): company is { uid: string; label: string } =>
      Boolean(company.uid),
    )
  ...
  return visible
    .replace(RAW_PERSON_UID, personLabel)
    .replace(RAW_COMPANY_UID, "Company");
}
