...
// ── Creatable workspaces ─────────────────────────────────────────────────────

/**
 * The subset of a Workspace this module needs to decide whether a new channel
 * can be created in it. Structural on purpose so the rule stays unit-testable
 * without importing the full Tauri-shaped Workspace.
 */
export interface CreatableWorkspaceInput {
  slug: string;
 ...
 * companies you can only look at (a stale manifest entry, a company whose
 * membership was revoked, a `broken` cloud_uid mismatch). Offering those as
 * create targets produced the "In: <some company> → You're not an active
 * member of that workspace" dead end: the server is the only thing that
 * rejected it, and only after the user had filled the whole form in.
 *
 * Fails OPEN on unknown membership: when `membershipStatus` is absent we keep
 * the workspace. That field is enriched from the cloud, so a transient outage
 * or an older cache leaves it null for companies the user is genuinely a
 * member of — dropping those would hide the user's real workspaces, which is
 * worse than the server error we already surface. We exclude only on positive
 * evidence: a membership that is present and not active, or a `broken` state.
 */
export function creatableCompanies(
  workspaces: readonly CreatableWorkspaceInput[],
): ScopeCompany[] {
  const out: ScopeCompany[] = [];
  const seen = new Set<string>();
  for (const w of workspaces) {
    if (w.kind === "personal") continue;
    if (!w.cloudUid) continue;
    if (w.state === "broken") continue;
    const status = w.membershipStatus?.trim().toLowerCase();
    if (status && status !== "active") continue;
    if (seen.has(w.cloudUid)) continue;
    seen.add(w.cloudUid);
    out.push({
      companyUid: w.cloudUid,
