...
import type { WorkProvenance } from "../common/provenance.js";
...
export type ProjectListStatus =
  "live" | "in-progress" | "complete" | "pending" | "archived";

/** The status-filter pills shown above the Board project list. */
export type StatusFilter =
  "all" | "active" | "in-progress" | "complete" | "archived";
...
export function matchesStatusFilter(
  ...
  switch (filter) {
    ...
    case "active":
      ...
      return (
        status === "live" || status === "in-progress" || status === "pending"
      );
    case "in-progress":
...
export function projectRecencyTime(
  ...
  return (
    projectTimestamp(project.updatedAt) || projectTimestamp(project.createdAt)
  );
}
...
export const EDITABLE_PROJECT_STATUS_LABEL: Record<
 ...
 * `planned`.
 */
export function toEditableStatus(
...
export function projectCompanies(projects: Project[]): string[] {
  return [
    ...new Set(projects.map((project) => project.company).filter(Boolean)),
  ].sort((a, b) => a.localeCompare(b));
}
...
export type PortfolioColumn =
  "not-started" | "in-progress" | "active" | "complete";
...
export function liveSessionsForProject(
  ...
  return sessions.filter(
    (session) =>
      isPortfolioLiveStatus(session.status) &&
      sessionMatchesProject(session, project),
  );
}
...
export function portfolioColumn(
  ...
  if (
    ...
    raw === "complete" ||
    raw === "done" ||
    deriveProjectState(project.storiesComplete, project.storiesTotal) ===
      "complete"
  ) {
    return "complete";
  }
  ...
  const rollup = deriveProjectState(
...
export function projectLiveRunView(
  ...
  const sorted = live.slice().sort((a, b) => {
    const aT =
      Date.parse(a.lastActivityAt ?? "") || Date.parse(a.startedAt ?? "") || 0;
    const bT =
      Date.parse(b.lastActivityAt ?? "") || Date.parse(b.startedAt ?? "") || 0;
    return bT - aT;
  });
...
export function liveSessionsForStory(
  ...
  return sessions.filter(
    (session) =>
      isPortfolioLiveStatus(session.status) &&
      sessionMatchesStory(session, story),
  );
}
...
export function taskStateContext(
  ...
  switch (column) {
    case "not-started": {
      ...
      const hasUnmet = (story.dependsOn ?? []).some(
        (depId) => !completedIds.has(depId),
      );
      return hasUnmet
        ? "Waiting on dependencies"
        : "No run expected before work begins";
    }
    case "in-progress":
 ...
 */
export function projectFilesRootFromPrdPath(
