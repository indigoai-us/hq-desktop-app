...
import type { Project, Story } from "./projects-model.js";
import {
  hasProvenance,
  mergeProvenance,
  normalizeProvenance,
...
// ---------------------------------------------------------------------------
// Platform seam (desktop-alt port). The Tauri invoke calls are replaced by
// an injected `adapter.projects` slice (configureProjectsApi), following the
// chat agency-store injection pattern. Loader/saver functions keep their
// original throw-on-failure contract: an `{ok:false, reason:"error"}` result
// throws an Error (optimistic-rollback signal), and an
// `{ok:false, reason:"unavailable"}` result (web: needs-new-API) throws
// `ProjectsUnavailableError` so pages render the standard degraded state.
// ---------------------------------------------------------------------------

/** Thrown when the platform has no local projects backend (web). */
export class ProjectsUnavailableError extends Error {
  constructor(message = "Projects are not available on this platform yet.") {
    super(message);
...
/** Inject the platform backend (adapter.projects) before any loader runs. */
export function configureProjectsApi(next: ProjectsApi | null): void {
  api = next;
}

function requireApi(): ProjectsApi {
  if (!api) throw new ProjectsUnavailableError();
  return api;
}

/** Unwrap an AdapterResult into the legacy throw-on-failure contract. */
function unwrap<T>(result: {
  ...
  if (result.ok) return (result as { ok: true; value: T }).value;
  if (result.reason === "unavailable") {
    throw new ProjectsUnavailableError(result.message);
  }
  throw new Error(result.message ?? "Projects backend call failed");
}
...
export function coercePriority(
...
export async function loadCompanyProjectProvenance(
  ...
  const response: unknown = unwrap(await requireApi().getProjectCreators(slug));
...
export async function loadLocalProjects(): Promise<Project[]> {
  const response: unknown = unwrap(await requireApi().listProjects());
...
function toObjective(wire: Partial<Objective>): Objective {
  return {
    ...
    linearInitiativeId:
      typeof wire.linearInitiativeId === "string"
        ? wire.linearInitiativeId
        : null,
  };
}
...
export async function loadCompanyGoals(slug: string): Promise<CompanyGoals> {
  const wire = unwrap(await requireApi().getGoals(slug)) as CompanyGoalsWire;
  return {
...
export async function loadLocalProjectStories(
  ...
  const prd = unwrap(
    await requireApi().getPrd(prdPath),
  ) as unknown as LocalProjectPrdWire;
...
export async function loadLocalProjectPrd(
  prdPath: string,
): Promise<LocalProjectPrd> {
  const wire = unwrap(
    await requireApi().getPrd(prdPath),
  ) as unknown as LocalProjectPrdWire;
  return {
 ...
 */
export async function loadLocalProjectReadme(
  prdPath: string,
): Promise<string | null> {
  const content = unwrap(await requireApi().getReadme(prdPath));
  return content || null;
}
...
export async function saveLocalProjectStatus(
  ...
  // The adapter surface takes an opaque project ref string + status. Encode the
  // full board-scoped identity (boardPath / projectId / prdPath) as JSON so the
  // platform layer can disambiguate legacy same-ID board entries.
  unwrap(
    await requireApi().setProjectStatus(
      JSON.stringify({ boardPath, projectId, prdPath }),
      status,
    ),
  );
}
...
export async function saveLocalStoryPasses(
  ...
  unwrap(await requireApi().setStoryPasses(prdPath, storyId, passes));
}
