<script lang="ts">
  ...
  import { labelColor, type Story } from "./projects-model.js";
  ...
  interface Props {
    ...
    /** Platform settings seam (adapter.settings) for the HQ root lookup. */
    settings?: SettingsApi | null;
    /** Platform shell seam (adapter.shell); desktop-only (canLaunchApps).
     *  Absent → per-file "Open in Claude Code" affordances suppress. */
    shell?: ShellApi | null;
  }
  ...
  $effect(() => {
    let cancelled = false;
    if (settings) {
      void settings.getConfig().then((result) => {
        if (cancelled) return;
        if (result.ok) {
          hqFolderPath =
          ...
          console.error("StoryDetailPanel get_config failed:", result.message);
        }
      });
    }
    return () => {
    ...
    <header class="detail-header">
      <div class="header-text">
        ...
        <div class="status-line">
          ...
          {#if activityLive}
            <span
              class="activity-inline is-active"
              data-testid="agent-activity-live"
            >
