<script lang="ts">
  ...
  // Wire the module-level projects seam to this platform adapter.
  $effect.pre(() => {
  ...
  /** Web (needs-new-API): goals backend unavailable → standard degraded state. */
  let goalsUnavailable = $state(false);
  ...
  const linkedProjectCount = $derived.by(() => {
    ...
    for (const objective of objectives) {
      for (const project of linkedProjects(objective))
        ids.add(projectIdentity(project));
    }
    return ids.size;
  });
  const targetQuarter = $derived.by(() => {
    const value = objectives
      .map((objective) => quarterLabel(objective.timeframe))
      .find(Boolean);
  ...
  const selectedGoal = $derived.by(() => {
    ...
    return (
      objectives.find((objective) => goalKey(objective) === selectedGoalId) ??
      null
    );
  });

  const selectedLinked = $derived(
    selectedGoal ? linkedProjects(selectedGoal) : [],
  );
  ...
  function reviewProposal(objective: Objective) {
    const prompt = [
      ...
      objective.description
        ? `Goal description: ${objective.description}`
        : null,
...
<section
  class="company-goals"
  aria-labelledby="company-goals-title"
  data-testid="company-goals-page"
>
  {#if selected}
    <ProjectDetailView
      {adapter}
  ...
  {:else}
    <header class="goals-header">
      <div class="goals-heading title-stack">
        ...
        <span class="goals-sub">
          {objectives.length}
          {objectives.length === 1 ? "goal" : "goals"} · {targetQuarter}
          · linked to {linkedProjectCount}
          {linkedProjectCount === 1 ? "project" : "projects"}
        </span>
      </div>
    ...
    {#if error}
      <div class="goals-error" role="alert" data-testid="goals-error">
        {error}
      </div>
    {/if}
    ...
    {#if goalsUnavailable}
      <UnavailableNote
        label="Goals"
        message="Company goals aren't available in this app yet. Use the HQ desktop app to browse goals."
    ...
    {:else}
      ...
      >
        <aside class="list-pane goals-list-pane" data-testid="goals-list-pane">
          ...
          >
            {#each objectives as objective (goalKey(objective))}
              ...
              >
                <span class={`status-dot ${status.tone}`} aria-hidden="true"
                ></span>
                <span class="goal-row-copy title-stack">
                  <span class="goal-row-title"
                    >{objective.title || "Untitled goal"}</span
                  >
        ...
        <div
          class="detail-pane goal-detail-pane"
          data-testid="goal-detail-pane"
        >
                ...
                <div class="goal-detail-heading title-stack">
                  <div class="goal-title-row">
                    ...
                    <span class="goal-status" data-testid="goal-detail-status">
                      <span
                        class={`status-dot ${selectedStatus.tone}`}
                        aria-hidden="true"
                      ></span>
                    ...
                    {:else}
                      {#each selectedGoal.keyResults as kr, index (kr.id || `${selectedGoal.id}-${index}`)}
                        ...
                        <tr data-testid="kr-row">
                          ...
                          <td class="kr-value">
                            {formatValue(kr.current, kr.unit)} → {formatValue(
                              kr.target,
                              kr.unit,
                            )}
                          </td>
                  ...
                  {:else}
                    ...
                    {#if overflowCount(selectedLinked) > 0}
                      <span class="muted-chip"
                        >+{overflowCount(selectedLinked)}</span
                      >
                    {/if}
                  {/if}
                </div>
          ...
          {:else}
            <div class="goal-detail-empty" data-testid="goal-detail-empty">
              ...
              <p>
                Choose a goal from the list to inspect key results and linked
                projects.
              </p>
            </div>
          {/if}
    ...
    <p class="goals-footnote">
      Goals are objectives with measurable key results. Projects ladder up to
      goals; progress rolls up automatically.
    </p>
  {/if}
</section>
