<script lang="ts">
  ...
  const prdDescription = $derived(
    prd?.description?.trim() || project.description,
  );
  ...
  $effect(() => {
    ...
    statusOverride =
      cachedStatus === null ? null : toEditableStatus(cachedStatus);
    statusOpen = false;
    statusError = null;
  ...
  $effect(() => {
    let cancelled = false;
    void adapter.settings.getConfig().then((result) => {
      if (cancelled) return;
      if (result.ok) {
        hqFolderPath =
          typeof result.value?.hqFolderPath === "string"
            ? result.value.hqFolderPath
            : "";
      } else {
        if (result.reason === "error") {
          console.error("ProjectDetailView get_config failed:", result.message);
        }
        hqFolderPath = "";
      }
    });
  ...
  async function openProjectInClaude() {
    ...
    const prompt = [
      ...
      .join("\n");
    try {
      // Desktop-only affordance (canLaunchApps): fall back to copying the
      // prompt on hosts that cannot launch external apps.
      if (!adapter.isAvailable("canLaunchApps")) {
        await navigator.clipboard.writeText(prompt);
        claudeMessage = "Prompt copied — paste it into Claude Code.";
        return;
      }
...
>
    ...
    {#if hasPrd && kpi.total > 0}
      <div class="kpi-strip" aria-label="Project metrics">
        ...
        <div class="kpi-tile">
          ...
          <span class="kpi-value" class:is-zero={kpi.active === 0}
            >{kpi.active}</span
          >
        </div>
        <div class="kpi-tile">
  ...
  <div class="workspace-body" data-testid="project-workspace-body">
    {#if selectedStory && tab === "tasks"}
      ...
      <div class="task-workspace" data-testid="project-task-workspace">
        <aside
          ...
          >
            {#each TASK_COLUMNS as column (column)}
              ...
              {#if columnStories.length > 0}
                <div
                  class="task-rail-section"
                  data-testid={`task-rail-section-${column}`}
                >
                  ...
                  {#each columnStories as item (item.story.id)}
                    ...
                    >
                      <span class="task-rail-id"
                        >{shortStoryId(item.story.id)}</span
                      >
        ...
        {#if tab === "overview"}
          <div class="overview detail-layout" data-testid="detail-overview">
            <main class="detail-main">
              ...
              {#if linkedGoal}
                <section
                  class="info-card key-results-card"
                  data-testid="detail-key-results"
                >
                  <h2>Key results</h2>
                  ...
                  {:else}
                    <div class="kr-list">
                      {#each keyResults as kr, index (kr.id || index)}
                        {@const progressValue = krProgress(
                          kr.current,
                          kr.target,
                        )}
                        <div class="kr-row">
          ...
          <div class="board-tab tasks-tab" data-testid="detail-board">
            ...
            {:else if !hasPrd}
              <div class="drill-empty">
                <p>
                  This project has no linked PRD yet, so there are no tasks to
                  show.
                </p>
              </div>
            {:else}
            ...
            <section class="activity-panel" aria-label="Project activity">
              <header class="activity-head">
                ...
                <span class="muted-note">
                  Live sessions matched to this project · contextual, not a
                  global dashboard
                </span>
              </header>
                  ...
                  {#each projectSessions as session (`${session.project}:${session.startedAt ?? session.cwd}:${session.status}`)}
                    <li class="session-row">
                      <div class="session-main">
                        ...
                        <strong class="session-project"
                          >{session.project || project.id}</strong
                        >
                        {#if session.model}
