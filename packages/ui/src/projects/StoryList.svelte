    ...
    {#each classified as item (item.story.id)}
      ...
      >
        ...
        <div class="story-main">
          <span class="story-title" title={item.story.title}
            >{item.story.title}</span
          >
          {#if labels.length > 0}
            <div class="row-labels">
              ...
              {/each}
              {#if overflow > 0}
                <span class="label-overflow" title={`${overflow} more`}
                  >+{overflow}</span
                >
              {/if}
            </div>
        ...
        <span class="state-badge" data-column={item.column}
          >{columnOf(item.column)}</span
        >
        {#if prio}
