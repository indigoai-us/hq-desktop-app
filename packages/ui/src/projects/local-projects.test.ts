...
// desktop-alt port: the Tauri invoke mock is replaced by a fake adapter
// slice that preserves the legacy (command, args) assertion shape.
const invokeMock = vi.fn();
configureProjectsApi(fakeProjectsApi(invokeMock));
...
describe("local project identity", () => {
  ...
  it("keeps same-company projects with repeated ids when their PRD paths differ", () => {
    ...
    expect(
      new Set([projectIdentity(first), projectIdentity(second)]).size,
    ).toBe(2);
  });
  ...
  it("falls back to trimmed company and id when a project has no PRD path", () => {
    expect(
      projectIdentity(
        project({ company: " indigo ", id: " hq-desktop-app ", prdPath: "" }),
      ),
  ...
  it.each([
    ...
    "treats %s from get_local_projects as an empty project list",
    async (_label, value) => {
...
describe("local/cloud provenance adapter", () => {
  ...
  it("normalizes project-level PRD provenance for detail fallback", async () => {
    ...
    await expect(
      loadLocalProjectPrd("companies/indigo/projects/launch/prd.json"),
