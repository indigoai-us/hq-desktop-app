<script lang="ts">
  ...
  import { projectIdentity } from "./local-projects.js";
  import {
  ...
  const searched = $derived(
    filterProjectsByQuery(statusFiltered, debouncedQuery),
  );
    ...
    {:else if !hasProjects && !error}
      <div class="list-empty">
        ...
        <p class="empty-detail">
          Projects are registered in each company’s board.json.
        </p>
      </div>
    {:else if noResults}
      <div class="list-empty">
    ...
    {:else}
      {#each sections as section (section.key)}
        {@const renderWindow = progressiveWindow(
          section.projects,
          visibleBySection[section.key] ?? PROJECT_RENDER_BATCH,
          PROJECT_RENDER_BATCH,
        )}
        <div class="project-section">
          ...
          >
            <span
              class="chevron"
              class:is-open={!collapsed[section.key]}
