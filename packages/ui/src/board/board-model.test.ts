...
describe("board-model (US-006 Board tab)", () => {
  ...
  describe("deriveBoardColumns", () => {
    ...
    it("maps shipped signal → DONE even when passes is false", () => {
      ...
      expect(
        columns.find((c) => c.id === "done")?.cards.map((c) => c.storyId),
      ).toEqual(["US-002"]);
    });
    ...
    it("maps PR open / reviewer → REVIEW", () => {
      ...
      expect(review?.cards.map((c) => c.storyId).sort()).toEqual([
        "US-003",
        "US-004",
      ]);
      expect(
        review?.cards.find((c) => c.storyId === "US-003")?.statusLine,
      ).toBe("PR OPEN · CI GREEN");
      expect(
        review?.cards.find((c) => c.storyId === "US-004")?.statusLine,
      ).toBe("ADA REVIEWING");
    });
    ...
    it("maps live agent + queued → IN PROGRESS (live first)", () => {
      ...
      expect(inProgress?.title).toBe("DOING");
      expect(inProgress?.cards.map((c) => c.storyId)).toEqual([
        "US-006",
        "US-005",
      ]);
    ...
    it("moves a card to DONE when pass-state flips true", () => {
      ...
      expect(
        after.find((c) => c.id === "done")?.cards.map((c) => c.storyId),
      ).toEqual(["US-010"]);
      expect(after.find((c) => c.id === "done")?.cards[0]?.statusLine).toBe(
        "SHIPPED",
      );
    });
  });
  ...
  describe("buildStatusLine vocabulary", () => {
    ...
    it("emits reviewer, PR/CI, SHIPPED, and QUEUED forms", () => {
      ...
      expect(
        buildStatusLine({ signal: { prOpen: true, ciGreen: false } }),
      ).toBe("PR OPEN · CI RED");
  ...
  describe("buildStoryPanelModel", () => {
    ...
    it("supports partial AC fixtures (2 of 4 struck) and review vocabulary", () => {
      ...
      expect(buildStatusLine({ signal: { reviewer: "Design" } })).toBe(
        "DESIGN REVIEW",
      );
      expect(buildStatusLine({ signal: { prOpen: true, ciGreen: true } })).toBe(
        "PR OPEN · CI GREEN",
