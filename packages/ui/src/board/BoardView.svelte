<script lang="ts">
   ...
   * sessions, with a story detail side panel. Platform-pure: all data
   * arrives via props; the host owns loading, wakes and polling (see
   * work-session-feed.ts) — the desktop's invoke/poll loop stays outside.
   */
  import {
    buildStoryActivity,
  ...
  interface Props {
    ...
    signals?: Record<string, BoardStorySignal> | null;
    branchName?: string | null;
    prdName?: string | null;
    /** Channel/thread system-event messages — feed the story activity list. */
  ...
  const columns = $derived(
    deriveBoardColumns(stories, sessions, signals, project),
  );
  ...
  const panelModel = $derived.by((): StoryPanelModel | null => {
    ...
    const signal = signals?.[key] ?? signals?.[key.toUpperCase()] ?? null;
  ...
  const activity = $derived.by((): StoryActivityItem[] =>
    selectedStoryId ? buildStoryActivity(messages, selectedStoryId) : [],
  );
...
>
  ...
  {:else if stories.length === 0}
    <div class="board-empty" role="status">No tasks yet.</div>
  {:else}
      ...
      {:else}
        {#each column.cards as card (card.storyId)}
          ...
          >
            ...
            <span
              class="card-status"
              class:ok={/SHIPPED|AGENT RUNNING|CI GREEN|GREEN/.test(
                card.statusLine,
              )}
