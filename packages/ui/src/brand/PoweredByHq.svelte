<script lang="ts">
  /**
   * Permanent "powered by" + HQ lettermark lockup (US-005).
   *
   * Renders the literal text "powered by" followed by the HQ lettermark SVG
   * (never the word "HQ" as text). Part of the branded logo-slot component;
   * not removable when branding is active.
   */
  interface Props {
    /** Accessible label for the lettermark. */
    markLabel?: string;
  }

  let { markLabel = 'HQ' }: Props = $props();
</script>

<span class="powered-by-hq" data-testid="powered-by-hq">
...
<style>
  .powered-by-hq {
    ...
    align-items: center;
    gap: 5px;
    color: var(--v4-text-3, var(--popover-text-muted, #5d5d63));
    font-size: 9.5px;
    font-weight: 400;
    line-height: 1;
