...
export type HistoryEventKind =
  "dispatched" | "completed" | "failed" | "checkpoint" | "handoff";
...
const KIND_MATCHERS: ReadonlyArray<{ kind: SessionKind; test: RegExp }> = [
  ...
  {
    kind: "pr",
    test: /\bpr\b|pull-request|babysit|land-batch|land-pr|review-pr/,
  },
  // CI watchers — distinct from deploy; checked before deploy so "ci" wins.
  {
    kind: "ci",
    test: /\bci\b|workflow-run|gh-actions|github-actions|build-watch/,
  },
  // Deploy / release monitors.
  { kind: "deploy", test: /deploy|release-monitor|ship-monitor|rollout/ },
  // Signup heartbeat / health pings.
  {
    kind: "signup-heartbeat",
    test: /signup-heartbeat|heartbeat|signup-watch|health-?ping/,
  },
...
export function groupSessions(sessions: AgentSession[]): SessionGroup[] {
  ...
  for (const session of sessions) {
    ...
    if (
      activityMillis(session.lastActivityAt) >
      activityMillis(group.freshestActivityAt)
    ) {
  ...
  for (const group of groups) {
    ...
    group.sessions.sort(
      (a, b) =>
        activityMillis(b.lastActivityAt) - activityMillis(a.lastActivityAt),
    );
  }
  ...
  groups.sort((a, b) => {
    ...
    const freshDelta =
      activityMillis(b.freshestActivityAt) -
      activityMillis(a.freshestActivityAt);
...
export const HISTORY_TOOL_FILTERS: ReadonlyArray<{
...
export function sortHistoryNewestFirst(events: HistoryEvent[]): HistoryEvent[] {
  return [...events].sort(
    (a, b) => eventMillis(b.timestamp) - eventMillis(a.timestamp),
  );
}
...
export function filterHistory(
  ...
  const matched = events.filter((event) => {
    if (filter.tool !== "all" && deriveEventTool(event) !== filter.tool)
      return false;
