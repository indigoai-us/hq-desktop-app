<script lang="ts">
  ...
  interface Props {
    /** Platform seam (adapter.sessions slice). Omit to reuse a prior config. */
  ...
  onMount(() => {
    ...
    if (api) configureSessionsApi(api);
    startSessionsStore();
  ...
  ): Array<{ status: SessionStatus; count: number }> {
...
{#snippet groupBlock(
  ...
  <div class="ls-group" role="listitem">
    ...
    >
      ...
      <span class="ls-fresh"
        >{relativeActivity(group.freshestActivityAt, now)}</span
      >
    </button>
      ...
      <div class="ls-rows">
        {#each shown as session (session.id)}
          <div class="ls-row" class:dim={isDimmed(session.status)}>
            ...
            <span class="ls-time"
              >{relativeActivity(session.lastActivityAt, now)}</span
            >
          </div>
        {/each}
        {#if overflow > 0}
...
<section class="ls" aria-label="Live sessions">
  <header class="ls-head">
    ...
    <div class="ls-head-meta">
      <span class="ls-count"
        >{totalActive} across {typeCount}
        {typeCount === 1 ? "type" : "types"}</span
      >
      <span class="ls-group-ctl" aria-hidden="true">Group: Type ▾</span>
      <span
        class="ls-besteffort"
        title="Liveness is sampled from on-disk activity, not a live connection."
      >
  ...
  {#if sessionsStore.unavailable}
    <!-- Standard degraded state: local agent sessions are desktop-only. -->
    <div class="ls-empty" data-testid="sessions-unavailable" role="status">
      <p class="ls-empty-title">Not available here</p>
      <p class="ls-empty-help">
        Live agent sessions run on the desktop app. Open HQ on your Mac to see
        the fleet.
      </p>
    </div>
  {:else if sessionsStore.loading}
          ...
          >
            ...
            <div class="ls-outpost-stats">
              <span class="ls-outpost-stat">
                ...
                <span class="ls-outpost-stat-value"
                  >{outpostCard.runtimeLabel}</span
                >
              </span>
              <span class="ls-outpost-stat">
                <span class="ls-outpost-stat-label">RELAY</span>
                <span
                  class="ls-outpost-stat-value"
                  class:ok={outpostCard.relayConnected}
                  class:bad={!outpostCard.relayConnected}
                >
              ...
              <span class="ls-outpost-stat">
                ...
                <span class="ls-outpost-stat-value mono">
                  {relativeActivity(
                    sessionsStore.outpost?.lastSeenAt ?? "",
                    now,
                  )}
                </span>
              </span>
