<script lang="ts">
  ...
  interface Props {
    /** Platform backend seam (web: /v1/notify/* via the platform adapter). */
    api: NotificationsApi;
    onback?: () => void;
    /** Fires whenever store unreadCount changes (drives titlebar badge). */
    onunreadchange?: (count: number) => void;
    /**
     * Wake-driven refresh trigger: bump this counter whenever the mesh
     * delivers a notifications wake; the feed re-fetches from REST
     * (wake-to-REST-reconcile — payloads are never applied as state).
     */
    wakeSeq?: number;
    /** Open the destination for a row (DM thread, Library for shares). */
  ...
  // Parent badge: only after the first fetch, so the empty initial state
  // does not clear/seed the titlebar dot.
  $effect(() => {
    if (!feedReady) return;
    const n = unreadCount;
  ...
  async function loadFeed(unreadOnly: boolean): Promise<void> {
    ...
    try {
      ...
      if (kind === "unsupported") {
        ...
        // Never surface a raw upstream error string (e.g.
        // "[UPSTREAM_UNREACHABLE] Upstream request failed") to the user —
        // the technical detail is in console.error above. Show friendly copy.
        listError =
          kind === "auth" && !signedIn
  ...
  async function handleRowClick(row: NotificationItem): Promise<void> {
    if (row.status === "unread") {
      feedState = reduceAck(feedState, row.id);
      void api.ackNotification(row.id).catch((err) => {
                      ...
                      >
                        {#each row.actionButtons as btn (btn.id)}
                          <button
                            ...
                            disabled={row.actionUsed ||
                              actionPendingId === row.id}
                            onclick={(e) => {
                              e.stopPropagation();
                              void handleAction(
                                row.id,
