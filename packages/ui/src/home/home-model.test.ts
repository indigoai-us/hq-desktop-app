...
import { emptyWorkspaceStats } from "../common/sync-model.js";
import type { Project } from "../projects/projects-model.js";
import type { MeetingEvent } from "../meetings/meetings-model.js";
import {
  activityFileVerb,
...
function coreState(
  overrides: Partial<HomeCoreState["driftReport"]> = {},
): HomeCoreState {
...
describe("US-003 Home meta line", () => {
  ...
  it("syncing: start time and auto-sync note", () => {
    ...
    expect(line).toBe(
      "sync in progress · started 11:32 AM · auto-sync on · HQ v15.0.15",
    );
  });
  ...
  it("error: pause + retry note in the meta line", () => {
    ...
    expect(line).toBe(
      "auto-sync paused after failure · retries on the next change · HQ v15.0.15",
    );
  });
...
describe("US-003 NEEDS YOU queue", () => {
  ...
  it("resolving conflicts disable the resolution actions", () => {
    ...
    expect(
      card.actions.find((action) => action.id === "keep-local")?.disabled,
    ).toBe(true);
    expect(
      card.actions.find((action) => action.id === "keep-remote")?.disabled,
    ).toBe(true);
  });
    ...
    const card = getDriftCardModel(
      coreState({
        ...
        modified: [
          ...
          {
            path: "core/scripts/reindex.sh",
            size: 1,
            gitShaLocal: "c",
            gitShaUpstream: "d",
          },
          {
            path: "core/docs/INDEX.md",
            size: 1,
            gitShaLocal: "e",
            gitShaUpstream: "f",
          },
  ...
  it("invite card offers Accept for a pending company membership", () => {
    ...
    expect(card.actions.map((a) => [a.id, a.label])).toEqual([
...
describe("safe-delete notices", () => {
  ...
  it("explains legacy journal protection as a safe reevaluation", () => {
    ...
    expect(copy.sub).toContain(
      "could not yet verify that the remote copy was unchanged",
    );
...
describe("US-003 syncing progress card", () => {
  ...
  it("renders the file counts, per-company fanout rows, and the queued row", () => {
    const model = getHomeProgressModel({
      ...
      progress: {
        company: "indigo",
        path: "policies/indigo-hq-slack-channel.md",
        bytes: 1,
      },
      companies,
      statsBySlug,
...
describe("US-003 error card", () => {
  it("auth expiry gets positive resume language, Sign in again, and technical details", () => {
    ...
    expect(model?.sub).toBe(
      "Your files are safe. Sign in once and HQ will resume automatically.",
    );
  ...
  it("non-auth failures offer Retry only and keep the raw message in details", () => {
    ...
    expect(model?.title).toBe(
      "Sync failed — Couldn't reach the sync server — check your connection.",
    );
...
describe("US-003 actor-grouped digest", () => {
  it("verb lanes map deleted/new/updated to DEL/ADD/UPD", () => {
    ...
    expect(activityFileVerb(entry({ direction: "down", isNew: true }))).toBe(
      "ADD",
    );
  ...
  it("attributes uploads to You and falls back to the company for anonymous downloads", () => {
    ...
    expect(groups.map((group) => group.actor).sort()).toEqual([
      "Indigo",
      "You",
    ]);
  });
...
function meeting(
  overrides: Partial<MeetingEvent> & { startISO?: string } = {},
): MeetingEvent {
...
describe("getHomeCompanyRows", () => {
  it("rolls up local projects/stories per company and reads role + last change from the workspace", () => {
    ...
    expect(indigo.stories).toBe("6 / 10 tasks"); // both projects' tasks roll up
...
describe("getHomeTodayAgenda", () => {
    ...
    const agenda = getHomeTodayAgenda({
      events: [
        meeting({
          id: "late",
          summary: "Standup",
          startISO: "2026-06-15T16:00:00",
          sourceCompanyUid: "cmp_indigo",
        }),
        meeting({
          id: "early",
          summary: "Kickoff",
          startISO: "2026-06-15T09:30:00",
        }),
        meeting({
          id: "tomorrow",
          summary: "Later",
          startISO: "2026-06-16T09:00:00",
        }),
      ],
  ...
  it("falls back to a placeholder title and caps the list", () => {
    const events = Array.from({ length: 9 }, (_, i) =>
      meeting({
        id: `e${i}`,
        startISO: `2026-06-15T0${i}:00:00`,
        summary: i === 0 ? undefined : `M${i}`,
      }),
    );
    const agenda = getHomeTodayAgenda({
      events,
