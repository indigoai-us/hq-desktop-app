<script lang="ts">
  ...
  import { setStoryPasses } from "../projects/projects-store.svelte.js";
  import {
    projectDisplayName,
    storyLiveRunView,
  ...
  interface Props {
    /** Platform seam — settings/shell slices + capability flags. May be null
     *  in pure-markup mounts; desktop-only affordances then suppress. */
      ...
      void adapter.settings.getConfig().then((result) => {
        ...
        if (result.ok) {
          ...
          if (result.reason === "error") {
            console.error("StoryPanel get_config failed:", result.message);
          }
          hqFolderPath = "";
        }
      });
  ...
  const progress = $derived(
    acItems.length > 0 ? (acComplete / acItems.length) * 100 : 0,
  );
  ...
  async function openPrd() {
    ...
    footerMessage = null;
    try {
      // Desktop-only affordance (canLaunchApps): degrade to a copied path.
      if (!adapter || !adapter.isAvailable("canLaunchApps")) {
        await navigator.clipboard.writeText(prdPath);
        footerMessage = "PRD path copied.";
        return;
      }
      const result = await adapter.shell.openInEditor(prdPath);
  ...
  async function runStory() {
    ...
    try {
      // Desktop-only affordance (canLaunchApps): degrade to a copied prompt.
      if (!adapter || !adapter.isAvailable("canLaunchApps")) {
  ...
  >
    <header class="panel-header">
      <div class="header-copy">
        ...
        <div class="meta-row">
          <span
            class="status-pill tone-{statusTone}"
    ...
    <div class="panel-body">
      ...
      <section class="section" data-testid="task-detail-acceptance">
        <div class="section-title-row">
          ...
          <span data-testid="ac-progress-count"
            >{acComplete}/{acItems.length}</span
          >
        </div>
        {#if acItems.length > 0}
          <div
          ...
          <p class="ac-note" data-testid="ac-readonly-note">
            These criteria complete together when the task-level pass state
            changes. Individual criterion state is not in the current model.
          </p>
        {:else}
