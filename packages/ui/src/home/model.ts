import { isWorkspaceSyncEnabled, type Workspace } from "../chat/workspaces.js";
 ...
 */
export function v4CompanyPrimaryForTab(
  tab: string | undefined | null,
): V4CompanyPrimaryId | null {
  switch (tab) {
    case undefined:
    case null:
    case "":
...
export function v4CompanyDotTone(workspace: Workspace): V4DotTone {
  ...
  if (workspace.kind === "company" && workspace.membershipStatus === "pending")
    return "idle";
 ...
 */
export function accountIdentityFromWorkspaces(
  ...
  const label =
    personalLabel && personalLabel.toLocaleLowerCase() !== "personal"
      ? personalLabel
      : null;
...
export function sortV4CompaniesConnectedFirst(
  ...
  for (const workspace of workspaces) {
    ...
    deduped.push({
      ...
      row: {
        slug: workspace.slug,
        label:
          workspace.kind === "personal" ? "Personal" : workspace.displayName,
        ownerLabel:
        ...
        children:
          active && workspace.membershipStatus !== "pending"
  ...
  deduped.sort((a, b) => {
    ...
    return a.row.label.localeCompare(b.row.label, undefined, {
      sensitivity: "base",
    });
  });
...
export type V4HydrationIssueKind =
  "workspace-list" | "manifest" | "sync-status";
  ...
  switch (input.syncState) {
    ...
    default: {
      ...
      return {
        tone: "ok",
        sentence: "All synced",
        meta: input.lastSyncLabel
          ? `${watched} · ${input.lastSyncLabel}`
          : watched,
        action: syncNow,
      };
    }
