...
import {
  ...
  timeLabel,
  type MeetingEvent,
} from "../meetings/meetings-model.js";
import {
  formatBytes,
  friendlySyncError,
  timeAgo,
 ...
 */
export function getDeleteRefusalCopy(
  refusal: HomeDeleteRefusal,
): HomeDeleteRefusalCopy {
  const explanation =
...
export function getConflictCardModel(
...
export function getDriftCardModel(
  core: HomeCoreState,
  restoring = false,
): HomeCardModel | null {
...
export function getNeedsYouCount(
  ...
  const driftCount =
    !driftDismissed && core && core.driftReport && core.driftReport.count > 0
      ? 1
      : 0;
  ...
  if (input.syncState === "error") {
    return joinMeta([
      ...
      input.autoSyncOn
        ...
        : "retry from the title bar",
      version,
    ]);
  }

  return joinMeta([
    input.autoSyncOn == null
      ? null
      : input.autoSyncOn
        ? "auto-sync on"
        : "auto-sync off",
    input.autoSyncOn ? "syncs on every change" : null,
    input.daemonRunning == null
      ? null
      : input.daemonRunning
        ? "daemon running"
        : "daemon idle",
    input.lastSyncLabel
      ? `last sync ${input.lastSyncLabel.toLowerCase()}`
      : "no syncs yet",
    version,
  ]);
}
...
export function getHomeProgressModel(
  input: HomeProgressInput,
): HomeProgressModel {
  const {
    filesProgressed,
    totalFiles,
  ...
  const pct =
    totalFiles > 0
      ? Math.min(100, Math.max(0, (filesProgressed / totalFiles) * 100))
      : null;
  ...
  for (const company of companies) {
    ...
    const isDone =
      ...
      Boolean(
        stats &&
        (stats.completedFiles > 0 || stats.completedBytes > 0 || stats.aborted),
      );
    ...
    if (isActive) {
      ...
      rows.push({
        ...
        detail:
          planned > 0
            ? `downloading ${file} · ${done} of ${planned}`
            : `downloading ${file}`,
      });
    } else if (isDone) {
...
export function getHomeErrorModel(
  input: HomeErrorInput,
): HomeErrorModel | null {
  ...
  if (input.syncState === "auth-error") {
    return {
      ...
      showSignIn: true,
      techLines: [
        input.syncErrorMessage.trim() ||
          "The saved session can no longer refresh.",
  ...
  else if (input.syncErrorMessage.trim())
    techLines.push(input.syncErrorMessage.trim());
  techLines.push(
    `runner: hq-sync v${input.appVersion} · journal ~/.hq/sync-journal.log`,
  );
  techLines.push(
    `${
      input.lastSyncLabel
        ? `last good sync ${input.lastSyncLabel.toLowerCase()}`
        : "no prior sync"
  ...
  return {
    title: company
      ? `Sync failed for ${company} — ${friendly.summary}`
      : `Sync failed — ${friendly.summary}`,
    sub: showSignIn
...
export function getHomeDigestGroups(
  ...
  for (const entry of activity) {
    ...
    if (group) group.entries.push(entry);
      ...
      const companyName = workspaceDisplayName(
        entry.company,
        workspaces,
        companies,
      );
      groups.set(key, {
        entries: [entry],
        label: actorLabel(entry, companyName),
      });
    }
  }
  ...
  for (const [key, group] of groups) {
    ...
    const companyNames = [
      ...new Set(
        entries.map((entry) =>
          workspaceDisplayName(entry.company, workspaces, companies),
        ),
      ),
    ];
    const where =
    ...
    if (latest.direction !== "up" && latest.author)
      metaParts.push(latest.author);
...
export function getHomePortfolioStats(input: {
  ...
  return [
    ...
    { label: "Open tasks", value: openStories.toLocaleString() },
  ];
}
...
export function getHomeCompanyRows(input: {
  ...
  const byCompany = new Map<
    string,
    { active: number; storiesTotal: number; storiesComplete: number }
  >();
  for (const p of input.projects) {
    const agg = byCompany.get(p.company) ?? {
      active: 0,
      storiesTotal: 0,
      storiesComplete: 0,
    };
  ...
  return portfolioWorkspaces(input.workspaces).map((w) => {
    ...
    const stories =
      agg && agg.storiesTotal > 0
        ? `${agg.storiesComplete.toLocaleString()} / ${agg.storiesTotal.toLocaleString()} tasks`
        : "No tracked tasks";
    return {
      slug: w.slug,
      name: w.displayName,
