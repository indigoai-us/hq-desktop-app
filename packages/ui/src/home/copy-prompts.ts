...
const builders: Record<IssueKind, (i: Issue) => string> = {
  ...
  "workspace-broken": (i) => {
    ...
    return [
      ...
      reason
        ? `Reason: ${reason}`
        : "The manifest cloud_uid does not match cloud reality.",
      "",
  ...
  "hq-core-drift-all": (i) => {
    ...
    const unaccounted = files
      .filter((f) => f.staging === "unaccounted")
      .map((f) => f.path);
    const stagingMain = files
      .filter((f) => f.staging === "staging-main")
      .map((f) => f.path);
