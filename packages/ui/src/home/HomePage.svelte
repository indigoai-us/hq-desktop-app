<script lang="ts">
   ...
   */
  interface Props {
    /** Desktop app version string. The desktop shell passed its build-time
     *  __APP_VERSION__ define; the pure port takes it as a prop. */
    appVersion?: string;
    syncState: SyncState;
  ...
  const driftCard = $derived(
    coreState && !driftDismissed
      ? getDriftCardModel(coreState, driftRestoring)
      : null,
  );
  ...
  async function handleAggregateConflictAction(
    actionId: string,
  ): Promise<void> {
...
<section class="home" aria-label="Home">
  ...
  {#if errorModel}
    <div
      class="home-section"
      aria-label={syncState === "auth-error"
        ? "Keep sync moving"
        : "Sync failed"}
    >
      <h2
        class:warn={syncState === "auth-error"}
    ...
    <div class="home-section" aria-label="Sync in progress">
      ...
      <div class="home-progress" data-testid="home-progress-card">
        ...
        <div
          ...
          aria-valuemin={0}
          aria-valuemax={100}
          aria-valuenow={progressModel.pct == null
            ? undefined
            : Math.round(progressModel.pct)}
        >
          <div
        ...
        <ol class="home-fanout">
          ...
          {#if progressModel.queued}
            <li class="home-fanout-row">
              ...
              <span class="home-fanout-detail"
                >{progressModel.queued.names}…</span
              >
            </li>
          {/if}
        </ol>
    ...
    <div class="home-section" aria-label="Needs you">
      ...
      <div class="home-queue">
        {#each inviteWorkspaces as invite (invite.slug)}
          <NeedsYouCard
            card={getInviteCardModel(
              invite,
              acceptingInviteSlug === invite.slug,
            )}
