...
describe("buildPrompt", () => {
  ...
  describe("hq-core-update-failed prompts", () => {
    ...
    it("threads the log tail + path so the agent can triage", () => {
      const out = buildPrompt({
        ...
        payload: {
          exitCode: 3,
          logTail: "fatal: clone failed",
          logPath: "/tmp/hq-sync-abc.log",
        },
      });
  ...
  it("refers to HQ skills the agent can invoke", () => {
    ...
    expect(buildPrompt({ kind: "sync-conflict" })).toContain(
      "/resolve-conflicts",
    );
    expect(buildPrompt({ kind: "auth-expired" })).toContain("/hq-login");
    expect(buildPrompt({ kind: "sync-failed" })).toMatch(
  ...
  describe("local-env-failure prompts", () => {
    ...
    it("disk-full instructs the agent to free space, not auto-delete", () => {
      ...
      expect(out).toMatch(
        /don.?t auto-delete|recommend.*don.?t|with my confirmation/i,
      );
    });
    ...
    it("node-missing tells the user to install Node before retrying Connect", () => {
      const out = buildPrompt({
        ...
        payload: {
          slug: "x",
          kind: "node-missing",
          detail: "node and npx were not found",
        },
      });
...
describe("parseLocalEnvFailure (IPC contract)", () => {
  ...
  it("handles every known kind", () => {
    ...
    for (const k of kinds) {
      const parsed = parseLocalEnvFailure(
        `local environment failure (${k}): detail text`,
      );
  ...
  it("returns null for unknown kinds (defensive against wire drift)", () => {
    ...
    expect(
      parseLocalEnvFailure(
        "local environment failure (mystery-kind): something",
      ),
    ).toBeNull();
  });
  ...
  it("returns null for empty / malformed input", () => {
    ...
    expect(
      parseLocalEnvFailure("local environment failure: no kind"),
    ).toBeNull();
  });
});
