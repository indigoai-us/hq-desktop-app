...
| `ui/src/settings/PrototypeSettingsPanes.svelte` | Hydrates Dock and desktop-widget toggles from host settings only when their native boolean keys are present, under the tray capability, and never overwrites a user click that races the read. | `ui/src/settings/PrototypeSettingsPanes.host-settings.test.ts` — "hydrates Dock and widget toggles from native settings without driving host setters"; "leaves local toggle values alone when native settings omit booleans"; "keeps a user toggle when the native settings read resolves later" |
