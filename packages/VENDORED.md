...
| `ui/src/shell/DesktopApp.svelte`, `ui/src/shell/embedded-navigation.ts` | `extraPages` — a generic host-destination extension point. A host registers `{ label, detail?, component }` under a page id; the shell contributes a `command-go-<id>` palette row, a full-bleed `view === "extra"` branch in the main column (keyed on id + param), and a new `{ kind: "extra"; page; param? }` embedded-navigation target. An id no host registered raises the existing navigation-error banner rather than a blank column. Sync uses this to mount its Sessions page, so `packages/ui` never imports a host page or `@tauri-apps/*`. | `ui/src/shell/DesktopApp.extra-pages.test.ts` (with its `ExtraPageProbe.test.svelte` stand-in) — "adds one palette row per registered page"; "mounts the registered component when its palette row is activated"; "routes an embedded-navigation extra target and forwards its param"; "remounts the page when a later deep link changes only the param"; "lets the page select its own param through onnavigate"; "reports an unregistered page id instead of painting a blank column"; "changes nothing when the host registers no extra pages" |
| `ui/src/chat/row-extras.ts`, `ui/src/chat/ChatSidebar.svelte`, `ui/src/shell/DesktopApp.svelte`, `ui/src/index.ts` | `rowExtras` — the row-level twin of `extraPages`. A host hands `DesktopApp` one `(row) => { badge?, hoverCard?, actions? } \| null` resolver; the shell forwards it to `ChatSidebar`, which paints the badge after the row title, mounts the host component beside the row while it is hovered (portal, 180 ms leave grace), and appends the actions to the row's context menu. Rows the host returns `null` for are untouched. Sync uses this for project channels ↔ agent sessions ("N sessions" badge, sessions hover card, "New session" action), so `packages/ui` never learns what a session is. | `ui/src/shell/DesktopApp.row-extras.test.ts` (with its `RowExtrasProbe.test.svelte` stand-in) — "shows the host badge after the title, only on rows the host decorates"; "mounts the host hover card with the hovered row, and only for decorated rows"; "appends the host action to the row's context menu and runs it — here, into an extra page"; "has no host action in the menu of an undecorated row"; "changes nothing when the host registers no rowExtras" |
