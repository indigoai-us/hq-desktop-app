...
| `ui/src/settings/PrototypeSettingsPanes.svelte` | Hydrates Dock and desktop-widget toggles from host settings only when their native boolean keys are present, under the tray capability, and never overwrites a user click that races the read. Failed host writes restore the local setting only when the write was attempted and failed, never when the capability is unavailable, never over newer user intent, and only for the latest write to that specific control. | `ui/src/settings/PrototypeSettingsPanes.host-settings.test.ts` — "hydrates Dock and widget toggles from native settings without driving host setters"; "leaves local toggle values alone when native settings omit booleans"; "keeps a user toggle when the native settings read resolves later"; "reverts a failed Dock write and lets the native read hydrate afterward"; "keeps a Dock preference when the host reports it unavailable"; "does not clobber a newer Dock toggle when an earlier write fails"; "reverts a failed desktop widget write"; "keeps the third Dock toggle and its dirty state when the first write fails"; "keeps the third desktop widget toggle and its dirty state when the first write fails"; "reconciles failed Dock and desktop widget writes independently" |
