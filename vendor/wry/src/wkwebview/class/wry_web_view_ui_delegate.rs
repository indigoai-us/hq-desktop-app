...
#[cfg(target_os = "macos")]
use objc2_app_kit::{
  NSAlert, NSAlertStyle, NSModalResponse, NSModalResponseOK, NSOpenPanel, NSWindowDelegate,
};
...
#[cfg(target_os = "macos")]
fn show_file_upload_panel_failure(mtm: MainThreadMarker) {
  let alert = NSAlert::new(mtm);
  alert.setAlertStyle(NSAlertStyle::Warning);
  alert.setMessageText(&NSString::from_str("File picker unavailable"));
  alert.setInformativeText(&NSString::from_str(
    "macOS could not open the file chooser. Please try again.",
  ));
...
define_class!(
  ...
  unsafe impl WKUIDelegate for WryWebViewUIDelegate {
    ...
    fn run_file_upload_panel(
      ...
      unsafe {
        let panel_kind = hq_desktop_core::panel_creation::WEBVIEW_FILE_UPLOAD_PANEL_KIND;
        let Some(mtm) = MainThreadMarker::new() else {
          hq_desktop_core::panel_creation::capture_open_panel_creation_failure(panel_kind, false);
          (*handler).call((null_mut(),));
          return;
        };
        let open_panel = hq_desktop_core::panel_creation::try_create_panel(panel_kind, || {
          // SAFETY: AppKit returns an autoreleased NSOpenPanel here when creation succeeds.
          let ptr: *mut NSOpenPanel = unsafe { msg_send![NSOpenPanel::class(), openPanel] };
          // SAFETY: The Objective-C factory returns either nil or a live NSOpenPanel.
          unsafe { Retained::retain(ptr) }
        });
        let Ok(open_panel) = open_panel else {
          hq_desktop_core::panel_creation::capture_open_panel_creation_failure(panel_kind, true);
          show_file_upload_panel_failure(mtm);
          (*handler).call((null_mut(),));
          return;
        };
