...
pub fn classify_watcher_fault_binary(name: &str) -> WatcherFaultBinary {
    ...
    match base.to_ascii_lowercase().as_str() {
        ...
        // macOS process (`comm`) basenames for the same watcher tree — a
        // JetsamEvent report names the Node runner as `node`/`hq-sync-runner`,
        // its `npx` launcher as `npx`, and the app as `hq-sync-menubar` (no
        // `.exe`). Reuse of the existing closed vocabulary keeps a single
        // allow-list; anything else still collapses to `Other`.
        "node" | "hq-sync-runner" => WatcherFaultBinary::NodeExe,
        "npx" => WatcherFaultBinary::NpxCmd,
        "hq-sync-menubar" => WatcherFaultBinary::HqSyncMenubarExe,
...
// ─────────────────────────────────────────────────────────────────────────────
// macOS jetsam / external-kill attribution (HQ-DESKTOP-4D)
// ─────────────────────────────────────────────────────────────────────────────
//
// On macOS the auto-sync watcher dies with `code=None signal=Some(9)` and the
// whole Windows fault surface above is `not_applicable`, so a jetsam (memory)
// kill was previously indistinguishable from an external `kill -9`. This is the
// pure half of the macOS attribution: the honesty vocabulary, the tolerant
// content-safe JetsamEvent `.ips` parser, and the binding decision. The bounded
// filesystem read that feeds it lives behind `cfg(target_os = "macos")` in
// `apps/sync/src-tauri/src/commands/process.rs`, exactly as the Win32 WER query
// does — so every interpretation is unit-testable off macOS. Content safety is
// identical to the fault module: only fixed tokens, bucketed integers, and bare
// integers ever leave the process; a victim `comm` name is mapped through the
// same closed [`classify_watcher_fault_binary`] allow-list and never copied out.

/// Default page size (bytes) assumed when a JetsamEvent omits `pageSize`. Apple
/// Silicon reports 16 KiB pages; an Intel report carries its own 4 KiB `pageSize`
/// which the parser prefers, so this default only applies to a truncated record.
pub const JETSAM_DEFAULT_PAGE_SIZE_BYTES: u64 = 16 * 1024;

/// Hard cap on victims retained from one report, so a machine-wide jetsam event
/// (which lists every victim process) can never grow this projection without
/// bound. Well above the handful of processes in the watcher tree.
const JETSAM_MAX_VICTIMS: usize = 64;

/// How confidently a macOS `signal=9` watcher death is attributed to a memory
/// (jetsam) kill of THIS generation — and, when nothing binds, precisely why.
/// The macOS counterpart to [`WatcherFaultProvenance`]. Every non-binding state
/// still names no victim beyond an allow-listed token, so absence never
/// masquerades as evidence.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum WatcherKillProvenance {
    /// A JetsamEvent record in the generation window names a recognised watcher
    /// victim whose PID is in the generation's sampled set — strongest binding.
    JetsamPidMatched,
    /// A JetsamEvent record falls in the generation window, but no recognised
    /// victim PID is in the sampled set — a weaker, coincidence-possible bind.
    JetsamWindowMatched,
    /// The reader ran and parsed reports, but none fell in the generation window.
    NoJetsamRecord,
    /// No jetsam report bound, but the generation's memory-pressure samples
    /// reached `critical` — a memory kill is likely even without a parsed record.
    PressureOnly,
    /// No jetsam report bound and memory pressure stayed `normal`: the kill came
    /// from outside the app and outside memory pressure — an external `kill -9`.
    ExternalKillSuspected,
    /// The app's own SIGTERM->SIGKILL escalation fired for this generation, so
    /// this SIGKILL was self-inflicted (defence in depth behind the durable
    /// cancellation record). Never reported as an external kill.
    SelfEscalated,
    /// The DiagnosticReports directories could not be listed/opened at all (TCC,
    /// sandbox, or a hardened runtime) — absence of a reader, not of a kill.
    ReaderUnavailable,
    /// The bounded deferred read reached its deadline before any report bound.
    DeadlineExpired,
    /// The read is deferred off the exit path and has not resolved; emitted only
    /// when a teardown flush preempts the worker. An honest "not read yet".
    Deferred,
    /// No macOS kill read applies (non-macOS platform, or a non-signal exit).
    NotApplicable,
}
...
impl WatcherKillProvenance {
    ...
    /// True only for the two states that bind a report to this generation and
    /// therefore may name a kill reason / victim footprint. Every other state
    /// must render no reason, so a producer bug is caught by the invariant test.
    pub fn is_bound(self) -> bool {
        matches!(self, Self::JetsamPidMatched | Self::JetsamWindowMatched)
    }
}

/// Closed vocabulary for the macOS memorystatus kill reason. The raw reason
/// string (e.g. `"per-process-limit"`, `"vm-pageshortage"`) is normalised and
/// mapped to one of these fixed tokens; anything unrecognised is `Other`, so no
/// report byte can ride through.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum WatcherKillReason {
    PerProcessLimit,
    Highwater,
    VmPageshortage,
    VmThrashing,
...
impl WatcherKillReason {
    pub const ALL: [WatcherKillReason; 5] = [
...
/// Map an untrusted jetsam reason string to a fixed allow-listed token. Only used
/// to SELECT a closed-vocabulary value, so the returned token is always a code
/// constant — no reason byte leaks.
pub fn classify_jetsam_kill_reason(raw: &str) -> WatcherKillReason {
    let normalized = raw.to_ascii_lowercase().replace([' ', '-'], "_");
    if normalized.contains("per_process") {
        WatcherKillReason::PerProcessLimit
    } else if normalized.contains("highwater") {
        WatcherKillReason::Highwater
    } else if normalized.contains("pageshortage") {
        WatcherKillReason::VmPageshortage
    } else if normalized.contains("thrashing") {
        WatcherKillReason::VmThrashing
    } else {
        WatcherKillReason::Other
    }
}

/// Bucket a victim's resident-page count (`rpages` × page size) into the SAME
/// fixed MB buckets [`crate::watcher_fault`]'s Windows job-commit bucket uses, so
/// a macOS jetsam footprint and a Windows commit peak are directly comparable.
/// A bucket, never a raw byte count, keeps the channel content-safe.
pub fn jetsam_rpages_bucket(rpages: u64, page_size_bytes: u64) -> &'static str {
    const MB: u64 = 1024 * 1024;
    let bytes = rpages.saturating_mul(page_size_bytes.max(1));
    match bytes / MB {
        0..=127 => "under_128mb",
...
/// macOS system memory-pressure level, from `kern.memorystatus_vm_pressure_level`
/// (which needs no entitlement). A low-RSS kill under `Normal` pressure is
/// positively distinguishable from a jetsam victim.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum WatcherMemoryPressure {
    Normal,
    Warn,
    Critical,
    Unknown,
...
impl WatcherMemoryPressure {
    pub const ALL: [WatcherMemoryPressure; 4] = [
        Self::Normal,
        Self::Warn,
        Self::Critical,
        Self::Unknown,
    ...
    /// The more severe of two samples, so the supervisor can retain a generation
    /// PEAK across ticks. `Unknown` ranks below every known level so a single
    /// failed read never masks a real `warn`/`critical` sample.
    pub fn max(self, other: Self) -> Self {
        fn rank(level: WatcherMemoryPressure) -> u8 {
            match level {
                WatcherMemoryPressure::Unknown => 0,
                WatcherMemoryPressure::Normal => 1,
                WatcherMemoryPressure::Warn => 2,
                WatcherMemoryPressure::Critical => 3,
            }
        }
...
/// Map the raw `kern.memorystatus_vm_pressure_level` sysctl value to a fixed
/// token. macOS reports 1=normal, 2=warn, 4=critical; anything else is `Unknown`.
pub fn classify_vm_pressure_level(raw: i32) -> WatcherMemoryPressure {
    match raw {
        1 => WatcherMemoryPressure::Normal,
        2 => WatcherMemoryPressure::Warn,
        4 => WatcherMemoryPressure::Critical,
        _ => WatcherMemoryPressure::Unknown,
    }
}

/// A content-safe projection of one victim in a JetsamEvent report. The `comm`
/// name is already mapped to a fixed allow-listed token; `pid` and `rpages` are
/// bare integers. No raw report byte is retained.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct JetsamVictim {
    pub pid: Option<u32>,
    pub name: WatcherFaultBinary,
    pub rpages: Option<u64>,
    pub reason: WatcherKillReason,
}

/// A content-safe projection of a macOS `JetsamEvent-*.ips` report: the report
/// timestamp, the page size, and the (bounded, name-tokenised) victim list. Only
/// what the binding decision needs — never a raw name, path, or reason string.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct JetsamReport {
    pub event_time_unix_ms: Option<i64>,
    pub page_size_bytes: u64,
    pub victims: Vec<JetsamVictim>,
}

impl JetsamReport {
    /// The victims whose `comm` name maps to a known watcher-tree binary — the
    /// only ones this generation could plausibly be. Unrelated third-party
    /// victims (mapped to `Other`) are never selected as this generation's kill.
    pub fn watcher_victims(&self) -> impl Iterator<Item = &JetsamVictim> {
        self.victims
            .iter()
            .filter(|victim| victim.name != WatcherFaultBinary::Other)
    }

    /// PIDs of the recognised watcher victims (bare integers), for PID binding.
    pub fn watcher_victim_pids(&self) -> Vec<u32> {
        self.watcher_victims().filter_map(|victim| victim.pid).collect()
    }
}

/// Parse a macOS JetsamEvent `.ips` document into a content-safe projection.
/// Tolerant of both the legacy single-JSON-object form and the modern
/// header-line + JSON-body `.ips` form, and of the `memoryStatus` wrapper being
/// present or the whole document being the memory record. Returns `None` only
/// when nothing JSON-shaped can be read; a recognisable-but-sparse report still
/// yields whatever it safely can. Never retains a raw name/path/reason byte.
pub fn parse_jetsam_report(text: &str) -> Option<JetsamReport> {
    let value = parse_ips_json(text)?;
    let event_time_unix_ms = value
        .get("timestamp")
        .and_then(|field| field.as_str())
        .and_then(parse_jetsam_timestamp_ms);
    // The memory record may be nested under `memoryStatus` or be the document.
    let memory = value.get("memoryStatus").unwrap_or(&value);
    let page_size_bytes = memory
        .get("pageSize")
        .and_then(|field| field.as_u64())
        .filter(|&size| size > 0)
        .unwrap_or(JETSAM_DEFAULT_PAGE_SIZE_BYTES);
    let mut victims = Vec::new();
    if let Some(processes) = memory.get("processes").and_then(|field| field.as_array()) {
        for process in processes.iter().take(JETSAM_MAX_VICTIMS) {
            let pid = process
                .get("pid")
                .and_then(|field| field.as_u64())
                .and_then(|pid| u32::try_from(pid).ok());
            let name = classify_watcher_fault_binary(
                process.get("name").and_then(|field| field.as_str()).unwrap_or(""),
            );
            let rpages = process.get("rpages").and_then(|field| field.as_u64());
            let reason = process
                .get("reason")
                .and_then(|field| field.as_str())
                .map(classify_jetsam_kill_reason)
                .unwrap_or(WatcherKillReason::Other);
            victims.push(JetsamVictim {
                pid,
                name,
                rpages,
                reason,
...
/// Read one `.ips` document as JSON, tolerating the modern two-part form (a
/// one-line JSON header, then the JSON body). Never interprets anything but JSON.
fn parse_ips_json(text: &str) -> Option<serde_json::Value> {
    let trimmed = text.trim_start();
    if let Ok(value) = serde_json::from_str::<serde_json::Value>(trimmed) {
        return Some(value);
    }
    let (_, body) = trimmed.split_once('\n')?;
    serde_json::from_str::<serde_json::Value>(body.trim_start()).ok()
}

/// Parse a JetsamEvent timestamp to unix milliseconds. Accepts RFC 3339 and the
/// macOS crash-report style (`2026-08-22 00:19:55.00 -0700`). `None` when absent
/// or unparseable, in which case binding treats the record as out-of-window.
fn parse_jetsam_timestamp_ms(raw: &str) -> Option<i64> {
    let raw = raw.trim();
...
/// Bind a parsed JetsamEvent record to THIS watcher generation and return the
/// single most-confident jetsam provenance, or the specific non-binding reason.
///
/// `record_ms` is the report timestamp; `[window_start_ms, window_end_ms]` the
/// generation lifetime as a closed unix-ms window; `victim_pids` the PIDs the
/// report named as recognised watcher victims; `victim_name_token` the strongest
/// recognised victim name token (`node_exe`, `npx_cmd`, …) or the sentinel. A
/// record outside the window is never reported, and a PID match is only claimed
/// when the victim name is a recognised watcher binary — so PID reuse by an
/// unrelated victim can never manufacture a false `jetsam_pid_matched`.
pub fn bind_jetsam_record(
    record_ms: Option<i64>,
    victim_pids: &[u32],
    victim_name_token: &str,
    sampled_pids: &[u32],
    window_start_ms: i64,
    window_end_ms: i64,
) -> WatcherKillProvenance {
    let in_window = record_ms
        .map(|ms| ms >= window_start_ms && ms <= window_end_ms)
        .unwrap_or(false);
    if !in_window {
        return WatcherKillProvenance::NoJetsamRecord;
    }
    let recognised = victim_name_token != WatcherFaultBinary::Other.as_str()
        && victim_name_token != WATCHER_FAULT_UNAVAILABLE;
    let pid_hit = victim_pids.iter().any(|pid| sampled_pids.contains(pid));
    if recognised && pid_hit {
...
/// Fold the pure jetsam binding together with the two live signals a report can
/// never carry — the app's own escalation flag and the generation's peak memory
/// pressure — into the single provenance the event carries.
///
/// `self_escalated` wins outright (the app killed its own child). A real jetsam
/// bind wins next. Otherwise a `no_jetsam_record` absence is resolved by
/// pressure: `critical` reads as `pressure_only`, `normal` as
/// `external_kill_suspected`, and an inconclusive `warn`/`unknown` stays
/// `no_jetsam_record`. Reader-state provenances (`reader_unavailable`,
/// `deadline_expired`, `deferred`, `not_applicable`) pass through unchanged.
pub fn decide_watcher_kill_provenance(
    bound: WatcherKillProvenance,
    self_escalated: bool,
    pressure_peak: WatcherMemoryPressure,
) -> WatcherKillProvenance {
    if self_escalated {
        return WatcherKillProvenance::SelfEscalated;
    }
    match bound {
        WatcherKillProvenance::JetsamPidMatched | WatcherKillProvenance::JetsamWindowMatched => {
            bound
        }
        WatcherKillProvenance::NoJetsamRecord => match pressure_peak {
            WatcherMemoryPressure::Critical => WatcherKillProvenance::PressureOnly,
            WatcherMemoryPressure::Normal => WatcherKillProvenance::ExternalKillSuspected,
            WatcherMemoryPressure::Warn | WatcherMemoryPressure::Unknown => {
...
mod tests {
  ...
  <EventData>
    ...
    fn every_emitted_token_is_denylist_free() {
        ...
        let kill_reason = WatcherKillReason::ALL.map(WatcherKillReason::as_str);
        let pressure = WatcherMemoryPressure::ALL.map(WatcherMemoryPressure::as_str);
        let rpages_buckets = [
            jetsam_rpages_bucket(0, JETSAM_DEFAULT_PAGE_SIZE_BYTES),
            jetsam_rpages_bucket(20_000, JETSAM_DEFAULT_PAGE_SIZE_BYTES),
            jetsam_rpages_bucket(60_000, JETSAM_DEFAULT_PAGE_SIZE_BYTES),
            jetsam_rpages_bucket(100_000, JETSAM_DEFAULT_PAGE_SIZE_BYTES),
            jetsam_rpages_bucket(1_000_000, JETSAM_DEFAULT_PAGE_SIZE_BYTES),
        ];
        for token in binary
            .into_iter()
            .chain(provenance)
    ...
    // A realistic single-document JetsamEvent `.ips`. The watcher tree's Node
    // runner is the largest victim (per-process-limit); an unrelated third-party
    // victim carries a secret-ish name that must NEVER survive parsing; a second
    // node child is a small highwater victim. Only pid/rpages/reason/name-token
    // may ever leave the parser.
    const JETSAM_IPS: &str = r#"{
      "bug_type": "298",
      "timestamp": "2026-08-22 00:19:55.00 -0700",
      "os_version": "macOS 26.5.2",
      "memoryStatus": {
        "pageSize": 16384,
        "processes": [
          { "pid": 4242, "name": "hq-sync-runner", "rpages": 400000, "reason": "per-process-limit" },
          { "pid": 900, "name": "SuperSecret-token-Vault", "rpages": 12345, "reason": "vm-pageshortage" },
          { "pid": 4243, "name": "node", "rpages": 200, "reason": "highwater" }
        ]
      }
    }"#;

    #[test]
    fn classify_watcher_fault_binary_maps_macos_process_basenames() {
        for (name, expected) in [
            ("node", WatcherFaultBinary::NodeExe),
            ("/usr/local/bin/node", WatcherFaultBinary::NodeExe),
            ("hq-sync-runner", WatcherFaultBinary::NodeExe),
            ("npx", WatcherFaultBinary::NpxCmd),
            ("hq-sync-menubar", WatcherFaultBinary::HqSyncMenubarExe),
            ("Finder", WatcherFaultBinary::Other),
        ] {
            assert_eq!(classify_watcher_fault_binary(name), expected, "name: {name:?}");
        }
    }
    ...
    #[test]
    fn kill_provenance_tokens_round_trip_without_gaps() {
        let mut seen = std::collections::HashSet::new();
        for provenance in WatcherKillProvenance::ALL {
            assert!(seen.insert(provenance.as_str()), "duplicate token {provenance:?}");
        }
        assert_eq!(seen.len(), WatcherKillProvenance::ALL.len());
        // Only the two jetsam bindings may name a reason/footprint.
        for provenance in WatcherKillProvenance::ALL {
            let bound = matches!(
                provenance,
                WatcherKillProvenance::JetsamPidMatched | WatcherKillProvenance::JetsamWindowMatched
            );
            assert_eq!(provenance.is_bound(), bound, "is_bound wrong for {provenance:?}");
        }
    }

    #[test]
    fn classify_jetsam_kill_reason_maps_closed_vocabulary() {
        for (raw, expected) in [
            ("per-process-limit", WatcherKillReason::PerProcessLimit),
            ("highwater", WatcherKillReason::Highwater),
            ("vm-pageshortage", WatcherKillReason::VmPageshortage),
            ("vm-thrashing-level", WatcherKillReason::VmThrashing),
            ("diagnostic", WatcherKillReason::Other),
            ("", WatcherKillReason::Other),
        ] {
            assert_eq!(classify_jetsam_kill_reason(raw), expected, "raw: {raw:?}");
        }
    }

    #[test]
    fn jetsam_rpages_bucket_boundaries() {
        let page = JETSAM_DEFAULT_PAGE_SIZE_BYTES; // 16384
        assert_eq!(jetsam_rpages_bucket(0, page), "under_128mb");
        assert_eq!(jetsam_rpages_bucket(8_191, page), "under_128mb"); // 127.98 MB
        assert_eq!(jetsam_rpages_bucket(8_192, page), "128mb_to_512mb"); // exactly 128 MB
        assert_eq!(jetsam_rpages_bucket(40_000, page), "512mb_to_1gb");
        assert_eq!(jetsam_rpages_bucket(400_000, page), "over_2gb"); // ~6.25 GB
        // A zero page size never divides-by-zero or panics.
        assert_eq!(jetsam_rpages_bucket(1, 0), "under_128mb");
    }

    #[test]
    fn classify_vm_pressure_level_and_peak() {
        assert_eq!(classify_vm_pressure_level(1), WatcherMemoryPressure::Normal);
        assert_eq!(classify_vm_pressure_level(2), WatcherMemoryPressure::Warn);
        assert_eq!(classify_vm_pressure_level(4), WatcherMemoryPressure::Critical);
        assert_eq!(classify_vm_pressure_level(0), WatcherMemoryPressure::Unknown);
        assert_eq!(classify_vm_pressure_level(3), WatcherMemoryPressure::Unknown);
        // Peak retention: a real level always beats Unknown; more severe wins.
        assert_eq!(
            WatcherMemoryPressure::Unknown.max(WatcherMemoryPressure::Normal),
            WatcherMemoryPressure::Normal
        );
        assert_eq!(
            WatcherMemoryPressure::Normal.max(WatcherMemoryPressure::Critical),
            WatcherMemoryPressure::Critical
        );
        assert_eq!(
            WatcherMemoryPressure::Critical.max(WatcherMemoryPressure::Warn),
            WatcherMemoryPressure::Critical
        );
    }

    #[test]
    fn parse_jetsam_report_extracts_only_content_safe_fields() {
        let report = parse_jetsam_report(JETSAM_IPS).expect("valid jetsam report parses");
        assert_eq!(report.page_size_bytes, 16384);
        let expected_ms = chrono::DateTime::parse_from_str(
            "2026-08-22 00:19:55.00 -0700",
            "%Y-%m-%d %H:%M:%S%.f %z",
        )
        .unwrap()
        .timestamp_millis();
        assert_eq!(report.event_time_unix_ms, Some(expected_ms));
        assert_eq!(report.victims.len(), 3);
        // The runner victim keeps only its content-safe projection.
        let runner = report.victims[0];
        assert_eq!(runner.pid, Some(4242));
        assert_eq!(runner.name, WatcherFaultBinary::NodeExe);
        assert_eq!(runner.rpages, Some(400_000));
        assert_eq!(runner.reason, WatcherKillReason::PerProcessLimit);
        // The unrelated third-party victim's secret-ish name is discarded to
        // `Other` — its raw bytes never enter the projection.
        let stranger = report.victims[1];
        assert_eq!(stranger.name, WatcherFaultBinary::Other);
        assert_eq!(stranger.name.as_str(), "other");
        // Only the two recognised watcher victims are candidates for binding.
        assert_eq!(report.watcher_victim_pids(), vec![4242, 4243]);
    }

    #[test]
    fn parse_jetsam_report_tolerates_two_part_ips_header() {
        // Modern `.ips`: a one-line JSON header, then the JSON body.
        let two_part = format!("{{\"app_name\":\"hq-sync\"}}\n{JETSAM_IPS}");
        let report = parse_jetsam_report(&two_part).expect("two-part ips parses");
        assert_eq!(report.victims.len(), 3);
        assert_eq!(report.watcher_victim_pids(), vec![4242, 4243]);
    }

    #[test]
    fn bind_jetsam_record_precedence() {
        let (start, end) = (1_000_i64, 2_000_i64);
        // In-window recognised victim whose PID we sampled: strongest.
        assert_eq!(
            bind_jetsam_record(Some(1_500), &[4242], "node_exe", &[4242, 4243], start, end),
            WatcherKillProvenance::JetsamPidMatched
        );
        // In-window recognised victim, PID not in our sampled set: window only.
        assert_eq!(
            bind_jetsam_record(Some(1_500), &[9999], "node_exe", &[4242], start, end),
            WatcherKillProvenance::JetsamWindowMatched
        );
        // In-window but the victim name is not a recognised watcher binary: a PID
        // collision must not manufacture a false pid_matched.
        assert_eq!(
            bind_jetsam_record(Some(1_500), &[4242], "other", &[4242], start, end),
            WatcherKillProvenance::JetsamWindowMatched
        );
        // Out of window, or no timestamp: no record binds.
        assert_eq!(
            bind_jetsam_record(Some(2_500), &[4242], "node_exe", &[4242], start, end),
            WatcherKillProvenance::NoJetsamRecord
        );
        assert_eq!(
            bind_jetsam_record(None, &[4242], "node_exe", &[4242], start, end),
            WatcherKillProvenance::NoJetsamRecord
        );
    }

    #[test]
    fn decide_watcher_kill_provenance_folds_live_signals() {
        use WatcherKillProvenance as P;
        use WatcherMemoryPressure as M;
        // The app's own escalation wins outright, even over a jetsam bind.
        assert_eq!(
            decide_watcher_kill_provenance(P::JetsamPidMatched, true, M::Normal),
            P::SelfEscalated
        );
        // A real jetsam bind is preserved.
        assert_eq!(
            decide_watcher_kill_provenance(P::JetsamPidMatched, false, M::Normal),
            P::JetsamPidMatched
        );
        // No record: pressure resolves the absence honestly.
        assert_eq!(
            decide_watcher_kill_provenance(P::NoJetsamRecord, false, M::Critical),
            P::PressureOnly
        );
        assert_eq!(
            decide_watcher_kill_provenance(P::NoJetsamRecord, false, M::Normal),
            P::ExternalKillSuspected
        );
        for inconclusive in [M::Warn, M::Unknown] {
            assert_eq!(
                decide_watcher_kill_provenance(P::NoJetsamRecord, false, inconclusive),
                P::NoJetsamRecord
            );
        }
        // Reader-state provenances pass through unchanged.
        for reader_state in [P::ReaderUnavailable, P::DeadlineExpired, P::Deferred, P::NotApplicable] {
            assert_eq!(
                decide_watcher_kill_provenance(reader_state, false, M::Critical),
                reader_state
            );
        }
    }
    ...
    #[test]
    fn parse_bind_decide_end_to_end_matches_the_6gb_event() {
        // The 2026-08-22 hq-sync@0.10.137 shape: a ~6.5 GB per-process-limit kill
        // of the Node runner whose PID the generation sampled alive.
        let report = parse_jetsam_report(JETSAM_IPS).unwrap();
        let sampled = [4242_u32, 4243];
        let (gen_start, gen_end) = (
            report.event_time_unix_ms.unwrap() - 300_000,
            report.event_time_unix_ms.unwrap() + 1_000,
        );
        let runner = report.watcher_victims().next().copied().unwrap();
        let bound = bind_jetsam_record(
            report.event_time_unix_ms,
            &[runner.pid.unwrap()],
            runner.name.as_str(),
            &sampled,
            gen_start,
        ...
        assert_eq!(bound, WatcherKillProvenance::JetsamPidMatched);
        assert_eq!(
            decide_watcher_kill_provenance(bound, false, WatcherMemoryPressure::Critical),
            WatcherKillProvenance::JetsamPidMatched
        );
        assert_eq!(runner.reason, WatcherKillReason::PerProcessLimit);
        assert_eq!(
            jetsam_rpages_bucket(runner.rpages.unwrap(), report.page_size_bytes),
            "over_2gb"
        );
    }
}
