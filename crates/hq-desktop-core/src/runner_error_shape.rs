...
///
/// The `~6.16.11` -> `~6.16.21` bump (the manifest-upload runner) was
...
pub const CAUSE_VOCABULARY_SOURCE_VERSION: &str = "~6.16.21";
...
pub enum RunnerErrorCause {
    ...
    // Added when the runner pin moved to ~6.16.21 — hq-cloud 6.16.21 introduced
...
fn cause_from_identifier(raw: &str) -> Option<RunnerErrorCause> {
    Some(match raw {
        ...
        // Added at the ~6.16.21 pin: the manifest-upload contract violation
...
mod tests {
    ...
    fn every_hq_cloud_identity_maps_to_a_distinct_named_cause() {
        ...
        // VaultCredentialScopeError), and from 51 to 52 at ~6.16.21 (added
