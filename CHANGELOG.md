...
- When HQ updates its command-line tool moments after a new version is published, the download can briefly fail because the package registry lists the new version a few seconds before it can hand it out. HQ now treats that short window as the temporary hiccup it is and quietly retries on its next scheduled check, instead of recording it as a failed update. A download that stays broken past that window is still reported.
