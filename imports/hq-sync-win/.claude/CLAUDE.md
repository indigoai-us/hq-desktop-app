...
| `commands/autostart.rs` | Login-item autostart. `ensure_autostart_on_launch()` (called from `main.rs` `.setup()`) idempotently reconciles the `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\HQSync` Registry value with the effective `startAtLogin` pref on every launch — **default-on** (a fresh install autostarts without opening Settings), honouring an explicit `"startAtLogin": false` opt-out (stale Registry value removed). Mirrors the `daemon.rs` `realtime_sync` default-on convention |
...
- **Build (per-arch):** `npm run tauri build -- --target x86_64-pc-windows-msvc` / `--target aarch64-pc-windows-msvc`
- **Bundle outputs:** `src-tauri/target/<triple>/release/bundle/{msi,nsis}/` — Tauri's bundler emits both a Windows Installer (`*.msi`) and an NSIS user-mode installer (`*-setup.exe`) per `tauri.conf.json`'s `bundle.targets`
- **Code signing:** SignTool via the CI workflow (`.github/workflows/release.yml`) — see `docs/RELEASE.md` for the signing-cert + secrets matrix
- **CI:** `.github/workflows/release.yml` runs on `windows-latest`, builds both arches, signs, and uploads MSI + NSIS to a draft GitHub Release
- **Auto-updater:** `latest.json` published to GitHub Releases (signed with `TAURI_SIGNING_PRIVATE_KEY`), generated by `scripts/generate-latest-json.sh`
