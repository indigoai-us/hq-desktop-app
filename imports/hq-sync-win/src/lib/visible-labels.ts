/** Human-readable label helpers for text rendered by the imported Windows UI. */
...
export function sanitizeVisibleIdentifiers(
  ...
  options: { companies?: CompanyLabelSource[]; personLabel?: string | null } = {},
): string {
  let visible = value ?? '';
  ...
  for (const company of companies) visible = visible.split(company.uid).join(company.label);
