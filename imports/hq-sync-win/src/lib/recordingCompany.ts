//! Pure helpers for resolving which company a meeting recording is attributed
//! to, extracted so the rules are unit-testable without Tauri.
//!
//! A default-recording-company resolved from settings (validated against the
//! *active* memberships), a per-meeting override the user can set, and a
//! back-fill that seeds late-loading "detected" rows with the resolved default
//! without clobbering an explicit user choice.
//!
//! ## Why a separate module
//!
//! MeetingsWindow.svelte owns the Tauri `invoke` surface and the recording
//! `$state`. Keeping the *decisions* here — pure functions over plain data —
//! lets `recordingCompany.test.ts` assert the attribution rules with plain
//! fixtures without booting the window.

/**
 * One membership row the user can attribute a recording to. Structurally a
 * subset of MeetingsWindow's `CompanyMembership`, so the active list can be
 * passed straight in.
 */
...
/**
 * The attribution-relevant slice of an active-meeting row. `companyUserSet`
...
/**
 * Filter memberships to the ones the user can actually record for. Invited /
 * suspended rows must not appear in the picker nor be a valid default.
 */
...
/**
 * Whether a "detected" row that loaded before the recording-company context was
 * ready should be back-filled with the resolved default. Only seeds a genuine
 * default onto a non-user-set row that doesn't already carry it — never touches
 * an explicit user choice and never overwrites with `null`.
 */
