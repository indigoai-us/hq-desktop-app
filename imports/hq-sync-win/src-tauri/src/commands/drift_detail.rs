...
pub async fn open_drift_detail(app: AppHandle, report: DriftReport) -> Result<(), String> {
    ...
    // `mut` is consumed only by the macOS cfg block below (which reassigns
    // `builder`); on non-macOS targets the binding is never mutated.
    ...
    // macOS traffic-light title-bar styling. `title_bar_style`,
    // `hidden_title`, and `traffic_light_position` are macOS-only Tauri
    // APIs (they don't exist on the Windows builder). The Windows fork
    // uses the standard decorated frame + Mica/Acrylic vibrancy applied
    // post-build below (see the `#[cfg(target_os = "windows")]` block).
    #[cfg(target_os = "macos")]
    {
        builder = builder
    ...
    // Windows: apply Mica (Win 11) / Acrylic (Win 10 fallback) so the drift
    // window inherits the same backdrop-blur "Liquid Glass" look as the main
    // popover. Reuses the crate-root helper the main window uses. Unlike the
    // macOS AppKit path, window-vibrancy's Windows backend doesn't require the
    // main thread, and the window was just built above, so apply directly.
