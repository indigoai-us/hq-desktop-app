//! Feature gate + data layer for the alt desktop "Company OS" UX surface.
//!
//! Indigo-only gate for the alternate desktop UX in development. Delegates
//! entirely to `feature_gate::is_indigo_user()` — there is no parallel cache
//! (PRD US-001 hard rule: reuse the existing OnceLock cache).
//!
...
//!
//! Beyond the gate, this module exposes the company-scoped data the desktop-alt
//! Company OS surface renders: board (goals/projects/in-flight), activity,
//! deployments, and secrets metadata — plus an aggregated `get_company_summary`
//! that powers the per-company tab counts.
//!
...
/// Open or focus the Indigo-only alternate desktop UX ("Company OS") window.
///
...
/// button) keeps the default screen.
#[tauri::command]
...
/// Window open/focus body, callable from non-command contexts. Keeps the Indigo
/// gate so every path is defense-in-depth.
///
...
pub async fn open_desktop_alt_window_inner(
    ...
    // Build hidden, apply vibrancy, then show — same ordering as the meetings /
    // meeting-permissions windows so the user never sees a flash of the
    // un-styled transparent frame before Mica/Acrylic lands. NO macOS
    // `title_bar_style` — the fork's standard `decorations(true)` frame is used
    // so the window boots on Windows (the upstream `Overlay` enum is macOS-only).
    ...
    // Apply Mica (Win 11) / Acrylic (Win 10) so the window matches the popover
    // and other secondary windows' liquid-glass look. Best-effort — the Svelte
    // view ships a solid-background fallback. (Reuses the crate-root helper.)
...
fn is_safe_deployment_host(host: &str) -> bool {
    host.strip_suffix(&format!(".{HQ_DEPLOY_APP_DOMAIN}"))
        .is_some_and(is_safe_deployment_label)
}
...
impl LiveBoardProject {
    fn status_column(&self) -> BoardColumn {
        // Inbox is the catch-all column. The explicit inbox statuses
        // (`inbox`/`backlog`/`todo`/`to_do`) plus any unrecognised or missing
        // status all fall through to `_ => Inbox`. (The fork's clippy 1.93
        // rejects an explicit `Some(..) | _` or-pattern as redundant, so the
        // documented statuses live in this comment instead of the arm.)
...
mod tests {
    ...
    // NOTE (Windows fork port): the upstream `Workspace` struct grew a `role`
    // field after this story's parity point; the fork's `Workspace` does not
    // carry it yet. The fixture below omits `role` so it compiles against the
    // fork's current struct (content-port: adapt to the fork's CURRENT shape).
