...
// macOS-only: backs the BlockingNotifyGuard CPU-spin mitigation, which is
// itself macOS-only (the Windows toast path has no blocking send).
#[cfg(target_os = "macos")]
...
// tracked separately.
//
// macOS-only: the busy-spin is a mac-notification-sys behaviour, and both the
// share and DM firing paths that use this guard are themselves cfg-gated to
// macOS. Gate the guard too so the Windows build (which fires via
// tauri-plugin-notification, no blocking send) stays warning-clean.
#[cfg(target_os = "macos")]
...
///
/// macOS-only: emitted from the mac-notification-sys hover-dropdown path. The
/// Windows toast path has no per-notification action callback, so neither the
/// const nor the struct below is referenced there — gate both to avoid an
/// unused-code warning on the Windows build.
#[cfg(target_os = "macos")]
        ...
        Ok(r) => {
            ...
            match r.json::<SharedWithMeResponse>().await {
                ...
                Ok(body) => {
                    ...
                    // macOS: fire notifications with hover-to-reveal action-button
                    // dropdowns via mac-notification-sys. cfg-gated — this dep is
                    // macOS-only (see Cargo.toml), so the whole block is excluded on
                    // Windows, which fires via tauri-plugin-notification below.
                    #[cfg(target_os = "macos")]
                    {
                        ...
                        } // end #[cfg(target_os = "macos")] notification block

                        // Windows / non-macOS: fire one native Action-Center toast per
                        // share event via tauri-plugin-notification. Unlike the macOS
                        // path this has no hover-to-reveal action dropdown (the desktop
                        // notification plugin exposes no per-notification action
                        // callback), so the toast is the lightweight surface and the
                        // ShareDetail window opens via the tray badge / popover entry.
                        // The OS toast pipeline honors Windows Focus Assist (Do Not
                        // Disturb) for free; the shareNotifications Settings toggle is
                        // checked upstream of this firing path so OFF means we never
                        // reach here.
                        #[cfg(not(target_os = "macos"))]
                        {
                            ...
                            for evt in &fresh {
                                    ...
                                    .show()
                                {
                                    Ok(()) => log(
                                        LOG_TAG,
                                        &format!("SHARE_NOTIFY_TOAST_SHOWN title={title}"),
                                    ),
                                    Err(e) => {
                    ...
                    // history window. Idempotent by event_id. Only the fresh
                    // (never-notified) subset — the inclusive `?since=` boundary
                    // events are already in history from a prior poll.
