...
pub enum LedgerAction {
    /// The user was shown a desktop notification (Windows toast / glass banner).
    Notified,
    /// The user clicked Record — a local SDK recording was started.
    Recorded,
...
/// Read → [`prune`] (drop entries older than 14 days) → write, under
/// [`LEDGER_LOCK`]. Called once on app launch from `main.rs` setup to bound
/// ledger growth (the module doc's "pruned on app launch" contract). No-op when
/// the ledger file is absent; best-effort on I/O error (a stale ledger that
/// couldn't be pruned is harmless — the suppression windows still expire by
/// timestamp).
