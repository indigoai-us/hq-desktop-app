...
/// not affected. Callers that need a tighter per-request budget can still
/// wrap their call in `tokio::time::timeout`; the client default is the
/// outer safety net that prevents a hung upstream from wedging a poll loop.
