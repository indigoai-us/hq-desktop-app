 ...
 *
 * Windows note: this module is platform-neutral — it is the bridge-side half of
 * the "stop watchdog" (US-002). The frontend half is src/lib/stopWatchdog.ts.
 * Neither depends on any macOS-only API.
 */
...
export function createRecordingTracker() {
  ...
  return {
     ...
     * in MeetingsWindow.svelte / activeMeetings.ts already routes it to the
     * `error` state.
     *
 ...
 * recording when the meeting window closes, but the SDK's own CHANGELOG shows
 * that auto-stop is unreliable per-platform, so a call ending could leave the
 * recording running indefinitely. This turns the existing `meeting-closed`
 * subscription into an action: if the closing window is a currently-tracked
 * active recording, ask the SDK to stop it.
 *
