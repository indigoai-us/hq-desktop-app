 ...
 *      `bundle.externalBin`. It is a real PE32+ executable (required — the
 *      Rust process spawner uses a direct CreateProcess with no shell, so a
 *      .cmd/.bat shim could not be launched). It is self-contained: end users
 ...
 *      FFmpeg/GStreamer DLL stack + gstreamer-1.0/ plugins (~98 MB). Tauri
 *      ships this verbatim via `bundle.resources` (mapped next to the exe), so
 *      the launcher's in-process `import("bridge.mjs")` can `require()` it. We
 *      only ensure it is installed here; placement is Tauri's job.
 *
 * USAGE
 ...
 * payload already installed is a fast no-op (so it is safe to chain from
 * Tauri's `beforeBuildCommand`). Pass --force to always rebuild the launcher.
 *
 * Cross-target note: by default the launcher is built for the HOST's node.exe
 * and named with TARGET_TRIPLE (default x86_64-pc-windows-msvc). The
 * aarch64-pc-windows-msvc matrix leg in release.yml would need an arm64 node
 * to produce a native arm64 launcher; see docs/RELEASE.md for that caveat.
 */
...
function ensureSdkPayload() {
  ...
  if (CHECK_ONLY) {
    fail(
      "@recallai/desktop-sdk runtime missing (no agent-windows.exe). Run an " +
        "install (npm/pnpm) in sidecar/recall-sdk-bridge/ first.",
    );
  }
  log("@recallai/desktop-sdk runtime missing — installing into sidecar/recall-sdk-bridge");
  // npm respects the .npmrc (node-linker=hoisted, minimum-release-age) and runs
...
async function buildLauncher() {
  ...
  // Windows (execFileSync can't spawn `.cmd` without a shell) and would pull
  // from the network. The blob name + sentinel fuse must match what Node's SEA
  // loader looks for.
  ...
  try {
    ...
    fail(
      "postject not installed — run `npm install` in sidecar/recall-sdk-bridge " +
        "(it is a devDependency). See docs/RELEASE.md.",
    );
  }
