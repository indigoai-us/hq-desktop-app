...
#
# `personal/<top-of-rest>/` is always created when missing.
#
...
# path. (On Windows, master-sync emits Git-Bash MSYS symlinks or copy
# fallbacks; `readlink` over Git Bash resolves the MSYS link target the
# same way, so the personal/-target heuristic holds.)
#
...
#
# Windows: master-sync running under Git Bash creates MSYS symlinks (or,
# when developer mode / privileges are unavailable, copy fallbacks that are
# NOT symlinks and so never match `-L`). `readlink` over Git Bash resolves
# an MSYS symlink's target the same way as on Unix, so the personal/-target
# heuristic is identical; a copy-fallback simply isn't seen here (it goes
# through the normal regular-file drift path instead, which is correct).
...
process_one() {
  ...
  if [ "$user_edited" = "1" ]; then
    if [ "$DRY_RUN" = "1" ]; then
      ...
      # Action order mirrors the dry-run classification: CLAUDE.md
      # (diff-append) -> overwrite-safe (rm) -> conflict (quarantine) ->
      # default (rescue to personal/). Chosen so a path matching multiple
      # predicates takes the cheaper outcome first.
      if [ "$rel" = ".claude/CLAUDE.md" ]; then
        rescue_one "$rel"
      elif is_overwrite_safe "$rel"; then
        rm -f "$local_path"
        COUNT_USER_OVERWRITE=$((COUNT_USER_OVERWRITE + 1))
        echo "    overwrite-safe: $rel  ->  upstream wins (no copy preserved)"
      elif is_conflict_class "$rel"; then
        conflict_one "$rel"
      else
        rescue_one "$rel"
